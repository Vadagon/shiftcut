...
const examplesDir = resolve(repoRoot, "registry", ITEM_TYPE_DIRS["shiftcut:example"]);
...
function fileTypeFor(path: string): FileType {
  if (path.endsWith(".html")) return "shiftcut:composition";
  return "shiftcut:asset";
}
...
function writeItem(item: RegistryItem): void {
  if (item.type !== "shiftcut:example") return;
