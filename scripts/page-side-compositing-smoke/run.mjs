 ...
 *   2. The local `@shiftcut/shader-transitions` IIFE bundle carries the
  ...
  if (count < 1) {
    fail(
      "shader-transitions bundle is missing the page-side compositor canary " +
        `("${PAGE_COMPOSITOR_CANARY}"). Rebuild @shiftcut/shader-transitions and re-run.`,
    );
  }
...
function assertCliCanary() {
  ...
  for (const needle of cliCanaries) {
    ...
    const count = Number(buf.toString().trim());
    if (count < 1) {
      fail(`bundled CLI is missing canary "${needle}". Rebuild @shiftcut/cli and re-run.`);
    }
