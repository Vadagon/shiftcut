...
describe("packed manifest verifier", () => {
  ...
  it("derives consumer specifiers from the packed export map", () => {
    assert.equal(packageExportSpecifier("@shiftcut/sdk", "."), "@shiftcut/sdk");
    assert.equal(
      packageExportSpecifier("@shiftcut/sdk", "./adapters/fs"),
      "@shiftcut/sdk/adapters/fs",
    );
    assert.deepEqual(
      listPackedExportContracts([
        {
          packedPackage: {
            name: "@shiftcut/example",
            exports: {
      ...
      [
        { specifier: "@shiftcut/example", typechecked: true },
        { specifier: "@shiftcut/example/runtime", typechecked: false },
      ],
    );
  });

  function withPackedFiles(files, packedFiles, callback) {
    const dir = mkdtempSync(join(tmpdir(), "shiftcut-pack-test-"));
    try {
