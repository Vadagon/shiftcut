...
describe("release prepare actions", () => {
  ...
  it("blocks on review when generated TODOs are still present", () => {
    assert.equal(
      resolveStableReleaseAction({
        missingArtifacts: [],
        unreviewedArtifacts: ["docs/changelog.mdx#ShiftCut v1.2.3"],
      }),
      "review",
    );
  });
...
describe("reviewed changelog detection", () => {
  ...
  it("checks only the matching docs changelog entry", () => {
    const docs = `
<Update label="ShiftCut v1.2.4">
Reviewed summary.
</Update>

<Update label="ShiftCut v1.2.3">
${CHANGELOG_REVIEW_TODO}
</Update>
`;

    assert.equal(docsChangelogEntryHasGeneratedTodo(docs, "ShiftCut v1.2.3"), true);
    assert.equal(docsChangelogEntryHasGeneratedTodo(docs, "ShiftCut v1.2.4"), false);
  });
});
