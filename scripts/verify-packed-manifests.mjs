...
function writeConsumerFixture(packDir, packedWorkspaces) {
    ...
    JSON.stringify(
      {
        ...
        type: "module",
        dependencies,
        // Each packed tarball pins its inter-@shiftcut deps to the exact
        ...
        // every @shiftcut/* to the sibling local tarball so the check is
  ...
  writeFileSync(
    ...
    `const specifiers = ${JSON.stringify(specifiers, null, 2)};\n` +
      ...
      `const sdk = await import("@shiftcut/sdk");\n` +
      `if (typeof sdk.openComposition !== "function") throw new Error("SDK root export missing");\n` +
      `const memory = await import("@shiftcut/sdk/adapters/memory");\n` +
      `if (typeof memory.createMemoryAdapter !== "function") throw new Error("memory adapter missing");\n` +
      `const fsAdapter = await import("@shiftcut/sdk/adapters/fs");\n` +
      `if (typeof fsAdapter.createFsAdapter !== "function") throw new Error("fs adapter missing");\n` +
      `await import("@shiftcut/core");\n` +
      `await import("@shiftcut/parsers");\n` +
      `await import("@shiftcut/lint");\n` +
      `await import("@shiftcut/studio-server");\n` +
...
function main() {
  const packDir = mkdtempSync(join(tmpdir(), "shiftcut-pack-"));
  try {
