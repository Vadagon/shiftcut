...
<script>
  document.getElementById("hero").src = __shiftcut.getVariables().heroVideo;
</script>
```
...
<video id="hero" data-start="0" data-track-index="0"></video>
<script>
  const { heroVideo, heroDuration } = __shiftcut.getVariables();
...
| Frame rate | `--fps` flag on `shiftcut render`, or `config.fps` in the SDK |
...
Inside any composition script, call `window.__shiftcut.getVariables()` to get the resolved variable values. The return type is `Partial<Record<string, unknown>>` — use destructuring with defaults matching the declared `default` values:
...
<html data-composition-variables='[
  ...
  <body>
    <div data-composition-id="card" data-width="1920" data-height="1080">
      ...
      <script>
        const { title = "Untitled", color = "#111827" } = __shiftcut.getVariables();
...
`__shiftcut.getVariables()` is a shorthand for `window.__shiftcut.getVariables()` and works in both top-level and sub-composition scripts. The runtime automatically scopes sub-compositions so each instance sees its own resolved values.
...
```bash Terminal
# Inline JSON
npx shiftcut render --variables '{"title":"Q4 Report","color":"#1d4ed8"}' --output q4.mp4

# JSON file
npx shiftcut render --variables-file ./vars.json --output out.mp4

# Fail on undeclared or mistyped variables
npx shiftcut render --variables '{"title":"Q4 Report"}' --strict-variables --output out.mp4
```
...
Each row is treated like a `--variables` object and merged over the composition defaults. Output paths support `{key}` placeholders from the row plus `{index}`. Shiftcut validates missing placeholders, output collisions, and `--strict-variables` issues before the first row starts rendering, then writes `manifest.json` next to the outputs with one status row per render.
...
| CLI `--variables` flag | Highest | `shiftcut render --variables '{...}'` |
...
If you are building tooling on top of `@shiftcut/core`, the variable declarations are readable without rendering:
...
- **Handoff** — copy the effective values as JSON or as a ready-to-run
  `shiftcut render --variables` command.
