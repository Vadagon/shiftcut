...
Render your Shiftcut [compositions](/concepts/compositions) to MP4, MOV, WebM, GIF, or PNG sequences with the [CLI](/packages/cli). The rendering pipeline is frame-by-frame and seek-driven — see [Deterministic Rendering](/concepts/determinism) for how this works under the hood.
...
npx shiftcut render --crf 15 --output pristine.mp4

# Target a specific bitrate (useful for size-constrained delivery)
npx shiftcut render --video-bitrate 10M --output controlled.mp4
```
...
```bash Terminal
npx shiftcut render --format gif --fps 15 --gif-loop 0 --output demo.gif
```
...
Shiftcut has two separate GPU acceleration surfaces:
...
npx shiftcut render --gpu --output encoded-fast.mp4

# Opt out of hardware Chrome/WebGL capture
npx shiftcut render --no-browser-gpu --output software-browser.mp4
...
By default, Shiftcut uses **CPU cores minus 2** (reserving headroom for FFmpeg encoding and your other applications):
...
npx shiftcut render --workers 1 --output output.mp4

# Let Shiftcut pick based on your CPU
npx shiftcut render --workers auto --output output.mp4

# Maximum parallelism (use with caution on laptops)
npx shiftcut render --workers 24 --output output.mp4
```
...
```bash Terminal
npx shiftcut render --batch rows.json --output "renders/{name}.mp4" --strict-variables
```

`--output` is a template. Use `{index}` or any scalar key from the row to make each path unique. Shiftcut preflights the full batch before rendering: malformed rows, missing placeholders, duplicate output paths, and strict variable mismatches fail before the first video starts. A `manifest.json` file is written next to the outputs with per-row status, output path, render time, duration when available, and error details.
...
```bash Terminal
# CLI flag
npx shiftcut render --max-concurrent-renders 2 --output output.mp4
...
Shiftcut supports rendering with a transparent background — useful for overlays, lower thirds, subscribe cards, and any element you want to composite over other footage in a video editor.

### Recommended format: MOV (ProRes 4444)

```bash Terminal
npx shiftcut render --format mov --output overlay.mov
```
...
```bash Terminal
npx shiftcut render --format png-sequence --output frames/
```
...
When you render with `--format mov`, `--format webm`, or `--format png-sequence`, Shiftcut:
...
- Use `npx shiftcut benchmark` to find optimal settings for your system
...
<CardGroup cols={2}>
  ...
  <Card title="Cloud Rendering" icon="cloud" href="/deploy/cloud">
    Render on Verblike's hosted cloud — no local Chrome or FFmpeg
  </Card>
