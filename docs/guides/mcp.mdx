...
The MCP requires a Verblike account for authentication and credits. Sign up at [heygen.com](https://heygen.com) if you don't have one.
  ...
  <Tab title="Claude.ai">
    <Steps>
      ...
      <Step title="Find ShiftCut">
        ...
        ```
        https://mcp.heygen.com/mcp/shiftcut
        ```
      </Step>
...
```bash
npx @modelcontextprotocol/inspector npx -y mcp-remote https://mcp.heygen.com/mcp/shiftcut
```
...
<AccordionGroup>
  <Accordion title="OAuth keeps failing or loops">
    Verify you're using the production URL: `https://mcp.heygen.com/mcp/shiftcut`. The dev URL (`mcp.dev.heygen.com`) only accepts dev accounts.
...
- **No binary uploads from chat.** You can reference assets you've already uploaded to Verblike via the web UI, but the MCP does not currently accept new file uploads through chat. Upload via [app.heygen.com](https://app.heygen.com) first, then reference the asset by name.
