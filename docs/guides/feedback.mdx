...
description: "How ShiftCut collects feedback, what data is collected, and how to opt out."
---

ShiftCut occasionally asks how likely you are to recommend the render experience or Studio. This page explains why we do it, when prompts appear, what data is collected, and how to disable them.
...
After a successful `shiftcut render`, a short prompt may appear:

```
  How likely are you to recommend ShiftCut? [0=not likely 10=extremely likely, enter to skip]
...
SHIFTCUT_FEEDBACK_INTERVAL=5 shiftcut render --output out.mp4
```
...
```
VITE_SHIFTCUT_FEEDBACK_INTERVAL=3
```

Invalid values (non-integer, zero, negative) fall back to the default.

**Note:** Disabling CLI telemetry (`shiftcut telemetry disable`) does not suppress the Studio bar — Studio feedback is gated separately. The submitted data is still sent through the same anonymous PostHog pipeline, so the same privacy guarantees apply.

### `shiftcut feedback` command

You can submit feedback manually at any time:

```bash
# Quick rating
shiftcut feedback --rating 10

# Rating with details
shiftcut feedback --rating 7 --comment "render succeeded but GSAP timeline didn't animate text overlay"
```
...
This command collects a doctor summary automatically, flushes telemetry, and exits. It appears under the **Settings** group in `shiftcut --help`.
...
```bash
shiftcut feedback --rating 3 --comment "GSAP timeline froze on seek" --file-issue
```
...
2. Publishes a minimal repro of the project and gets a public URL (the same upload as `shiftcut publish`).
...
When an AI agent is detected, ShiftCut **skips the interactive readline prompt** and prints a structured hint instead:

```
  [shiftcut] Agent feedback: shiftcut feedback --rating <0-10> --comment "..."
```

Agents can then submit feedback using the `shiftcut feedback` command above.
...
The `doctor_summary` is a compact string with environment context — included automatically so you don't need to run `shiftcut doctor` when reporting a problem:
...
The CLI persists feedback state in `~/.shiftcut/config.json`:
...
Studio stores the equivalent state in `localStorage` under the `shiftcut-studio:` prefix.
...
```bash
# Via CLI command (persisted to ~/.shiftcut/config.json)
shiftcut telemetry disable

# Or via environment variable (per-session)
SHIFTCUT_NO_TELEMETRY=1 shiftcut render --output out.mp4

# Or via DO_NOT_TRACK (respects the global standard)
DO_NOT_TRACK=1 shiftcut render --output out.mp4
```

Once disabled, the `shiftcut feedback` command will print `Telemetry is disabled. Feedback not sent.` and exit without sending anything.
...
shiftcut render --quiet --output out.mp4
```
...
```
VITE_SHIFTCUT_FEEDBACK_INTERVAL=20
```

## Related

- [Telemetry](/packages/cli#telemetry) — full telemetry settings and what usage data is collected
- [`shiftcut feedback`](/packages/cli#feedback) — CLI command reference
