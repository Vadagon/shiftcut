...
description: "Fast reference for video editors and creative people directing agents, cutting timing, tweaking layouts, previewing, and publishing ShiftCut projects."
---

Use this as a fast reference when you are directing agents, cutting timing, making visual layout tweaks, previewing, and sharing ShiftCut projects.
...
npx shiftcut lint
npx shiftcut check
npx shiftcut render --quality standard --output review.mp4
```

For fast iteration renders, use draft quality:

```bash
npx shiftcut render --quality draft --output draft.mp4
```

For final delivery:

```bash
npx shiftcut render --quality high --fps 30 --output final.mp4
```
...
Common ShiftCut project folders:
...
Run ShiftCut commands from the project root, where `index.html` lives. If you are not sure where you are, run `pwd` then `ls`. If you see `index.html`, you are in the right place.
...
| Command | Use it for |
| --- | --- |
| `npx shiftcut init my-video` | Create a new project |
| `npx shiftcut init my-video --example warm-grain` | Start from a visual template |
| `npx shiftcut init my-video --video source.mp4` | Import video and generate captions from the source audio |
| `npx shiftcut capture https://example.com` | Capture a website as source material for a video |
| `npx shiftcut preview` | Open the live Studio preview |
| `npx shiftcut lint` | Catch structural mistakes before preview or render |
| `npx shiftcut check` | Run the composition in headless Chrome to catch runtime errors |
| `npx shiftcut check` | Find text overflow and layout problems across the timeline |
| `npx shiftcut snapshot --at 1,3,5` | Save PNG checks at exact timestamps |
| `npx shiftcut render --output final.mp4` | Render the video |
| `npx shiftcut publish` | Upload the project and get a shareable ShiftCut URL |
| `npx shiftcut doctor` | Check Node.js, FFmpeg, Chrome, Docker, and other dependencies |
| `npx shiftcut docs` | Open local CLI docs |
| `npx shiftcut upgrade` | Check for a newer CLI version |
...
| Goal | Command |
| --- | --- |
| Fast iteration | `npx shiftcut render --quality draft --output draft.mp4` |
| Review link | `npx shiftcut render --quality standard --output review.mp4` |
| Final export | `npx shiftcut render --quality high --fps 30 --output final.mp4` |
| Transparent overlay | `npx shiftcut render --format webm --output overlay.webm` |
| Deterministic output | `npx shiftcut render --docker --output final.mp4` |
...
Publish zips the current project, uploads it, and prints a stable `shiftcut.verblike.com` URL. The URL includes a claim token so the recipient can open it, claim the project, and continue editing in the web app.

```bash
npx shiftcut publish ./my-video   # publish a specific folder
npx shiftcut publish --yes        # skip the confirmation prompt in scripts
```
...
Use it when you want the agent to open the ShiftCut Studio preview, play or scrub the video, click timeline controls, inspect visible UI text, capture screenshots for review, or record proof of a tested flow.

For editor-facing changes, keep `npx shiftcut preview` running, then have the agent use `agent-browser` against the local preview URL.

## Quick Fixes

| Problem | Command or check |
| --- | --- |
| Preview will not start | `npx shiftcut doctor` |
| Port already in use | `npx shiftcut preview --port 4567` |
| Render fails | `npx shiftcut lint` then `npx shiftcut check` |
| Need exact frame checks | `npx shiftcut snapshot --at 1,2.5,5` |
| Text overflows in the frame | `npx shiftcut check` |
| Final render is too slow | Try `--quality draft`, reduce image sizes, or lower `--fps` |
| Need to share editable project | `npx shiftcut publish` |
