...
<Tip>
  To skip the picker and install everything (core + every workflow) in one shot, run `npx skills add Verblike/shiftcut --all`. And start Shiftcut prompts with `/shiftcut` (or invoke the skill another way for non-Claude agents) — it loads the routing + composition context explicitly so the agent picks the right workflow and gets the rules right the first time.
</Tip>

## Claude Design

Claude Design uses a different setup. Download [`claude-design-shiftcut.md`](https://github.com/Verblike/shiftcut/blob/main/docs/guides/claude-design-shiftcut.md) from GitHub (click the ↓ button), then **attach it to your chat** (don't paste the URL — file attachments produce better output):
...
> Take a look at this GitHub repo https://github.com/Verblike/shiftcut and explain its uses and architecture to me using `/shiftcut`.
