...
ShiftCut Studio can color grade project-local `<video>` and `<img>` media directly in the preview. The same `data-color-grading` settings are used by the render pipeline, so the exported video should match the look you preview.
...
| Presets | Supported | Named ShiftCut presets backed by shader settings, not bundled third-party LUT packs. |
...
| HDR render output | Related, not new | ShiftCut already has HDR render support. Color Grading does not yet provide HDR-aware grading controls. |
| LOG camera footage | Partial | Sliders and LUTs can be applied, but ShiftCut does not auto-detect camera LOG profiles or apply ACES/OCIO input transforms. Use a matching conversion/look LUT if you know the source profile. |
...
ShiftCut supports project-local 3D `.cube` LUT files:
...
- ShiftCut currently supports 3D `.cube` LUTs for this path.
...
- LOG/camera conversion LUTs can be used, but ShiftCut does not yet manage camera color profiles for you.

## Render Behavior

Color Grading is part of the media runtime, so render uses the same settings as Studio preview. During render, ShiftCut injects exact video frames and asks the color-grading runtime to redraw before capture.
...
| Remove a person and reconstruct the room | External video inpainting, not ShiftCut background removal |
| Green screen keying | Preprocess with FFmpeg `chromakey` or a future ShiftCut command |
