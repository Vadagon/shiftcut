---
title: Figma Import
description: "Bring Figma designs into ShiftCut — frozen assets, brand tokens, editable components, storyboard reconstruction, and Figma Motion timelines translated to GSAP."
---

The work your designer already did in Figma — layout, color, type, motion — becomes the starting point of a composition instead of a thing to rebuild by hand. Point at a Figma URL; the artifact lands as a native ShiftCut piece: a frozen local file, a composition variable, editable HTML, or a paused GSAP timeline.

## What you can import

| Capability | What you get | Surface |
| --- | --- | --- |
| **Static assets** | A frame/layer rendered to SVG/PNG/JPG/PDF, frozen under `.media/` | `shiftcut figma asset` |
| **Brand tokens** | Figma variables/styles as composition brand variables | `shiftcut figma tokens` (Enterprise) or the `/figma` skill via MCP (any plan) |
| **Components** | A frame as editable HTML with brand-linked colors | `shiftcut figma component` |
...
Needed for anything you run from the `shiftcut figma` CLI.
...
```bash
shiftcut figma asset 'https://www.figma.com/design/KEY/Title?node-id=1-2'
```
...
```bash
shiftcut figma component 'https://www.figma.com/design/KEY/Title?node-id=10-20'
```
