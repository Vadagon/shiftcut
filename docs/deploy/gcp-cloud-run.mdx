...
description: "Deploy distributed ShiftCut rendering to Google Cloud Run + Cloud Workflows, and drive renders from a laptop or CI."
---

ShiftCut ships a Google Cloud deployment that mirrors the [AWS Lambda](/deploy/aws-lambda) one: a single Cloud Run service fronts a Cloud Workflows definition that fans renders out across many parallel chunk workers, with intermediate artifacts in Google Cloud Storage. The render primitives are identical — only the storage, compute, and orchestration adapters differ.

It's the right choice for teams already running their backend and storage on Google Cloud who want distributed ShiftCut rendering without adding AWS infrastructure.
...
gcloud builds submit . \
  --tag us-central1-docker.pkg.dev/PROJECT/Verblike/shiftcut-render:v1
...
terraform apply \
  ...
  -var image=us-central1-docker.pkg.dev/PROJECT/Verblike/shiftcut-render:v1
```
...
import {
  renderToCloudRun,
  getRenderProgress,
} from "@shiftcut/gcp-cloud-run/sdk";
