...
description: "Render a composition on Verblike's hosted cloud — no local Chrome, no FFmpeg, no AWS to manage."
---

Render any ShiftCut composition on Verblike's managed cloud: the CLI zips your project, uploads it, runs the render on Verblike's infrastructure, and downloads the finished video. There's nothing to deploy and no Chrome or FFmpeg to install — you pay per credit.

```bash
shiftcut auth login            # one-time sign-in
shiftcut cloud render          # zip → upload → render → download
```
...
This is the zero-infra alternative to running your own renderer. If you'd rather own the compute, see [AWS Lambda](/deploy/aws-lambda), [GCP Cloud Run](/deploy/gcp-cloud-run), or the [Vercel, Cloudflare, or Modal templates](/guides/deploy). For local iteration during authoring, use [`shiftcut render`](/guides/rendering).

## Authenticate

Cloud rendering needs a Verblike credential. Sign in once — the CLI stores it in `~/.heygen/credentials` (mode `0600`), and the same credential drives every `cloud` subcommand.
...
<Steps>
  <Step title="Sign in">
    ...
    echo "$HEYGEN_API_KEY" | shiftcut auth login --api-key
    ```
  </Step>
...
The credential is **shared with the [`heygen` CLI](https://github.com/Verblike/heygen-cli)** — sign in with one and the other picks up the session. Credentials resolve in this order (first match wins):
...
<Note>
  Point the CLI at a different backend with `HEYGEN_API_URL` (default `https://api.heygen.com`). Use `shiftcut auth refresh` to force-refresh an OAuth token before a long job; `shiftcut auth logout` clears the stored credential. For the keys voice, music, and capture use across the skills — and the fully local fallback — see [Authentication & API keys](/guides/authentication).
</Note>

## How a cloud render flows

`shiftcut cloud render` runs the whole pipeline end-to-end:

```
 Your machine                          Verblike cloud
┌─────────────────────────┐          ┌─────────────────────────────────┐
│ zip project             │ ──PUT───▶│ direct-to-S3 asset upload       │
│ (.shiftcutignore +   │  upload  │  → asset_id                     │
│  generated outputs)     │          │                                 │
│                         │ ──POST──▶│ /v3/shiftcut/renders         │
...
5. **Submit** the render to `POST /v3/shiftcut/renders`.
...
Hosted cloud project uploads are limited to 200 MB. ShiftCut automatically excludes root-level `renders/` and `snapshots/` plus development-only paths such as `.git`, `node_modules`, `dist`, `.next`, `coverage`, and dotfiles.

Use a gitignore-style `.shiftcutignore` at the project root for additional generated or intermediate files that are not needed at render time:
...
```bash
shiftcut cloud render . --dry-run
shiftcut cloud render . --dry-run --json
```

The dry run reports compressed size, file count, and the ten largest included files. Rules also apply to `shiftcut publish`. Avoid broad patterns such as `assets/`: dynamically selected media may not appear as an obvious static HTML reference.

## Render options

The most-used flags — see the [CLI reference](/packages/cli#shiftcut-cloud) for the full list.
...
Cloud rendering supports [variables](/concepts/variables) — the same mechanism that powers templates everywhere else in ShiftCut. Declare `data-composition-variables` on your composition, then fill them at render time:

```bash
# Inline JSON
shiftcut cloud render --variables '{"title":"Q4 Recap","theme":"dark"}'

# From a file
shiftcut cloud render --variables-file ./vars.json

# Fail fast on undeclared keys or wrong types
shiftcut cloud render --variables '{"title":"Q4 Recap"}' --strict-variables
```

For a **local project**, the CLI validates your `--variables` against the composition's declared schema *before* uploading. For `--asset-id` / `--url` the schema lives server-side, so mismatches surface as a `shiftcut_project_invalid` API error.
...
shiftcut cloud render ./card-template

# 2. Re-render the same asset with new values (skips zip + upload).
shiftcut cloud render --asset-id asst_abc123 --variables '{"name":"Ada"}'
shiftcut cloud render --asset-id asst_abc123 --variables '{"name":"Linus"}'
```
...
#    Poll with: shiftcut cloud get hfr_def456
```
...
```bash
shiftcut cloud list                 # recent renders (--limit, --token, --all)
shiftcut cloud get hfr_def456       # full detail + short-lived signed video_url
shiftcut cloud delete hfr_def456    # soft-delete (--no-confirm to skip the prompt)
```
...
- **`shiftcut render`** (local) — fastest iteration loop; use while authoring. See [Rendering](/guides/rendering).
- **`shiftcut cloud render`** — zero-infra; Verblike runs the render and you pay per credit. Use when you don't want to manage Chrome/FFmpeg/AWS.
- **`shiftcut lambda render`** — bring-your-own-AWS distributed rendering with chunked parallelism. Use when you've already invested in AWS. See [AWS Lambda](/deploy/aws-lambda).
...
<CardGroup cols={2}>
  ...
  <Card title="CLI reference" icon="terminal" href="/packages/cli#shiftcut-cloud">
