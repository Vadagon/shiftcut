...
ShiftCut templates are compositions that take typed variables — a name, a colour, a chart payload, a CTA URL — and produce a finished render parameterised by those values. Pair a template with the deployed Lambda stack and `lambda render-batch`, and you get personalised-video-at-scale in one CLI call:
...
This guide walks the full loop: declare variables on a composition, iterate locally with `shiftcut render`, deploy to Lambda once, then fan out N renders from a batch file. The same flow also drives single personalised renders via `lambda render --variables` and programmatic batches via `renderToLambda({ variables })`.

```mermaid
flowchart LR
  A["Local iteration<br/>shiftcut render --variables"] --> B["Deploy stack<br/>shiftcut lambda deploy"]
  B --> C["Upload site once<br/>shiftcut lambda sites create"]
  C --> D["Fan out renders<br/>shiftcut lambda render-batch"]
  D --> E["N personalised videos<br/>in S3"]
```

## What's a template

A template is just a ShiftCut composition whose top-level HTML element declares a `data-composition-variables` attribute listing the variables it accepts. The composition reads the runtime values via `window.__shiftcut.getVariables()`.
...
<body style="margin:0;background:#f6f5f1">
  ...
  <script>
    (function () {
      var v = window.__shiftcut.getVariables();
...
The runtime helper is exposed as a global — `window.__shiftcut.getVariables()` — not as a fetchable module. Use a plain `<script>` (not `<script type="module">`) so the runtime is already initialized when the script executes.
...
Fast iteration is the whole point of templates — you should not have to deploy to Lambda to see how a value looks. Use `shiftcut render` locally with `--variables` (or `--variables-file`) to render the template against any payload:

```bash
shiftcut render --variables '{"title":"Hello Alice","accentColor":"#ff0000"}' \
...
```bash
shiftcut render --variables-file ./alice.json --strict-variables \
  --output renders/alice-preview.mp4
```

## Deploying to Lambda

Templates render on the standard `shiftcut lambda` stack — there's no special template-only deployment. Run:
...
```bash
shiftcut lambda sites create ./my-template
...
```bash
shiftcut lambda render ./my-template \
  --site-id abc1234deadbeef0 \
  --width 1920 --height 1080 \
...
`--wait` streams progress lines until the render finishes; without it the CLI returns immediately and you poll via `shiftcut lambda progress <renderId>`.
...
```bash
shiftcut lambda progress arn:aws:states:us-east-1:1234:execution:hf:hf-render-abcd
```
...
The same surface is available from TypeScript via `@shiftcut/aws-lambda/sdk`. Deploy the site once and parallel-render the batch:

```typescript
import { deploySite, renderToLambda } from "@shiftcut/aws-lambda/sdk";
...
const siteHandle = await deploySite({
  ...
  bucketName: process.env.SHIFTCUT_BUCKET!,
});
...
const handles = await Promise.all(
  users.map((user) =>
    renderToLambda({
      siteHandle,
      bucketName: process.env.SHIFTCUT_BUCKET!,
      stateMachineArn: process.env.SHIFTCUT_SFN_ARN!,
      config: {
        fps: 30,
        width: 1920,
        height: 1080,
...
If your typed-data payload genuinely exceeds 256 KiB (e.g. a long structured record per render with no media), [file an issue](https://github.com/Verblike/shiftcut/issues/new) — there's a clean path via S3-hosted variable files, but we want to see real demand before designing the API.
...
In-process vs distributed crossover: for a single render under ~30 seconds, the in-process renderer (`shiftcut render`) wins on latency because there's no S3 round-trip per chunk. Distributed wins for renders over ~60 seconds or when you need a personalised batch — that's the whole reason this surface exists. (The Phase 7 small-render shortcut, when it lands, will collapse the gap for short renders.)

## Migrating from @remotion/lambda inputProps

Remotion's `inputProps` API and ShiftCut' `variables` are isomorphic — both are JSON objects injected as render-time overrides on top of declared composition defaults. The mapping is mechanical:

| Remotion | ShiftCut |
|----------|-------------|
| `Composition.defaultProps` | `data-composition-variables` declaration on the root HTML element |
| `useCurrentFrame()` + `props.<x>` | `window.__shiftcut.getVariables().<x>` (read once on DOMContentLoaded) |
...
- **TypeScript types generated from `data-composition-variables`**: `shiftcut types generate <projectDir>` is sketched and may land in v1.5; it would let SDK callers `import type { Variables } from "./template/variables"` for autocomplete + typecheck.
...
If your template pipeline hits a wall the docs don't cover, [file an issue on GitHub](https://github.com/Verblike/shiftcut/issues/new) — the batch surface is new and the feedback loop on it is short.
