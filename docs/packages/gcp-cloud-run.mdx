---
title: "@shiftcut/gcp-cloud-run"
description: "Google Cloud Run and Workflows adapter for distributed ShiftCut rendering."
---

The GCP Cloud Run package runs ShiftCut' distributed render primitives on Google Cloud. It ships a Cloud Run HTTP service, a Node client SDK, and a Terraform module for provisioning the bucket, service, workflow, and service accounts.

```bash
npm install @shiftcut/gcp-cloud-run
```

## When to Use

**Use `@shiftcut/gcp-cloud-run` when you need to:**
...
| Import | Description |
|--------|-------------|
| `@shiftcut/gcp-cloud-run` | Server handler, event types, GCS transport, and client SDK exports |
| `@shiftcut/gcp-cloud-run/server` | Cloud Run HTTP service entry point |
| `@shiftcut/gcp-cloud-run/sdk` | Lightweight Node SDK for deploying sites, starting renders, polling progress, and estimating cost |
...
The service is intentionally close to the AWS Lambda adapter: thin cloud transport around the same `@shiftcut/producer/distributed` primitives.
...
```typescript
import { getRenderProgress, renderToCloudRun } from "@shiftcut/gcp-cloud-run/sdk";

const handle = await renderToCloudRun({
  projectDir: "./my-composition",
  config: { fps: 30, width: 1920, height: 1080, format: "mp4" },
  bucketName: "shiftcut-render-my-project",
