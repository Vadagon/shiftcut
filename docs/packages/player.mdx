---
title: "@shiftcut/player"
description: "Embeddable web component for playing ShiftCut compositions in any web page."
---

The player package provides a `<shiftcut-player>` custom element that embeds a ShiftCut composition anywhere — in any framework or plain HTML. Zero dependencies, 3KB gzipped.
...
**Use `@shiftcut/player` when you need to:**
...
- Preview during development — use the [CLI](/packages/cli) (`npx shiftcut preview`)
...
```html
<shiftcut-player src="/compositions/intro.html" controls></shiftcut-player>
```
...
```vue
<template>
  <shiftcut-player :src="compositionUrl" controls />
</template>
...
This is the canonical way to bridge the player into editor tools like [`@shiftcut/studio`](/packages/studio). The studio exports a `resolveIframe` helper that handles both direct iframe refs and web-component refs:
...
<Warning>
  **Common gotcha** — if you pass the `<shiftcut-player>` element itself (not `iframeElement`) into a hook or API that expects an `<iframe>`, every `.contentWindow` / `.contentDocument` access returns `null` because the iframe lives inside the player's Shadow DOM. Timeline seek, play, pause, and DOM inspection all silently no-op. **Always extract `iframeElement` first**, or use `resolveIframe` from `@shiftcut/studio` which handles both iframe and web-component hosts transparently.
</Warning>
...
The player communicates with the composition via the ShiftCut runtime bridge protocol (`postMessage`). Existing compositions work without modification.
