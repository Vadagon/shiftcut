---
title: "@shiftcut/aws-lambda"
description: "AWS Lambda and Step Functions adapter for distributed ShiftCut rendering."
---

The AWS Lambda package runs ShiftCut' distributed render primitives on AWS. It ships the Lambda handler, a Node client SDK, and an optional CDK construct for provisioning the render stack inside an adopter's AWS account.

```bash
npm install @shiftcut/aws-lambda
```

## When to Use

**Use `@shiftcut/aws-lambda` when you need to:**
...
| Import | Description |
|--------|-------------|
| `@shiftcut/aws-lambda` | Handler types, runtime helpers, S3 transport, and client SDK exports |
| `@shiftcut/aws-lambda/handler` | Lambda handler entry point for Step Functions dispatch |
| `@shiftcut/aws-lambda/sdk` | Lightweight Node SDK for deploying sites, starting renders, polling progress, and estimating cost |
| `@shiftcut/aws-lambda/cdk` | Optional CDK construct for provisioning the render bucket, Lambda, and Step Functions stack |

`aws-cdk-lib` and `constructs` are optional peer dependencies. SDK-only consumers do not need them unless they import from `@shiftcut/aws-lambda/cdk`.
...
```typescript
import { deploySite, getRenderProgress, renderToLambda } from "@shiftcut/aws-lambda/sdk";
...
const handle = await renderToLambda({
  ...
  stateMachineArn: "arn:aws:states:us-east-1:123456789012:stateMachine:shiftcut-render",
  config: {
    fps: 30,
    width: 1920,
    height: 1080,
...
const render = new ShiftcutRenderStack(stack, "ShiftcutRender", {
