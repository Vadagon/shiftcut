---
title: "@shiftcut/studio"
description: "Visual composition editor with live preview, timeline view, and hot reload."
---

The studio package provides a browser-based visual editor for creating and previewing Shiftcut compositions. It gives you a real-time preview of your video, a visual timeline of all clips, and player controls for seeking and playback — all updating live as you edit your HTML.
...
**Use `@shiftcut/studio` when you need to:**
- Build a custom composition editor UI (e.g., embedded in your own web application)
- Integrate the Shiftcut preview player into a larger product
- Extend the editor with custom panels, toolbars, or integrations

**Use a different package if you want to:**
- Preview compositions during development — use the [CLI](/packages/cli) (`npx shiftcut preview`), which launches the studio for you
...
<Tip>
  **For most development workflows, you do not need to install the studio directly.** Running `npx shiftcut preview` starts the studio automatically with hot reload. Install `@shiftcut/studio` only if you are embedding the editor into your own application.
</Tip>
...
bun run --filter @shiftcut/studio dev
```
...
| Import | Description |
|--------|-------------|
| `@shiftcut/studio` | React components, hooks, and types |
| `@shiftcut/studio/tailwind-preset` | Tailwind CSS preset for studio styling |
...
```typescript
import { NLELayout, NLEPreview, CompositionBreadcrumb } from '@shiftcut/studio';
import type { CompositionLevel } from '@shiftcut/studio';
...
import {
  ...
  PlayerControls,
  Timeline,
} from '@shiftcut/studio';
import type { TimelineElement } from '@shiftcut/studio';
...
```typescript
import { SourceEditor, PropertyPanel, FileTree } from '@shiftcut/studio';
...
```typescript
import { StudioApp } from '@shiftcut/studio';
...
```typescript
import { useTimelinePlayer } from '@shiftcut/studio';
...
```typescript
import { usePlayerStore, liveTime, formatTime } from '@shiftcut/studio';
...
```typescript
import { useElementPicker } from '@shiftcut/studio';
...
The studio renders your composition in an iframe using the Shiftcut runtime. What you see in the preview is exactly what will be captured during rendering — the same runtime code, the same seek logic, the same clip lifecycle.
...
1. **Iframe preview** — your composition HTML is loaded in an isolated iframe with the Shiftcut runtime injected. This ensures the preview uses the same rendering path as production.
...
3. **Timeline component** — parses the composition using `@shiftcut/core` to extract clip timing data and renders the visual timeline panel.
...
```typescript
// tailwind.config.ts
import studioPreset from '@shiftcut/studio/tailwind-preset';
...
<CardGroup cols={2}>
  <Card title="CLI" icon="terminal" href="/packages/cli">
    Launches the studio via `npx shiftcut preview` — the easiest way to preview compositions.
  </Card>
