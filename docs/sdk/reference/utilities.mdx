...
description: "History module, persist queue, document utilities, constants, and error types exported from @shiftcut/sdk."
---

This page covers everything exported from `@shiftcut/sdk` that is not covered in [`openComposition`](/sdk/reference/open-composition), the [`Composition` interface](/sdk/reference/composition), [edit operations](/sdk/reference/edit-operations), [types](/sdk/reference/types), or [adapters](/sdk/reference/adapters).
...
```typescript
import { createHistory } from "@shiftcut/sdk";
import type { HistoryModule, HistoryOptions, HistoryEntry } from "@shiftcut/sdk";
```
...
```typescript
import { openComposition, createHistory } from "@shiftcut/sdk";
...
```typescript
import { createPersistQueue } from "@shiftcut/sdk";
import type { PersistQueueModule, PersistQueueOptions } from "@shiftcut/sdk";
```
...
```typescript
import { openComposition, createPersistQueue } from "@shiftcut/sdk";
...
```typescript
import { buildDocument, buildRoots, flatElements } from "@shiftcut/sdk";
```

Low-level utilities for building the `SdkDocument` model and `ShiftCutElement` trees from parsed HTML. These are the same functions the SDK uses internally on every `openComposition()` call. You rarely need them directly — they are exposed for hosts that parse HTML outside of a session, or for testing.
...
```typescript
function buildRoots(document: Document): ShiftCutElement[];
```
...
```typescript
function flatElements(roots: readonly ShiftCutElement[]): ShiftCutElement[];
```
...
```typescript
import { buildDocument, flatElements } from "@shiftcut/sdk";
...
```typescript
import { resolveScoped, findById, bareId, escapeHfId, isNewHostBoundary } from "@shiftcut/sdk";
```
...
```typescript
import { readVariableDefault } from "@shiftcut/sdk";
```
...
```typescript
import { ORIGIN_APPLY_PATCHES } from "@shiftcut/sdk";

const ORIGIN_APPLY_PATCHES = "@shiftcut/sdk:applyPatches";
```
...
The value is a namespaced string rather than a `Symbol` so it survives realm boundaries — `postMessage`, structured clone, and JSON serialization all preserve it correctly. T3 embedded hosts that forward patch events across frames or workers rely on this property. The namespace prefix (`@shiftcut/sdk:`) makes accidental collision with a host-chosen origin string negligible.

### ORIGIN\_LOCAL

```typescript
import { ORIGIN_LOCAL } from "@shiftcut/sdk";
...
```typescript
import { UnsupportedOpError } from "@shiftcut/sdk";
```
...
```typescript
import { resolveNearestHfElement } from "@shiftcut/sdk";
...
```typescript
import { resolveElementAffordances } from "@shiftcut/sdk/editing";
```

Determines which editing operations are available for a live element given its current DOM state and model. Imported from the `@shiftcut/sdk/editing` subpath. Full documentation is in the [Editing Affordances guide](/sdk/guides/editing-affordances).
