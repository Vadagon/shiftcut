---
title: "Types"
description: "All exported types from @shiftcut/sdk."
---

This page documents every type exported from `@shiftcut/sdk`. Types are verified against `packages/sdk/src/types.ts` and the export barrel at `packages/sdk/src/index.ts`.
...
A stable ShiftCut element identifier — the value of the `data-hf-id` attribute stamped on every editable element by `ensureHfIds`. Use `HfId` wherever you would write a raw `string` to signal that the value is an element id.
...
<ResponseField name="children" type="readonly ShiftCutElement[]">
  Direct child elements. Build a flat list with `flatElements()` from `@shiftcut/sdk` utilities.
</ResponseField>
...
```typescript
type ElementSnapshot = ShiftCutElement;
```

Type alias for `ShiftCutElement`. Used in return types of `getElements()` and `getElement()` to signal that the value is a read-only snapshot — it does not update after subsequent mutations.
...
<ResponseField name="roots" type="readonly ShiftCutElement[]">
...
A single keyframe entry for `addWithKeyframes` and `replaceWithKeyframes`. This is a structural shape, not a runtime export — you pass plain objects of this form to those methods; it is **not** re-exported from the `@shiftcut/sdk` barrel, so there is nothing to import.
