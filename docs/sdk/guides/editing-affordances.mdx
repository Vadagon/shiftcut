...
<Note>
  This API lives on the `@shiftcut/sdk/editing` subpath, available since `@shiftcut/sdk@0.7.22`. If the import fails to resolve, upgrade: `npm install @shiftcut/sdk@latest`.
</Note>
...
function resolveElementAffordances(
  liveEl: HTMLElement,
  modelEl: Pick<ShiftCutElement, "text" | "animationIds" | "start"> | null,
...
**Browser-only.** `resolveElementAffordances` calls `getComputedStyle` internally. It must not be imported or called in Node, a server action, or any non-browser path. The SDK adapter (`@shiftcut/sdk/editing`) is separate from the pure core resolver precisely to keep the Node SDK path free of browser globals.
...
The full type definitions — `EditingAffordances`, `DomEditCapabilities`, `EditingSectionApplicability`, `AffordanceContext`, `ShiftCutElement` — are documented at [`/sdk/reference/types`](/sdk/reference/types).

The pure DOM-free resolver (`resolveEditingAffordances`, `EditableElementFacts`) lives in `@shiftcut/core` and is documented at [`/packages/core`](/packages/core).
