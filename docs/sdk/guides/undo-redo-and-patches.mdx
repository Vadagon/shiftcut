...
| `ORIGIN_APPLY_PATCHES` | `"@shiftcut/sdk:applyPatches"` | Automatically applied by `applyPatches()` |

Both are exported from `@shiftcut/sdk`.
...
`ORIGIN_APPLY_PATCHES` is a namespaced string (`"@shiftcut/sdk:applyPatches"`) rather than a `Symbol` so it survives `postMessage`, structured clone, and JSON round-trips — important when the host forwards patch events across frames or workers.
...
```typescript
import { openComposition, ORIGIN_APPLY_PATCHES } from "@shiftcut/sdk";
