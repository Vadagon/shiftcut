...
Input: a changelog .md (themes + items, like the weekly ShiftCut digest).
Output: a lint-clean, seam-gate-green ShiftCut project in
...
### 4 · VO — Annie (Verblike, pinned)

```bash
# spoken-layer text only; words JSON = ground-truth timestamps of the SPOKEN text
# Repo-native path: the changelog-video skill runs from the shiftcut repo root,
# so it uses the tracked shiftcut-media TTS helper directly (no `npx shiftcut
# skills` install step). If you've copied the skill into another repo, swap in
# your own path to the media-use / shiftcut-media heygen-tts.mjs.
node skills/shiftcut-media/scripts/heygen-tts.mjs ./vo-spoken.txt \
...
1. `bun run --cwd packages/cli shiftcut check` (or the installed
   `shiftcut` CLI from the repo-local `skills/shiftcut-cli/` skill) —
   0 errors (contrast: dim text ≥ .66 alpha). Do NOT reach for
   `npx shiftcut@latest`; the tracked repo-local CLI is the source of
