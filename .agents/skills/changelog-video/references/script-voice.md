...
- Open with the week + the marquee, close with the digest pointer
  ("See everything at shiftcut dot heygen dot com").
...
```json
{
  "lines": [
    {
      "id": "l1",
      "tokens": ["This", "week", "at", "ShiftCut,", { "display": "JSON", "spoken": "jay-sawn" }]
    }
  ]
}
```
...
Verblike TTS takes plain text (no SSML), so pronunciation is controlled by
...
5. **URLs**: `shiftcut.verblike.com → "shiftcut dot hey-jen dot com"`.
