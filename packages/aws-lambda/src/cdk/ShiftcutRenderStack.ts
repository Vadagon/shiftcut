/**
 * `ShiftcutRenderStack` — aws-cdk-lib L2 construct that emits the same
 * topology as `examples/aws-lambda/template.yaml`.
 *
 * Adopters who embed ShiftCut inside their own CDK app can extend this
 ...
 * `@shiftcut/aws-lambda` don't pull the CDK tree at runtime.
 *
 * Drift from the SAM template is guarded by the snapshot test
 * (`ShiftcutRenderStack.snapshot.test.ts`), which diffs the synthed
...
/** Construction-time props for {@link ShiftcutRenderStack}. */
export interface ShiftcutRenderStackProps {
  /** Name prefix applied to function / state-machine / alarm names. Default `"shiftcut"`. */
  projectName?: string;
...
export class ShiftcutRenderStack extends Construct {
  ...
  constructor(scope: Construct, id: string, props: ShiftcutRenderStackProps = {}) {
    super(scope, id);

    const projectName = props.projectName ?? "shiftcut";
    ...
    this.renderFunction = new lambda.Function(this, "RenderFunction", {
      ...
      environment: {
        ...
        TMPDIR: "/tmp",
        SHIFTCUT_LAMBDA_CHROME_SOURCE: chromeSource,
        SHIFTCUT_RENDER_BUCKET: this.bucket.bucketName,
      },
    });
 ...
 * `node_modules/@shiftcut/aws-lambda/` install.
 */
