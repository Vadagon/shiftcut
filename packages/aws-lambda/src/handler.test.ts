 ...
 * `@shiftcut/producer/distributed` and have their own coverage in
...
import type { AssembleResult, ChunkResult, PlanResult } from "@shiftcut/producer/distributed";
...
describe("handler dispatch", () => {
  it("routes Action='plan' to the plan primitive", async () => {
    ...
    const result = await handler(event, {
      ...
      primitives: {
        plan: planMock as unknown as typeof import("@shiftcut/producer/distributed").plan,
        renderChunk:
          renderChunkMock as unknown as typeof import("@shiftcut/producer/distributed").renderChunk,
        assemble:
          assembleMock as unknown as typeof import("@shiftcut/producer/distributed").assemble,
      },
      tmpRoot,
  ...
  it("routes Action='assemble' to the assemble primitive", async () => {
    ...
    const result = await handler(event, {
      ...
      primitives: {
        plan: mock(async () => {
          throw new Error("should not be called");
        }) as unknown as typeof import("@shiftcut/producer/distributed").plan,
        renderChunk: mock(async () => {
          throw new Error("should not be called");
        }) as unknown as typeof import("@shiftcut/producer/distributed").renderChunk,
        assemble:
          assembleMock as unknown as typeof import("@shiftcut/producer/distributed").assemble,
      },
      tmpRoot,
...
describe("handler — S3 URI allowlist (security: F-004)", () => {
  ...
  beforeEach(() => {
    prevBucket = process.env.SHIFTCUT_RENDER_BUCKET;
  });

  afterEach(() => {
    if (prevBucket === undefined) {
      delete process.env.SHIFTCUT_RENDER_BUCKET;
    } else {
      process.env.SHIFTCUT_RENDER_BUCKET = prevBucket;
    }
  });

  it("rejects a plan event whose ProjectS3Uri is outside the allowed bucket", async () => {
    process.env.SHIFTCUT_RENDER_BUCKET = "good-bucket";
    const tmpRoot = makeTmpRoot();
    const s3 = new FakeS3Client();
