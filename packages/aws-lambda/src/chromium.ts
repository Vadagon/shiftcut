 ...
 *
 * The runtime path is selected by the `SHIFTCUT_LAMBDA_CHROME_SOURCE`
 * env var (set by `build-zip.ts`):
 *
 *   "sparticuz"          → use `@sparticuz/chromium.executablePath()`
 *   "chrome-headless-shell" → use `process.env.SHIFTCUT_LAMBDA_CHROME_PATH`
 *
 * Adapters that bundle this package can override
 * `SHIFTCUT_LAMBDA_CHROME_PATH` directly when running outside Lambda
 ...
 * see {@link ShiftcutRenderStack}'s NON_RETRYABLE_* lists.
 */
...
const SPARTICUZ_WEDGE_HINT =
  ...
  "bumped marker var, or redeploy via `shiftcut lambda deploy --skip-build`) to force fresh " +
...
export function resolveChromeSource(): ChromeSource {
  const raw = process.env.SHIFTCUT_LAMBDA_CHROME_SOURCE?.toLowerCase();
 ...
 * `SHIFTCUT_LAMBDA_CHROME_PATH`. Throws if absent or non-existent so a
...
export async function resolveChromeExecutablePath(): Promise<string> {
  ...
  const explicit = process.env.SHIFTCUT_LAMBDA_CHROME_PATH;
  if (!explicit) {
    throw new ChromeBinaryUnavailableError(
      source,
      null,
      "SHIFTCUT_LAMBDA_CHROME_SOURCE=chrome-headless-shell requires " +
        "SHIFTCUT_LAMBDA_CHROME_PATH to be set to the absolute path of the bundled binary.",
    );
  }
  if (!existsSync(explicit)) {
    throw new ChromeBinaryUnavailableError(
      source,
      explicit,
      `SHIFTCUT_LAMBDA_CHROME_PATH=${JSON.stringify(explicit)} does not exist on disk.`,
    );
  }
  return explicit;
}
