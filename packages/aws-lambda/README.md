...
AWS Lambda adapter for ShiftCut distributed rendering. Ships three
things together:
...
3. An **`aws-cdk-lib` L2 construct** (`ShiftcutRenderStack`) that
...
│   handler.mjs                                                    │
│     ├─ Action="plan"        → @shiftcut/producer/distributed  │
│     ├─ Action="renderChunk" → @shiftcut/producer/distributed  │
│     └─ Action="assemble"    → @shiftcut/producer/distributed  │
...
The handler reads `SHIFTCUT_LAMBDA_CHROME_SOURCE` at boot. The build
...
ShiftCut' renderer drives Chrome via the CDP
...
```ts
import { deploySite, getRenderProgress, renderToLambda } from "@shiftcut/aws-lambda";

// One-time upload per project version.
const site = await deploySite({
  projectDir: "./my-composition",
  bucketName: "shiftcut-render-bucket",
});
...
const handle = await renderToLambda({
  ...
  stateMachineArn: "arn:aws:states:us-east-1:123:stateMachine:shiftcut-render",
  config: {
    fps: 30,
    width: 1920,
    height: 1080,
...
import { ShiftcutRenderStack } from "@shiftcut/aws-lambda/cdk";

const app = new App();
const stack = new Stack(app, "MyApp");
const render = new ShiftcutRenderStack(stack, "Render", {
...
`@shiftcut/aws-lambda/cdk`.

## What's still ahead

- `shiftcut lambda` CLI (deploy / sites create / render / progress / destroy) — PR 6.5.
