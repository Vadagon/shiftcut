...
Embeddable web component for playing ShiftCut compositions. Zero dependencies, works with any framework.

## Install

```bash
npm install @shiftcut/player
```
...
```html
<script src="https://cdn.jsdelivr.net/npm/@shiftcut/player/dist/shiftcut-player.global.js"></script>
```

## Usage

```html
<shiftcut-player src="./my-composition/index.html" controls></shiftcut-player>
```
...
```typescript
import "@shiftcut/player";

// The custom element is now registered — use it in your markup
// React: <shiftcut-player src="..." controls />
// Vue:   <shiftcut-player :src="url" controls />
```
...
When a composition uses `@shiftcut/shader-transitions`, the player can own preview-only shader capture settings:
...
```js
const player = document.querySelector("shiftcut-player");
...
This is the canonical way to bridge the player into tools like [`@shiftcut/studio`](../studio). The studio exports a `resolveIframe` helper that works with both iframe refs and web-component refs:

```ts
import { useTimelinePlayer, resolveIframe } from "@shiftcut/studio";
...
```tsx
import "@shiftcut/player";
import type { ShiftcutPlayer } from "@shiftcut/player";
import { useTimelinePlayer, resolveIframe } from "@shiftcut/studio";

function StudioPreview({ src }: { src: string }) {
  const { iframeRef, onIframeLoad } = useTimelinePlayer();
  const playerRef = useRef<ShiftcutPlayer>(null);

  useEffect(() => {
    iframeRef.current = resolveIframe(playerRef.current);
  });

  return <shiftcut-player ref={playerRef} src={src} onLoad={onIframeLoad} />;
}
```

> **Heads up — common gotcha**
>
> If you pass the `<shiftcut-player>` element itself (not `iframeElement`) into a hook that expects an `<iframe>`, every `.contentWindow` / `.contentDocument` access returns `null` because the iframe lives inside the player's Shadow DOM. Always extract `iframeElement` first, or use `resolveIframe` from `@shiftcut/studio` which handles both iframe and web-component hosts transparently.
...
The player renders compositions in a sandboxed `<iframe>` inside a Shadow DOM. It communicates with the ShiftCut runtime via `postMessage`. If the composition has GSAP timelines (`window.__timelines`) but no runtime, the player auto-injects it from CDN.

## Distribution

| Format | File                           | Use case                       |
| ------ | ------------------------------ | ------------------------------ |
| ESM    | `shiftcut-player.js`        | Bundlers (Vite, webpack, etc.) |
| CJS    | `shiftcut-player.cjs`       | Node.js / require()            |
| IIFE   | `shiftcut-player.global.js` | `<script>` tag, CDN            |
