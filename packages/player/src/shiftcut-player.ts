...
import { runtimeProtocolMetadata } from "@shiftcut/core/runtime/protocol";
...
class ShiftcutPlayer extends HTMLElement {
   ...
   * `<shiftcut-player>` element itself returns `null` (Shadow DOM).
   */
  ...
  private _isSlideshowPlayer(): boolean {
    return this.closest("shiftcut-slideshow") !== null;
  }
  ...
  private _rescale() {
    ...
    if (!applied && this._ready && !this._rescaleWarned) {
      ...
      console.warn("[shiftcut-player] rescale no-op after ready — zero-size player element", {
...
if (!customElements.get("shiftcut-player")) {
  customElements.define("shiftcut-player", ShiftcutPlayer);
}

export { ShiftcutPlayer };
