...
import { dropInvalidSlides } from "./shiftcut-slideshow.js";
...
// packages/player/src/shiftcut-player.test.ts.)

describe("<shiftcut-slideshow>", () => {
  beforeEach(async () => {
    await import("./shiftcut-slideshow.js");
  });
  ...
  function makeEl(opts: {
    ...
    const el = document.createElement("shiftcut-slideshow") as any;
  ...
  it("is registered as a custom element", () => {
    expect(customElements.get("shiftcut-slideshow")).toBeDefined();
  });
  ...
  it("mute and stopMedia affect only media owned by that slideshow", () => {
    ...
    const attachPlayerMedia = (slideshow: HTMLElement, id: string) => {
      const player = document.createElement("shiftcut-player") as any;
      player.id = id;
      player.muted = false;
    ...
    const slideshowA = document.createElement("shiftcut-slideshow") as any;
    const slideshowB = document.createElement("shiftcut-slideshow") as any;
...
// ---------------------------------------------------------------------------
describe("<shiftcut-slideshow> presenter mode", () => {
  beforeEach(async () => {
    localStorage.clear();
    await import("./shiftcut-slideshow.js");
  });
  ...
  function makeMediaSyncedEl(mode: "presenter" | "audience", opts: { playRejects?: boolean } = {}) {
    ...
    const player = document.createElement("shiftcut-player");
...
describe("<shiftcut-slideshow> deferred init (Bug 1)", () => {
  beforeEach(async () => {
    await import("./shiftcut-slideshow.js");
  });
  ...
  it("connectedCallback defers init to a macrotask so parser-appended children are found", async () => {
    ...
    const el = document.createElement("shiftcut-slideshow") as HTMLElement & {
  ...
  it("renders initial nav chrome from the manifest before scene metadata arrives", async () => {
    ...
    const fakePlayer = document.createElement("shiftcut-player");
...
// ---------------------------------------------------------------------------
describe("<shiftcut-slideshow> Fix 1 — hotspot id/label XSS escape", () => {
  beforeEach(async () => {
    await import("./shiftcut-slideshow.js");
  });
...
// ---------------------------------------------------------------------------
describe("<shiftcut-slideshow> Fix 2 — presenter text XSS escape", () => {
  beforeEach(async () => {
    await import("./shiftcut-slideshow.js");
  });
...
// ---------------------------------------------------------------------------
describe("<shiftcut-slideshow> Fix 3 — controller dispose on lifecycle", () => {
  beforeEach(async () => {
    await import("./shiftcut-slideshow.js");
  });
...
// ---------------------------------------------------------------------------
describe("<shiftcut-slideshow> Fix 5 — waitForReady timeout", () => {
  beforeEach(async () => {
    await import("./shiftcut-slideshow.js");
  });
...
// ---------------------------------------------------------------------------
describe("<shiftcut-slideshow> Fix 6 — keydown form-control guard", () => {
  beforeEach(async () => {
    await import("./shiftcut-slideshow.js");
  });
...
// ---------------------------------------------------------------------------
describe("<shiftcut-slideshow> Fix 7 — audience mode ignores window postMessage nav", () => {
  beforeEach(async () => {
    await import("./shiftcut-slideshow.js");
  });
...
// ---------------------------------------------------------------------------
describe("<shiftcut-slideshow> Fix #1 — chrome re-appended on reconnect", () => {
  beforeEach(async () => {
    await import("./shiftcut-slideshow.js");
  });
...
// ---------------------------------------------------------------------------
describe("<shiftcut-slideshow> Fix #14 — controller and offChange nulled on disconnect", () => {
  beforeEach(async () => {
    await import("./shiftcut-slideshow.js");
  });
...
// ---------------------------------------------------------------------------
describe("<shiftcut-slideshow> Fix #2 — malformed island try/finally", () => {
  beforeEach(async () => {
    await import("./shiftcut-slideshow.js");
  });
...
// ---------------------------------------------------------------------------
describe("<shiftcut-slideshow> Fix #4/#6/#9 — epoch counter cancels stale init", () => {
  beforeEach(async () => {
    await import("./shiftcut-slideshow.js");
  });
...
describe("dropInvalidSlides — phantom slide filtering", () => {
  ...
  ): import("@shiftcut/core/slideshow").ResolvedSlide {
...
// ---------------------------------------------------------------------------
describe("<shiftcut-slideshow> conditional prev/next buttons", () => {
  beforeEach(async () => {
    await import("./shiftcut-slideshow.js");
  });
...
// ---------------------------------------------------------------------------
describe("<shiftcut-slideshow> Fix 4 — back affordance (postMessage only; chrome breadcrumb/back removed)", () => {
  beforeEach(async () => {
    await import("./shiftcut-slideshow.js");
  });
...
// ---------------------------------------------------------------------------
// Mechanical `interactive` attribute on inner <shiftcut-player>
// ---------------------------------------------------------------------------
// The slideshow auto-applies the `interactive` attribute to every inner
// <shiftcut-player>, so clickable controls, links, native media controls,
...
// ---------------------------------------------------------------------------
describe("<shiftcut-slideshow> auto-sets `interactive` on inner <shiftcut-player>", () => {
  beforeEach(async () => {
    await import("./shiftcut-slideshow.js");
  });
  ...
  it("inner <shiftcut-player> gets `interactive` attribute after mount", async () => {
    const el = document.createElement("shiftcut-slideshow");
    const player = document.createElement("shiftcut-player");
    el.appendChild(player);
    document.body.appendChild(el);
  ...
  it("dynamically-inserted <shiftcut-player> children also get `interactive`", async () => {
    const el = document.createElement("shiftcut-slideshow");
    document.body.appendChild(el);
