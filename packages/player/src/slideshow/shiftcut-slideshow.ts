...
export class ShiftcutSlideshow extends HTMLElement {
  ...
  connectedCallback(): void {
    ...
    // parsed before we query for <shiftcut-player>. This matters when the
    // bundle is loaded synchronously (e.g. <script src> in <head>), where
    // connectedCallback fires while the parser is still inside the
    // <shiftcut-slideshow> open tag — before its children exist. A microtask
  ...
  present(): void {
    ...
    if (!this.openAudienceTab(url.href)) {
      console.warn("[shiftcut-slideshow] present(): browser blocked the audience tab");
      return;
    }
  ...
  private async init(): Promise<void> {
    ...
    try {
      const playerEl = this.querySelector("shiftcut-player");
      ...
      if (errors.length > 0) {
        console.warn("[shiftcut-slideshow] manifest errors:", errors);
      }
      const cleaned = dropInvalidSlides(resolved);
      if (cleaned.slides.length === 0 && manifest.slides.length > 0) {
        console.error(
          "[shiftcut-slideshow] no main-line slides resolved — the scene timeline may not have loaded in time, or sceneIds/timing are invalid:",
          errors,
        );
      }
  ...
  private mediaPlayerElements(): (Partial<PlayerElement> & HTMLElement)[] {
    return Array.from(this.querySelectorAll("shiftcut-player")).filter(
      (player): player is Partial<PlayerElement> & HTMLElement => player instanceof HTMLElement,
    );
  }

  /**
   * Inner `<shiftcut-player>` instances inside a slideshow need the
  ...
  private ensureInteractivePlayers(): void {
    for (const player of this.querySelectorAll("shiftcut-player")) {
  ...
  /**
   * Watch for `<shiftcut-player>` children added after the initial mount
  ...
  private warnIframeKeyForwardingUnavailable(): void {
    ...
    console.warn(
      "[shiftcut-slideshow] iframe keyboard forwarding is unavailable for this composition, likely because the player iframe is cross-origin. Arrow shortcuts work when focus is outside the iframe.",
    );
  }
  ...
  private onKey = (e: KeyboardEvent): void => {
    ...
    const multiInstance = document.querySelectorAll("shiftcut-slideshow").length > 1;
  ...
  private stepAutoplay(safeId: string, state: AutoplayPollState): boolean {
    ...
    void video.play().catch((err: unknown) => {
      ...
      if (name !== "AbortError" && name !== "NotAllowedError" && !state.warned) {
        state.warned = true;
        console.warn("[shiftcut-slideshow] autoplay play() failed:", err);
      }
    });
    return false;
  }
  ...
  private presenterNotesDeckKey(): string {
    ...
    const playerSrc = this.querySelector("shiftcut-player")?.getAttribute("src") ?? "";
...
if (!customElements.get("shiftcut-slideshow")) {
  customElements.define("shiftcut-slideshow", ShiftcutSlideshow);
}
