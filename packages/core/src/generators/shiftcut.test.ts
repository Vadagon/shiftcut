...
describe("generateShiftcutHtml", () => {
  ...
  it("includes GSAP CDN script tag when includeScripts is true", () => {
    const elements = [makeTextElement()];
    const html = generateShiftcutHtml(elements, 5, { includeScripts: true });
  ...
  it("sets resolution data attribute", () => {
    ...
    const landscapeHtml = generateShiftcutHtml(elements, 5, { resolution: "landscape" });
    expect(landscapeHtml).toContain('data-resolution="landscape"');

    const portraitHtml = generateShiftcutHtml(elements, 5, { resolution: "portrait" });
  ...
  it("generates video elements with proper tags", () => {
    ...
    const html = generateShiftcutHtml(elements, 10);
  ...
  it("includes custom compositionId", () => {
    const elements = [makeTextElement()];
    const html = generateShiftcutHtml(elements, 5, { compositionId: "test-comp-123" });
  ...
  it("includes style tags when includeStyles is true", () => {
    const elements = [makeTextElement()];
    const html = generateShiftcutHtml(elements, 5, { includeStyles: true });
  ...
  it("serializes keyframes as data attributes", () => {
    ...
    const html = generateShiftcutHtml(elements, 5, { keyframes });
  ...
  it("serializes zoom keyframes on zoom container", () => {
    ...
    const html = generateShiftcutHtml(elements, 10, { stageZoomKeyframes });
...
describe("generateShiftcutStyles", () => {
  it("generates core CSS with stage dimensions for landscape", () => {
    const elements = [makeTextElement()];
    const { coreCss } = generateShiftcutStyles(elements, "landscape");
  ...
  it("generates core CSS with stage dimensions for portrait", () => {
    const elements = [makeTextElement()];
    const { coreCss } = generateShiftcutStyles(elements, "portrait");
  ...
  it("includes custom CSS when provided", () => {
    const elements = [makeTextElement()];
    const { customCss } = generateShiftcutStyles(
      elements,
      "landscape",
      ".custom { color: blue; }",
    );
  ...
  it("generates Google Fonts link for Inter (always included)", () => {
    const elements = [makeTextElement()];
    const { googleFontsLink } = generateShiftcutStyles(elements, "landscape");
