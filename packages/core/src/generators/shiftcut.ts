...
import { serializeGsapAnimations, keyframesToGsapAnimations } from "@shiftcut/parsers";
...
export function generateShiftcutStyles(
...
export function generateShiftcutHtml(
  elements: TimelineElement[],
  totalDuration: number,
  options: SerializeOptions = {},
): string {
  ...
  let googleFontsLink = "";
  if (includeStyles) {
    const styles = generateShiftcutStyles(sortedElements, resolution, customStyles);
