 ...
 * It dispatches to:
 *
 *   - `console.debug` with the label, the error, and a `[shiftcut]` prefix
 *     when `window.__hfDebug === true` (or the legacy `__SHIFTCUT_DEBUG`
...
export function swallow(label: string, error?: unknown): void {
  ...
  if (w.__hfDebug || w.__SHIFTCUT_DEBUG) {
    // eslint-disable-next-line no-console -- intentional debug surface
    console.debug(`[shiftcut] ${label} swallowed:`, error);
  }
}
