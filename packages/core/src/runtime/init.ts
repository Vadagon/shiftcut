...
export function initSandboxRuntimeModular(): void {
  ...
  if (window.__HF_EXPORT_RENDER_SEEK_CONFIG) {
    console.info("[shiftcut] render runtime fps", {
  ...
  const publishRenderReadyAfterTimelineBinding = () => {
    ...
    if (!state.capturedTimeline) {
      ...
      if (registeredKeys.length > 0) {
        ...
        console.warn(
          `[shiftcut] Root timeline not bound — render will freeze at t=0. ` +
            (rootCompositionId
  ...
  const transportTick = () => {
    ...
    try {
      ...
      // Looping is handled at the player layer (<shiftcut-player>),
