...
function emitUnavailableDiagnostic(
  ...
  console.info(`[shiftcut] ${DIAGNOSTIC_UNAVAILABLE_CODE}: "${asset}" (${reason}): ${note}`);
}
...
export function swapToProxy(
  ...
  console.info(
    `[shiftcut] ${DIAGNOSTIC_FALLBACK_CODE}: "${originalSrc}" uses a codec (${codecName ?? "unknown"}) this browser can't decode; ` +
