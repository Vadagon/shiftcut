...
describe("swallow", () => {
  ...
  it("logs to console.debug when window.__hfDebug is true", () => {
    ...
    expect(console.debug).toHaveBeenCalledWith("[shiftcut] test.debug swallowed:", err);
  });

  it("also honors window.__SHIFTCUT_DEBUG (legacy flag)", () => {
    w.__SHIFTCUT_DEBUG = true;
    swallow("test.legacy", "string-error");
    expect(console.debug).toHaveBeenCalledWith(
      "[shiftcut] test.legacy swallowed:",
      "string-error",
    );
  });
