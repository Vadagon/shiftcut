...
import { isScalarVariableValue as isScalar } from "@shiftcut/parsers/composition";
...
export function applyVariableBindings(doc: Document): void {
  ...
  for (const el of Array.from(doc.querySelectorAll("[data-var-src]"))) {
    ...
    if (!VAR_SRC_TAGS.has(el.tagName.toLowerCase())) {
      console.warn(
        `[shiftcut] Ignoring data-var-src on <${el.tagName.toLowerCase()}>: variable-bound src is only allowed on ${Array.from(VAR_SRC_TAGS).join("/")}.`,
      );
      continue;
    }
    ...
    if (!isSafeMediaUrl(url)) {
      console.warn(`[shiftcut] Ignoring data-var-src="${id}": unsafe URL protocol.`);
      continue;
    }
