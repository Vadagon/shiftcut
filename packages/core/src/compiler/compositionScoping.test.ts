...
`,
  ...
  it("exposes a scoped __shiftcut.getVariables that reads __hfVariablesByComp[compId]", () => {
    ...
    const wrapped = wrapScopedCompositionScript(
      `window.__captured = __shiftcut.getVariables();`,
      "card-1",
    );
  ...
  it("routes the documented window.__shiftcut.getVariables() to the scoped variant too", () => {
    // Regression: the docs (variables-and-media.md) show `window.__shiftcut.
    // getVariables()`, but inside a sub-comp the scoped `window` proxy used to
    // fall through to the HOST page's base __shiftcut, returning the wrong
    // (or empty) variables — the bare `__shiftcut` param was the only form
    ...
    const wrapped = wrapScopedCompositionScript(
      `window.__captured = window.__shiftcut.getVariables();`,
      "card-1",
    );
  ...
  it("preserves non-getVariables members on window.__shiftcut (only getVariables is rescoped)", () => {
    const { document } = parseHTML(`<div data-composition-id="card-1"></div>`);
    let fitCalled = false;
    const fakeWindow: Record<string, unknown> = {
      document,
    ...
    const wrapped = wrapScopedCompositionScript(
      `window.__shiftcut.fitTextFontSize();`,
      "card-1",
    );
  ...
  it("scoped getVariables returns a fresh object — mutations don't leak into the shared table", () => {
    ...
    const wrapped = wrapScopedCompositionScript(
      `var v = __shiftcut.getVariables(); v.title = "MUTATED"; v.added = "extra";`,
      "card-1",
    );
...
`,
  ...
  it("escapes </script> in scoped composition script source to prevent injection", () => {
    ...
    expect(wrapped).toContain("(function(document, gsap, window, __shiftcut)");
