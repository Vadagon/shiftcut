...
Reference for generating and editing ShiftCut HTML compositions. This is your source of truth for how to author compositions.

**New to ShiftCut?** Start with the [quickstart template](./quickstart-template.html) — a copy-paste composition with inline comments explaining every required piece. See [common mistakes](./common-mistakes.md) for pitfalls that break compositions.
...
ShiftCut uses HTML as the source of truth for describing a video:
...
A composition is the fundamental grouping unit in ShiftCut. Every clip — video, image, audio — must live inside a composition. The `index.html` file is itself a composition (the top-level one), and it can contain nested compositions within it. Any composition can be imported into another composition as a sub-composition — there is no special "root" type.
