 ...
 * and verifies that window.__shiftcut.fitTextFontSize produces
 * correct results with real canvas measureText.
 *
 * Requires: puppeteer (dep of @shiftcut/engine)
 * Run: cd packages/engine && npx tsx scripts/test-fitTextFontSize-browser.ts
 */

import { buildShiftcutRuntimeScript } from "../../core/src/inline-scripts/shiftcutRuntime.engine";
...
async function main() {
  ...
  const runtimeSource = buildShiftcutRuntimeScript({ minify: false });
  assert(
    runtimeSource !== null,
    "buildShiftcutRuntimeScript returned null — entry.ts not found",
  );
...
<script>
  ...
  var r3 = window.__shiftcut.fitTextFontSize(
  ...
  window.__testResults.exists = typeof window.__shiftcut.fitTextFontSize === "function";
</script>
</body></html>`;
  ...
  try {
    ...
    assert(results.exists === true, "fitTextFontSize should exist on window.__shiftcut");
    assert(
