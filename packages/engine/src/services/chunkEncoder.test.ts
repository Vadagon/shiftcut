...
describe("muxVideoWithAudio audio codec handling", () => {
  it("copies ShiftCut AAC sidecars into MP4 instead of re-encoding", async () => {
    const { spawn, calls } = createSpawnSpy();
    vi.resetModules();
    vi.doMock("child_process", () => ({ spawn }));
  ...
  it("copies ShiftCut AAC sidecars into MOV containers without MP4 faststart flags", async () => {
    const { spawn, calls } = createSpawnSpy();
    vi.resetModules();
    vi.doMock("child_process", () => ({ spawn }));
