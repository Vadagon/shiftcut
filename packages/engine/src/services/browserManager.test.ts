...
describe("resolveHeadlessShellPath", () => {
  ...
  const originalShiftcutBrowserPath = process.env.SHIFTCUT_BROWSER_PATH;
  ...
  afterEach(() => {
    ...
    if (originalShiftcutBrowserPath === undefined) delete process.env.SHIFTCUT_BROWSER_PATH;
    else process.env.SHIFTCUT_BROWSER_PATH = originalShiftcutBrowserPath;
  });
  ...
  it("uses SHIFTCUT_BROWSER_PATH when the CLI resolved a browser explicitly", () => {
    const dir = mkdtempSync(join(tmpdir(), "shiftcut-engine-browser-env-"));
    try {
      const binary = join(dir, "chrome-headless-shell");
      writeFileSync(binary, "");
      delete process.env.PRODUCER_HEADLESS_SHELL_PATH;
      process.env.SHIFTCUT_BROWSER_PATH = binary;
  ...
  it("reuses chrome-headless-shell from the ShiftCut-managed cache", () => {
    const home = mkdtempSync(join(tmpdir(), "shiftcut-engine-browser-cache-"));
    try {
      const binary = join(
        home,
        ".cache",
      ...
      delete env.SHIFTCUT_BROWSER_PATH;
