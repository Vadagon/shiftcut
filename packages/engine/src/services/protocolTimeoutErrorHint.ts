 ...
 * points at the ShiftCut-specific knobs. Puppeteer's stock error text
 ...
 * ShiftCut, or what the currently-applied effective value is — so field
...
export function augmentProtocolTimeoutError(err: unknown, effectiveTimeoutMs: number): Error {
  ...
  const augmented = new Error(
    `${err.message}\n\n` +
      `ShiftCut effective protocolTimeout: ${effectiveTimeoutMs} ms.\n\n` +
