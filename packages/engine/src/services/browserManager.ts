 ...
 * then the CLI browser override, ShiftCut' managed cache, and Puppeteer's cache.
 */
export function resolveHeadlessShellPath(
    ...
    if (!existsSync(envPath)) {
      throw new Error(
        `[BrowserManager] Chrome binary not found at PRODUCER_HEADLESS_SHELL_PATH="${envPath}". ` +
          "Run `shiftcut browser ensure` to re-download.",
      );
    }
    return envPath;
  }
  if (process.env.SHIFTCUT_BROWSER_PATH) {
    const envPath = process.env.SHIFTCUT_BROWSER_PATH;
    if (!existsSync(envPath)) {
      throw new Error(
        `[BrowserManager] Chrome binary not found at SHIFTCUT_BROWSER_PATH="${envPath}". ` +
          "Run `shiftcut browser ensure` to re-download.",
      );
    }
    return envPath;
  ...
  return (
    findCachedHeadlessShell(
      join(home, ".cache", "shiftcut", "chrome", "chrome-headless-shell"),
...
function logResolvedBrowserGpuMode(resolved: "hardware" | "software", reason: string): void {
  console.error(`[shiftcut] browserGpuMode auto → ${resolved} (${reason})`);
}
