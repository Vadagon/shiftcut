...
import { decodeUrlPathVariants, MEDIA_DURATION_CLAMP_EPSILON_SECONDS } from "@shiftcut/core";
...
export async function extractAllVideoFrames(
      ...
      if (!existsSync(videoPath)) {
        ...
        if (!warnedSrcs.has(video.src)) {
          ...
          process.stderr.write(
            `[shiftcut:render] WARNING: video src="${video.src}" ` +
  ...
  if (configuredCacheRootDir) {
    try {
      ...
      process.stderr.write(
        `[shiftcut:render] WARNING: extraction cache dir ${configuredCacheRootDir} is not writable; caching disabled for this render\n`,
      );
    }
  }
