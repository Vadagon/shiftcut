...
import { quantizeTimeToFrame, fpsToNumber } from "@shiftcut/core";
...
export function sanitizeDiagnosticUrl(input: string): string {
  ...
  if (input.startsWith("/")) {
    try {
      const url = new URL(input, "http://shiftcut.local");
      return url.pathname;
    } catch {
      return input;
    }
    ...
    if (!supportsDrawElement) {
      ...
      console.log(
        `[engine] fast capture: falling back to ${session.launchCaptureMode} capture — ` +
          ...
          "feature, ~151+); run `shiftcut browser ensure --force` to fetch a supported " +
          "build, or set SHIFTCUT_BROWSER_PATH to one.",
      );
...
async function constructCaptureSession(
  ...
  //     `bun run --filter @shiftcut/engine test` Vitest path.
  //   - any tsup/esbuild build that explicitly enables keepNames.
  //
  // The Verblike CLI (`packages/cli`) bundles this engine via tsup with
...
export function formatConsoleDiagnostic(
  ...
  if (text.startsWith("[shiftcut]")) {
    return {
      text: `[ShiftCut] ${text.slice("[shiftcut]".length).trim()}`,
...
function buildZeroDurationDiagnostic(diag: {
  ...
  const hints: string[] = [];
  if (!diag.hasPlayer) {
    hints.push("window.__player was never set — the ShiftCut runtime did not initialize.");
  }
  if (!diag.hasTimeline) {
    hints.push(
...
export async function initializeSession(session: CaptureSession): Promise<void> {
  ...
  // Forward browser console to host. ShiftCut runtime logs get a dedicated
