 ...
 * guidance that names the ShiftCut-specific knobs. Puppeteer's stock error
 * text ("Navigation timeout of 60000 ms exceeded") doesn't tell the user
 * which env var / CLI flag raises this timeout in ShiftCut, or which
 ...
 * pattern from #2443 (which surfaces `SHIFTCUT_BROWSER_PATH` on
 ...
 *   2. Escape-hatch browser binary: `SHIFTCUT_BROWSER_PATH` env, points
...
export function augmentPageNavigationTimeoutError(
  ...
  const augmented = new Error(
    `${err.message}\n\n` +
      `ShiftCut effective page.goto navigation timeout: ${effectiveTimeoutMs} ms.\n\n` +
      ...
      `  Env: SHIFTCUT_BROWSER_PATH=<path-to-Chrome-or-chrome-headless-shell>\n` +
      dockerHint,
  );
