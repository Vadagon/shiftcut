 ...
 * `@shiftcut/parsers/ff-binaries` subpath, never from a browser bundle.
 */

export const FFMPEG_PATH_ENV = "SHIFTCUT_FFMPEG_PATH";
export const FFPROBE_PATH_ENV = "SHIFTCUT_FFPROBE_PATH";
...
function findInProjectLocalBin(name: FfBinaryName): string | undefined {
  ...
  const candidate = resolve(".shiftcut", "bin", `${name}${extension}`);
 ...
 * `.shiftcut/bin`, then well-known Unix install dirs. System lookups are
