...
// Moved from @shiftcut/core/core.types in the parsers extraction refactor.
 ...
 * stage (`@shiftcut/producer`) and `suggestMatchingPreset` here.
 *
 ...
 * compile stage (`@shiftcut/producer`) for the canonical remap.
 */
 ...
 * every entrypoint that emits a `DistributedRenderConfig` (`shiftcut
 * cloudrun render`, `shiftcut lambda render` / `render-batch`, the local
