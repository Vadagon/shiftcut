...
describe("findFfBinary", () => {
  ...
  it("returns the resolved env override without touching the system", async () => {
    process.env.SHIFTCUT_FFMPEG_PATH = "/tools/ffmpeg";
    vi.resetModules();
  ...
  it("treats a missing env override as not-found when configuredMustExist is set", async () => {
    process.env.SHIFTCUT_FFMPEG_PATH = join(tmpdir(), "definitely-missing-ffmpeg");
    vi.resetModules();
    const { findFfBinary } = await importFresh();
  ...
  it("falls back to scanning PATH when which/where fails", async () => {
    delete process.env.SHIFTCUT_FFMPEG_PATH;
    const binDir = mkdtempSync(join(tmpdir(), "shiftcut-ffbinaries-"));
  ...
  it("falls back to the project-local .shiftcut bin", async () => {
    delete process.env.SHIFTCUT_FFMPEG_PATH;
    process.env.PATH = "";
