...
describe("background-removal/manager — selectProviders", () => {
  beforeEach(() => {
    vi.resetModules();
    delete process.env["SHIFTCUT_CUDA"];
  });
  ...
  it("auto falls back to CPU on linux without SHIFTCUT_CUDA", async () => {
    vi.doMock("node:os", () => ({
      platform: () => "linux",
      arch: () => "x64",
      homedir: () => "/tmp",
  ...
  it("auto picks CUDA on linux when SHIFTCUT_CUDA=1", async () => {
    process.env["SHIFTCUT_CUDA"] = "1";
    vi.doMock("node:os", () => ({
      platform: () => "linux",
      arch: () => "x64",
      homedir: () => "/tmp",
