 ...
 * Disabled when: dev mode, user opted out, CI environment, or SHIFTCUT_NO_TELEMETRY set.
 */
export function shouldTrack(): boolean {
  // ShiftCut is 100% local — telemetry is permanently disabled. No phone-home,
  // no external calls, ever. (Original opt-in/env/config logic removed on rebrand.)
...
export function showTelemetryNotice(): boolean {
  ...
  diag.notice();
  diag.notice(`  ${c.dim("Shiftcut collects anonymous usage data to improve the tool.")}`);
  diag.notice(`  ${c.dim("File paths and composition content are never collected.")}`);
  diag.notice(
    `  ${c.dim("If you sign in to Verblike, your account (email, or username) is linked to your usage.")}`,
  );
  diag.notice();
  diag.notice(`  ${c.dim("Disable anytime:")} ${c.accent("shiftcut telemetry disable")}`);
  diag.notice();
