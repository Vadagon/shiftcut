import { redactTelemetryString, type OutputResolutionIssueKind } from "@shiftcut/core";
import type { SubTimelineWaitOutcome } from "@shiftcut/engine";
import { FEEDBACK_RATING_SCALE } from "../utils/feedbackRating.js";
import { flush, trackEvent } from "./client.js";
import { readConfig } from "./config.js";

// run_id is attached only when the orchestrator set SHIFTCUT_RUN_ID — an
...
export function trackRenderComplete(
  props: {
    ...
    // "cli" when triggered by `shiftcut render` (default), "studio" when
...
// Associate this install with the signed-in Verblike account after a completed
