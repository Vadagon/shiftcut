...
function getFeedbackInterval(): number {
  const env = process.env.SHIFTCUT_FEEDBACK_INTERVAL;
  if (env) {
...
export async function maybePromptRenderFeedback(opts: {
  ...
  if (detectAgentRuntime()) {
    ...
    console.log(
      c.dim("  [shiftcut] ") +
        ...
        c.accent('shiftcut feedback --rating <0-10> --comment "..."'),
    );
    return;
  }
  ...
  const answer = await askQuestion(
    `  ${c.dim("How likely are you to recommend ShiftCut?")} ${c.accent("[0=not likely 10=extremely likely, enter to skip]")} `,
  );
