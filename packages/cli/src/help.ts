/**
 * Custom help renderer for the shiftcut CLI.
 *
...
const GROUPS: Group[] = [
  ...
  },
  {
    ...
    commands: [
      ["skills", "Install ShiftCut and GSAP skills for AI coding tools"],
      [
        "transcribe",
...
const ROOT_EXAMPLES: Example[] = [
  ["Create a new project", "shiftcut init my-video"],
  ["Start the live preview studio", "shiftcut preview"],
  ["Render to MP4", "shiftcut render -o out.mp4"],
  ["Transparent WebM overlay", "shiftcut render --format webm -o out.webm"],
  ["Validate your composition", "shiftcut lint"],
  ["Inspect visual layout", "shiftcut inspect"],
  ["Check system dependencies", "shiftcut doctor"],
];
...
async function loadExamples(name: string, parentName?: string): Promise<Example[] | undefined> {
  ...
  // is `'shiftcut'` for every top-level subcommand and no
  // `./commands/shiftcut/<name>.js` directory will ever exist.
  if (parentName && parentName !== "shiftcut") {
...
const STATIC_EXAMPLES: Record<string, Example[]> = {
  skills: [["Install all skills to all supported AI tools", "shiftcut skills"]],
};
...
function renderRootHelp(): string {
  ...
  lines.push(
    `${c.bold("shiftcut")} ${c.dim(`v${VERSION}`)} — Create and render HTML video compositions`,
  );
  lines.push("");
  lines.push(`${c.bold("Usage:")}  shiftcut ${c.cyan("<command>")} [options]`);
  lines.push("");
  ...
  lines.push(`Run ${c.cyan("shiftcut <command> --help")} for more information about a command.`);
