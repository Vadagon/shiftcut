...
import { resolveExtractCacheDir } from "@shiftcut/engine";
...
export const examples: Example[] = [
  ...
  ["Output as JSON for CI / agents", "shiftcut doctor --json"],
];
 ...
 * + free space at that path lets `shiftcut doctor` catch the mismatch
  ...
  if (freeMb < 2048) {
    return {
      ...
      hint:
        ...
        "Relocate via SHIFTCUT_EXTRACT_CACHE_DIR=<path> or `shiftcut render --frames-cache-dir <path>`.",
    };
  }
...
function sourceLabel(source: "env" | "default"): string {
  return source === "env" ? "SHIFTCUT_EXTRACT_CACHE_DIR" : "default";
}
...
export function checkArchiveExtractor(
  ...
  return {
    ok: false,
    detail: "Not found",
    hint: "Install unzip so ShiftCut can extract its managed Chrome download.",
  };
}
...
export default defineCommand({
  ...
  async run({ args }) {
    ...
    if (args.json) {
      ...
      // `shiftcut doctor --json || fail` the next time a new version is
    ...
    console.log();
    console.log(c.bold("shiftcut doctor"));
    console.log();
