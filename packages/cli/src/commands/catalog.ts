...
export const examples: Example[] = [
  ["List all blocks and components", "shiftcut catalog"],
  ["List blocks only", "shiftcut catalog --type block"],
  ["Filter by tag", "shiftcut catalog --type block --tag social"],
  ["Machine-readable JSON", "shiftcut catalog --json"],
  ["Interactive picker (install on select)", "shiftcut catalog --human-friendly"],
];

import * as clack from "@clack/prompts";
import { type ItemType } from "@shiftcut/core";
...
export default defineCommand({
  ...
  async run({ args }) {
    ...
    if (args.type === "block") typeFilter = "shiftcut:block";
    else if (args.type === "component") typeFilter = "shiftcut:component";
    else if (args.type) {
    ...
    const filtered = entries.filter((e) => e.type !== "shiftcut:example");
    ...
    if (json) {
      const output = matching.map((item) => ({
        name: item.name,
        type: item.type.replace("shiftcut:", ""),
        title: item.title,
    ...
    for (const item of matching) {
      const type = item.type.replace("shiftcut:", "");
    ...
    console.log("");
    console.log(c.dim(`${matching.length} items. Run "shiftcut add <name>" to install.`));
  },
});
