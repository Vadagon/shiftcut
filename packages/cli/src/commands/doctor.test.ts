...
describe("checkFramesCache", () => {
  it("reports disabled state with the raw env value when the user opts out", () => {
    const result = checkFramesCache(
      { SHIFTCUT_EXTRACT_CACHE_DIR: "off" },
      () => 100_000,
      () => true,
    );
  ...
  it("reports a positive env override with source: SHIFTCUT_EXTRACT_CACHE_DIR", () => {
    const result = checkFramesCache(
      { SHIFTCUT_EXTRACT_CACHE_DIR: "/mnt/scratch/hf" },
      () => 10_000,
      () => true,
    );
    expect(result.ok).toBe(true);
    expect(result.detail).toContain("/mnt/scratch/hf");
    expect(result.detail).toContain("SHIFTCUT_EXTRACT_CACHE_DIR");
  });

  it("fails with a relocation hint when free space at the cache location is <2 GB", () => {
    const result = checkFramesCache(
      { SHIFTCUT_EXTRACT_CACHE_DIR: "/tmp/hf" },
      () => 512, // 0.5 GB
      () => true,
    );
    expect(result.ok).toBe(false);
    expect(result.hint).toContain("--frames-cache-dir");
    expect(result.hint).toContain("SHIFTCUT_EXTRACT_CACHE_DIR");
  });

  it("falls back to the first existing ancestor when the cache dir does not exist yet", () => {
    const seen: string[] = [];
    const result = checkFramesCache(
      { SHIFTCUT_EXTRACT_CACHE_DIR: "/mnt/scratch/newly/created/subdir" },
      (path) => {
        seen.push(path);
        return 20_000;
      },
  ...
  it("handles a null free-space reading without failing the check", () => {
    const result = checkFramesCache(
      { SHIFTCUT_EXTRACT_CACHE_DIR: "/mnt/scratch" },
      () => null,
      () => true,
    );
    expect(result.ok).toBe(true);
