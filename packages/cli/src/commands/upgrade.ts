...
export const examples: Example[] = [
  ["Check for updates interactively", "shiftcut upgrade"],
  ["Check for updates without prompting", "shiftcut upgrade --check"],
  ["Upgrade non-interactively", "shiftcut upgrade --yes"],
  ["Bump a project's pinned CLI scripts", "shiftcut upgrade --project"],
];
...
export default defineCommand({
  ...
  args: {
    ...
    project: {
      type: "string",
      description:
        "Bump this project's package.json shiftcut@<version> script pins to latest (default: current dir)",
    },
  },
  ...
  async run({ args }) {
    ...
    clack.intro(c.bold("shiftcut upgrade"));
...
function applyUpgrade(result: UpdateCheckResult, autoYes: boolean): void {
  ...
  const npxFallback = `npx shiftcut@${result.latest}`;
...
function printProjectPinResult(
  ...
  if (!res.changed) {
    console.log(`   ${c.success("◇")}  Project scripts already on shiftcut@${res.to}`);
    return;
  }
  ...
  if (checkOnly)
    console.log(`   ${c.dim("Run `npx shiftcut@latest upgrade --project` to apply.")}`);
}
