...
export const examples: Example[] = [
  ["Present the current deck", "shiftcut present"],
  ["Present a specific project directory", "shiftcut present ./my-deck"],
  ["Use a custom port", "shiftcut present --port 8080"],
  ["Start without opening the browser", "shiftcut present --no-open"],
  ["Open with a specific browser", "shiftcut present --browser-path /usr/bin/chromium"],
];
...
export default defineCommand({
  ...
  async run({ args }) {
    ...
    if (!islandMatch?.[1]) {
      clack.log.error(
        `No slideshow island found in ${project.indexPath}. ` +
          `Add a <script type="application/shiftcut-slideshow+json"> block — see /shiftcut (slideshow).`,
      );
      process.exitCode = 1;
      return;
    }
    ...
    clack.intro(c.bold("shiftcut present"));
  ...
  <head>
    ...
    <style>
      * { margin: 0; padding: 0; box-sizing: border-box; }
      ...
      shiftcut-slideshow { display: block; position: relative; width: 100vw; height: 100vh; }
      shiftcut-player { position: absolute; inset: 0; }
    </style>
    <script src="/player.js"></script>
    <script src="/slideshow.js"></script>
  </head>
  <body>
    <shiftcut-slideshow tabindex="0" sound>
      <shiftcut-player interactive src="/composition/index.html"></shiftcut-player>
...
${islandJson}
    ...
    <script>
      ...
      (function () {
        ...
        // Mute state is owned by <shiftcut-slideshow sound>; mirror it.
        var muted = false;
        ...
        var ss = document.querySelector("shiftcut-slideshow");
        if (ss) {
