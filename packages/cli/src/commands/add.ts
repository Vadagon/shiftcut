...
export const examples: Example[] = [
  ["Add a block to the current project", "shiftcut add claude-code-window"],
  ["Add a component effect", "shiftcut add shader-wipe"],
  ["Add all HTML-in-Canvas blocks", "shiftcut add html-in-canvas"],
  ["Add all caption blocks", "shiftcut add captions"],
  ["Target a specific project directory", "shiftcut add shader-wipe --dir ./my-video"],
  ["Skip the clipboard copy (CI/headless)", "shiftcut add shader-wipe --no-clipboard"],
];

import { existsSync } from "node:fs";
import { resolve, relative } from "node:path";
import { ITEM_TYPE_DIRS, type RegistryItem } from "@shiftcut/core";
...
// user's `shiftcut.json#paths` so a project can reshape its layout
...
export function remapTarget(
  ...
  if (item.type === "shiftcut:component") {
...
export async function runAdd(opts: RunAddArgs): Promise<RunAddResult> {
  ...
  if (item.type === "shiftcut:example") {
    throw new AddError(
      `"${item.name}" is an example — use \`shiftcut init <dir> --example ${item.name}\` instead.`,
      "example-type",
    );
  }
  ...
  const primaryFile =
    itemForInstall.files.find((f) => f.type === "shiftcut:snippet") ??
    itemForInstall.files.find((f) => f.type === "shiftcut:composition") ??
