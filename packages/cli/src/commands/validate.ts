...
export async function auditClipDurations(
  ...
  analyzeClipMediaFit: typeof import("@shiftcut/engine").analyzeClipMediaFit,
  extraWaitMs: number,
...
async function validateInBrowser(
  ...
  try {
    ...
    const { buildChromeArgs, analyzeClipMediaFit } = await import("@shiftcut/engine");
...
Examples:
  shiftcut validate
  shiftcut validate ./my-project
  shiftcut validate --json
  shiftcut validate --timeout 5000`,
  },
  args: {
