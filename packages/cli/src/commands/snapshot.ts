 ...
 * default applied by `@shiftcut/engine`'s `runFfmpeg` so a pathological
 * clip (corrupt media, stalled network mount, codec edge case) cannot wedge
 * `shiftcut snapshot` indefinitely. */
...
async function captureSnapshots(
  ...
  try {
    ...
    try {
      ...
      try {
        const engine = (await import("@shiftcut/engine")) as {
        ...
        console.warn(
          `   ${c.warn("⚠")} @shiftcut/engine unavailable — <video> elements will appear black in snapshots. Verify media via a draft render's extracted frames instead.`,
        );
      }
...
export default defineCommand({
  ...
  args: {
    ...
    proxy: {
      type: "boolean",
      description:
        "Auto-transcode browser-hostile video codecs for snapshots (default: on; overrides shiftcut.json media.autoProxy)",
      default: undefined,
    },
  },
  async run({ args }) {
      ...
      if (describeArg !== null) {
        try {
          ...
          if (!geminiKey) {
            ...
            const model = process.env.SHIFTCUT_GEMINI_MODEL || "gemini-3.1-flash-lite-preview";
