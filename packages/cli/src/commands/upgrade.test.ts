...
describe("runDetectedInstall", () => {
  ...
  it("spawns the manager binary with shell:false and inherited stdio", async () => {
    ...
    runDetectedInstall(
      { bin: "bun", args: ["add", "-g", "shiftcut@1.2.3"] },
      "bun add -g shiftcut@1.2.3",
      "1.2.3",
    );

    expect(execSpy).toHaveBeenCalledTimes(1);
    expect(execSpy).toHaveBeenCalledWith("bun", ["add", "-g", "shiftcut@1.2.3"], {
      stdio: "inherit",
      shell: false,
    });
  });
  ...
  it("sets a non-zero exit code when the install fails, without throwing", async () => {
    ...
    try {
      expect(() =>
        runDetectedInstall(
          { bin: "npm", args: ["install", "-g", "shiftcut@1.2.3"] },
          "npm install -g shiftcut@1.2.3",
          "1.2.3",
        ),
      ).not.toThrow();
