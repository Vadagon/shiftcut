...
export const examples: Example[] = [
  ["Create a project with the interactive wizard", "shiftcut init my-video"],
  ["Pick a starter example", "shiftcut init my-video --example warm-grain"],
  ["Scaffold a 4K project", "shiftcut init my-video --resolution 4k"],
  ["Scaffold a portrait video", "shiftcut init my-video --resolution portrait"],
  ["Start from an existing video file", "shiftcut init my-video --video clip.mp4"],
  ["Start from an audio file", "shiftcut init my-video --audio track.mp3"],
  ["Scaffold with Tailwind CSS", "shiftcut init my-video --example blank --tailwind"],
  [
    "Non-interactive mode (for CI or AI agents)",
    "shiftcut init my-video --example blank --non-interactive",
  ],
  [
    "Opt out of the GitHub skills check (CI/tests only)",
    "SHIFTCUT_SKIP_SKILLS=1 shiftcut init my-video --example blank --non-interactive",
  ],
];
import {
  existsSync,
...
function toPackageName(projectName: string): string {
  ...
  return normalized || "shiftcut-project";
}

function getShiftcutPackageSpecifier(): string {
  return VERSION === "0.0.0-dev" ? "shiftcut" : `shiftcut@${VERSION}`;
}

function shiftcutScript(command: string): string {
  return `npx --yes ${getShiftcutPackageSpecifier()} ${command}`;
}

function buildPackageScripts(): Record<string, string> {
  return {
    dev: shiftcutScript("preview"),
    check: shiftcutScript("check"),
    render: shiftcutScript("render"),
    publish: shiftcutScript("publish"),
  };
}
...
async function scaffoldProject(
  ...
  // Write shiftcut.json so `shiftcut add` knows which registry to use
  // and where to drop block/component files. Overwritten only if absent.
  if (!existsSync(resolve(destDir, "shiftcut.json"))) {
 ...
 * set. Guarantees the core set (the `/shiftcut` entry router + shared
 ...
 * workflow is triggered (`shiftcut skills update <name>`, which the router
...
async function keepSkillsCurrent(destDir: string): Promise<void> {
  ...
  try {
    ...
    if (result.presenceOnly) {
      ...
      console.log(
        c.dim("Skills freshness unverified — run `npx shiftcut skills update` when online."),
      );
...
export default defineCommand({
  ...
  args: {
    ...
    "skip-skills": {
      type: "boolean",
      description:
        "[temporarily ignored] init always checks AI skills against GitHub while the skills.sh registry catches up; set SHIFTCUT_SKIP_SKILLS=1 to opt out (CI/tests)",
    },
    tailwind: {
      type: "boolean",
  ...
  async run({ args }) {
    if (args.template !== undefined) {
      ...
      console.error(
        c.error(
          `The --template flag was renamed to --example. Example:\n  npx shiftcut init ${args.name ?? "my-video"} --example "${args.template}"`,
        ),
      );
      process.exit(1);
    }
    ...
    // shiftcut@latest`). CI and unit tests still opt out via the
    // SHIFTCUT_SKIP_SKILLS=1 env var, which the agent/user CLI path never sets.
    // Revert to `args["skip-skills"] === true` once skills.sh catches up.
    const skipSkills = process.env.SHIFTCUT_SKIP_SKILLS === "1";
    ...
    if (!interactive) {
      ...
      if (skipSkills) {
        ...
        console.log(`     ${c.accent("npx shiftcut skills update")}`);
      } else {
        console.log(
      ...
      console.log(
        `     ${c.dim('"Using /shiftcut, create a 15-second intro about [your topic]"')}`,
      );
      console.log(`     ${c.dim("More patterns: shiftcut.verblike.com/guides/prompting")}`);
      console.log();
      ...
      console.log();
      console.log(`  ${c.dim("Full docs: shiftcut.verblike.com")}`);
      return;
    }
    ...
    // -----------------------------------------------------------------------
    printBanner();
    clack.intro("Create a new ShiftCut project");
