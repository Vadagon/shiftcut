...
export const examples: Example[] = [
  ["Check current telemetry status", "shiftcut telemetry status"],
  ["Disable telemetry", "shiftcut telemetry disable"],
  ["Enable telemetry", "shiftcut telemetry enable"],
];
...
function runStatus(): void {
  ...
  console.log();
  console.log(`  ${c.dim("Disable:")}    ${c.accent("shiftcut telemetry disable")}`);
  console.log(`  ${c.dim("Env var:")}    ${c.accent("SHIFTCUT_NO_TELEMETRY=1")}`);
  console.log();
}
...
export default defineCommand({
  ...
  async run({ args }) {
    ...
    if (!subcommand || subcommand === "") {
      console.log(`
${c.bold("shiftcut telemetry")} ${c.dim("<subcommand>")}
...
${c.dim("You can also set")} ${c.accent("SHIFTCUT_NO_TELEMETRY=1")} ${c.dim("to disable.")}
`);
      return;
    }
    ...
    switch (subcommand) {
      ...
      default:
        console.error(
          `${c.error("Unknown subcommand:")} ${subcommand}\n\nRun ${c.accent("shiftcut telemetry --help")} for usage.`,
        );
        process.exit(1);
    }
  },
