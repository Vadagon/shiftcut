...
export const examples: Example[] = [
  ["Render to MP4", "shiftcut render --output output.mp4"],
  ["Render a specific composition", "shiftcut render -c compositions/intro.html -o intro.mp4"],
  [
    "Upsample any composition to 4K (supersamples via Chrome DPR)",
    "shiftcut render --resolution 4k --output 4k.mp4",
  ],
  ["Render transparent overlay (ProRes)", "shiftcut render --format mov --output overlay.mov"],
  ["Render transparent WebM overlay", "shiftcut render --format webm --output overlay.webm"],
  [
    "Render animated GIF for PRs/docs",
    "shiftcut render --format gif --fps 15 --gif-loop 0 --output demo.gif",
  ],
  [
    "Render PNG sequence (RGBA frames for AE/Nuke/Fusion)",
    "shiftcut render --format png-sequence --output frames/",
  ],
  ["High quality at 60fps", "shiftcut render --fps 60 --quality high --output hd.mp4"],
  ["Deterministic render via Docker", "shiftcut render --docker --output deterministic.mp4"],
  ["Parallel rendering with 6 workers", "shiftcut render --workers 6 --output fast.mp4"],
  ["Opt out of browser GPU render", "shiftcut render --no-browser-gpu --output cpu.mp4"],
  [
    "Relocate frame cache off C: (Windows) or another small partition",
    "shiftcut render --frames-cache-dir D:/hf-cache --output out.mp4",
  ],
  ["HDR output (auto-detected)", "shiftcut render --output hdr-output.mp4"],
  [
    "Override composition variables (parametrized render)",
    'shiftcut render --variables \'{"title":"Q4 Report","theme":"dark"}\' --output q4.mp4',
  ],
  [
    "Variables from a JSON file",
    "shiftcut render --variables-file ./vars.json --output out.mp4",
  ],
  [
    "Batch render one output per variables row",
    'shiftcut render --batch rows.json --output "renders/{name}.mp4"',
  ],
];
...
import { readConfigFresh, writeConfig, type ShiftcutConfig } from "../telemetry/config.js";
...
import { formatRenderOutputTimestamp } from "@shiftcut/core";
...
import type { ProducerLogger, RenderJob } from "@shiftcut/producer";
import {
...
export default defineCommand({
  ...
  args: {
    ...
    variables: {
      type: "string",
      description:
        'JSON object of variable values, merged over the composition\'s data-composition-variables defaults. Example: --variables \'{"title":"Hello"}\'. Read inside the composition via window.__shiftcut.getVariables().',
    },
    "variables-file": {
      type: "string",
      description:
    ...
    "frames-cache-dir": {
      ...
      description:
        ...
        "Env: SHIFTCUT_EXTRACT_CACHE_DIR.",
    },
  },
  // `run` is the citty handler for `shiftcut render` — sequential flag
  ...
  async run({ args }) {
    ...
    if (typeof args.skill === "string" && args.skill.trim() !== "" && !authoringSkill) {
      ...
      process.stderr.write(
        `shiftcut: ignoring --skill="${args.skill}" — not a valid slug ` +
    ...
    // Sugar for SHIFTCUT_EXTRACT_CACHE_DIR. Set BEFORE resolveConfig() so
    ...
    if (typeof args["frames-cache-dir"] === "string" && args["frames-cache-dir"].trim() !== "") {
      ...
      process.env.SHIFTCUT_EXTRACT_CACHE_DIR = isDisableAlias ? raw : resolve(raw);
    }
    ...
    if (!quiet) {
      try {
        const { slideshowIslandRegex } = await import("@shiftcut/core/slideshow");
    ...
    if (!useDocker) {
      ...
      try {
        ...
        errorBox(
          ...
          "Run: npx shiftcut browser ensure",
        );
        process.exit(1);
      }
    }
...
const DOCKER_IMAGE_PREFIX = "shiftcut-renderer";
...
function dockerImageTagForPlatform(version: string, platform: string): string {
  ...
  // shiftcut version coexist in the local cache (a developer who flips
...
function ensureDockerImage(version: string, platform: string, quiet: boolean): string {
  ...
  const tmpDir = join(tmpdir(), `shiftcut-docker-${Date.now()}`);
...
function resolveDockerHostPlatform(options: RenderOptions): string {
  ...
  if (options.gpu && platform === "linux/arm64") {
    errorBox(
      ...
      "Drop --gpu, or run a native (non-Docker) render on this host, or set SHIFTCUT_DOCKER_PLATFORM=linux/amd64 if you need GPU encoding (slow under qemu but works).",
    );
    process.exit(1);
  }
  ...
  if (!options.quiet && platform === "linux/arm64") {
    ...
    // SHIFTCUT_DOCKER_PLATFORM=linux/amd64 to force parity (qemu-emulated,
    // slower).
    console.log(
      c.dim(
        "  Host is arm64 — using linux/arm64 image with Playwright's " +
          "chrome-headless-shell (output won't be byte-identical to amd64 " +
          "renders; set SHIFTCUT_DOCKER_PLATFORM=linux/amd64 to force parity).",
      ),
    );
  }
...
async function renderDocker(
  ...
  if (!options.quiet && isDevMode()) {
    console.log(c.dim("  Dev mode: using shiftcut@latest in Docker image"));
  }
...
export async function renderLocal(
  ...
  if (preflight.ffmpegPath) process.env.SHIFTCUT_FFMPEG_PATH = preflight.ffmpegPath;
  if (preflight.ffprobePath) process.env.SHIFTCUT_FFPROBE_PATH = preflight.ffprobePath;
 ...
 * to `~/.shiftcut/config.json` succeeds. `writeConfig` swallows all fs
 ...
 * `--batch` run (all rows share one process) a `shiftcut telemetry off`
 ...
 * finding).
 */
function isDeParallelRouterTrialBlocked(config: ShiftcutConfig): boolean {
  const overRenderCap =
 ...
 * to manually set the env var — see `ShiftcutConfig.deParallelRouterTrialFired`.
...
function maybeEnableDeParallelRouterTrial(quiet: boolean, enabled: boolean): boolean {
  ...
  // mid-batch `shiftcut telemetry off` (or another process persisting
 ...
 * reports an fs failure (unwritable `~/.shiftcut` — retrying a failed
    ...
    console.warn(
      c.warn(
        "  Could not persist the parallel drawElement trial's off-switch to " +
          "~/.shiftcut/config.json (unwritable?). The experiment stays off for this " +
