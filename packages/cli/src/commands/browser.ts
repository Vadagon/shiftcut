...
export const examples: Example[] = [
  ["Find or download Chrome for rendering", "shiftcut browser ensure"],
  ["Purge a stale/partial download and re-download", "shiftcut browser ensure --force"],
  ["Print the Chrome executable path", "shiftcut browser path"],
  ["Remove cached Chrome download", "shiftcut browser clear"],
];
...
async function runEnsure(options?: { force?: boolean }): Promise<void> {
  clack.intro(c.bold("shiftcut browser ensure"));
...
function runClear(): void {
  clack.intro(c.bold("shiftcut browser clear"));
...
export default defineCommand({
  ...
  async run({ args }) {
    ...
    if (!subcommand || subcommand === "") {
      console.log(`
${c.bold("shiftcut browser")} ${c.dim("<subcommand>")}
...
${c.bold("EXAMPLES:")}
  ${c.accent("npx shiftcut browser ensure")}           ${c.dim("Download Chrome if needed")}
  ${c.accent("npx shiftcut browser ensure --force")}   ${c.dim("Purge a stale/partial download and re-download")}
  ${c.accent("npx shiftcut browser path")}             ${c.dim("Print path for scripts")}
  ${c.accent("npx shiftcut browser clear")}            ${c.dim("Remove cached browser")}
`);
      return;
    }
    ...
    switch (subcommand) {
      ...
      default:
        ...
        console.error(
          `${c.error("Unknown subcommand:")} ${subcommand}\n\nRun ${c.accent("shiftcut browser --help")} for usage.`,
        );
        process.exit(1);
    }
  },
