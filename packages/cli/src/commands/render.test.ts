...
// real writeConfig's silent fs-error swallowing (unwritable ~/.shiftcut):
...
describe("renderLocal browser GPU config", () => {
  ...
  beforeEach(() => {
    ...
    resetTrialState();
    savedEnv.clear();
    savedEnv.set("SHIFTCUT_FFMPEG_PATH", process.env.SHIFTCUT_FFMPEG_PATH);
    savedEnv.set("SHIFTCUT_FFPROBE_PATH", process.env.SHIFTCUT_FFPROBE_PATH);
    ...
    delete process.env.SHIFTCUT_FFMPEG_PATH;
  ...
  it("passes preflight-resolved FFmpeg, FFprobe, and browser paths through env", async () => {
    ...
    expect(process.env.SHIFTCUT_FFMPEG_PATH).toBe("/usr/bin/ffmpeg");
    expect(process.env.SHIFTCUT_FFPROBE_PATH).toBe("/usr/bin/ffprobe");
...
describe("renderLocal — DE parallel-router CLI trial", () => {
  ...
  it("does not enable the trial when config.telemetryEnabled is false, even if shouldTrack() is stale-true (e.g. `shiftcut telemetry off` mid-batch)", async () => {
    configState.disk = {
  ...
  it("observes a telemetry opt-out written by another process mid-batch (arm site reads fresh, not cached)", async () => {
    ...
    // Another process runs `shiftcut telemetry off`, writing straight to
  ...
  it("blocks re-arming for the rest of the process when the fired flag can never persist (unwritable config)", async () => {
    ...
    configState.failWrites = Number.MAX_SAFE_INTEGER; // ~/.shiftcut is unwritable
    producerState.executeImpl = async (job) => {
      job.perfSummary = {
        resolution: { width: 100, height: 100 },
        drawElement: { parallelRouter: "reverted" },
...
describe("checkRenderResolutionPreflight", () => {
  ...
  describe("aspect-agnostic (--resolution 1080p / hd / 4k / uhd)", () => {
    ...
    it("blocks IG 4:5 (non-preset aspect) early with an aspect-aware message", async () => {
      ...
      // `@shiftcut/parsers/outputResolutionCompatibility`).
