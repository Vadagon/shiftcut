...
export const examples: Example[] = [
  ["Preview the current project", "shiftcut preview"],
  ["Print the current Studio selection as JSON", "shiftcut preview --selection --json"],
  ["Print current Studio context as JSON", "shiftcut preview --context --json"],
  ["Preview a specific project directory", "shiftcut preview ./my-video"],
  ["Use a custom port", "shiftcut preview --port 8080"],
  ["Force a new server even if one is already running", "shiftcut preview --force-new"],
  ["Keep preview running after this command exits", "shiftcut preview --background"],
  ["Show the background preview for this project", "shiftcut preview --status"],
  ["Stop the background preview for this project", "shiftcut preview --stop"],
  ["Start without opening the browser", "shiftcut preview --no-open"],
  ["Open with a specific browser", "shiftcut preview --browser-path /usr/bin/chromium"],
  [
    "Open with CDP enabled (requires browser path + isolated profile)",
    "shiftcut preview --browser-path /usr/bin/chromium --user-data-dir /tmp/hf-profile --remote-debugging-port 9222",
  ],
  ["List all active preview servers", "shiftcut preview --list"],
  ["Kill all active preview servers", "shiftcut preview --kill-all"],
  [
    "Disable auto-proxying of browser-hostile video codecs (HEVC, ProRes, AV1)",
    "shiftcut preview --no-proxy",
  ],
];
import {
  existsSync,
...
import { parseStoryboard, STORYBOARD_FILENAME } from "@shiftcut/core/storyboard";
...
import type { StudioSelectionSnapshot } from "@shiftcut/studio-server";
import {
  openBrowser,
...
export default defineCommand({
  ...
  args: {
    ...
    proxy: {
      type: "boolean",
      description:
        "Auto-transcode browser-hostile video codecs (HEVC, ProRes, AV1) to a cached authoring proxy for preview (default: on; overrides shiftcut.json's media.autoProxy)",
  ...
  async run({ args }) {
    ...
    // If @shiftcut/studio is installed locally, use Vite for full HMR
    ...
    if (args.background) {
      ...
      clack.intro(c.bold("shiftcut preview"));
      printStudioSummary(projectName, url, {
        ...
        footer: `Stop with: shiftcut preview ${JSON.stringify(dir)} --stop`,
      });
...
async function printCurrentSelection(
  ...
  if (!server) {
    printSelectionFailure(
      "preview-not-running",
      "No running Studio preview found for this project. Start one with: npx shiftcut preview",
      json,
    );
    return;
  }
...
/**
 * Check if @shiftcut/studio is installed locally in the project's node_modules.
 */
function hasLocalStudio(dir: string): boolean {
  try {
    const req = createRequire(join(dir, "package.json"));
    req.resolve("@shiftcut/studio/package.json");
    return true;
  } catch {
    return false;
  }
...
/**
 * Local studio mode: spawn Vite using a locally installed @shiftcut/studio.
...
async function runLocalStudioMode(dir: string, options?: StudioLaunchOptions): Promise<void> {
  const req = createRequire(join(dir, "package.json"));
  const studioPkgPath = dirname(req.resolve("@shiftcut/studio/package.json"));
  const pName = options?.projectName ?? basename(dir);
  ...
  clack.intro(c.bold("shiftcut preview") + c.dim(" (local studio)"));
  const s = clack.spinner();
  s.start("Starting studio...");
 ...
 *
 * If an existing ShiftCut server for the same project is detected,
...
async function runEmbeddedMode(
  ...
  printStudioSummary(pName, url, {
    details: [
      "Edit with your AI agent — it has ShiftCut skills installed.",
      "Changes reload automatically in the studio.",
    ],
  ...
  return new Promise<void>((resolveRun) => {
    ...
    const shutdown = (): void => {
      ...
      const cleanup = async () => {
        ...
        const { drainBrowserPool, killTrackedProcesses } = await import("@shiftcut/engine");
