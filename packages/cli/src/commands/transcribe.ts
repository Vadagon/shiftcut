...
export const examples: Example[] = [
  ["Transcribe an audio file", "shiftcut transcribe audio.mp3"],
  ["Transcribe a video file", "shiftcut transcribe video.mp4"],
  ["Use a larger model for better accuracy", "shiftcut transcribe audio.mp3 --model medium.en"],
  ["Set language to filter non-target speech", "shiftcut transcribe audio.mp3 --language en"],
  ["Import an existing SRT file", "shiftcut transcribe subtitles.srt"],
  ["Import an OpenAI Whisper JSON response", "shiftcut transcribe response.json"],
  ["Export captions to SRT", "shiftcut transcribe transcript.json --to srt"],
  [
    "Export single-word/CJK captions without re-grouping",
    "shiftcut transcribe transcript.json --to vtt --preserve-cues",
  ],
];
...
// Minimum accepted value for `--timeout` / `SHIFTCUT_TRANSCRIBE_TIMEOUT_MS`.
  ...
  args: {
    ...
    timeout: {
      ...
      description:
        ...
        "Env: SHIFTCUT_TRANSCRIBE_TIMEOUT_MS.",
    },
  },
  async run({ args }) {
 ...
 * `SHIFTCUT_TRANSCRIBE_TIMEOUT_MS` env var. The flag wins over env; both
...
function parseTimeoutMs(raw: string | undefined, json: boolean): number | undefined {
  const source = raw ?? process.env["SHIFTCUT_TRANSCRIBE_TIMEOUT_MS"];
  if (source == null || source === "") return undefined;

  const parsed = Number.parseInt(source, 10);
  if (!Number.isFinite(parsed) || parsed < CLI_TIMEOUT_MIN_MS) {
    const origin = raw != null ? "--timeout" : "SHIFTCUT_TRANSCRIBE_TIMEOUT_MS";
    failWith(
