...
export const examples: Example[] = [
  [
    ...
    "shiftcut remove-background avatar.mp4 -o transparent.webm",
  ],
  [
    "Output ProRes 4444 .mov for editing round-trip",
    "shiftcut remove-background avatar.mp4 -o transparent.mov",
  ],
  [
    "Remove background from a single image, output transparent PNG",
    "shiftcut remove-background portrait.jpg -o cutout.png",
  ],
  [
    "Separate the layers — emit both the cutout and an inverse-alpha background plate (subject region transparent)",
    "shiftcut remove-background avatar.mp4 -o subject.webm --background-output plate.webm",
  ],
  [
    "Force CPU (skip CoreML/CUDA)",
    "shiftcut remove-background avatar.mp4 -o transparent.webm --device cpu",
  ],
  [
    "Smaller file at the cost of color match (text-behind-subject won't blend as cleanly)",
    "shiftcut remove-background avatar.mp4 -o transparent.webm --quality fast",
  ],
  [
    "Visually-lossless WebM (master / re-encode source)",
    "shiftcut remove-background avatar.mp4 -o transparent.webm --quality best",
  ],
  ["Show detected providers without rendering", "shiftcut remove-background --info"],
];
...
export default defineCommand({
  ...
  async run({ args }) {
    ...
    if (!args.input) {
      console.error(
        c.error(
          "Input file is required. Run `shiftcut remove-background --info` for providers.",
        ),
      );
      process.exit(1);
    }
...
async function showInfo(json: boolean): Promise<void> {
  ...
  console.log(c.bold("shiftcut remove-background — system info"));
  console.log("");
