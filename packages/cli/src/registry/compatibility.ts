...
export function checkRegistryItemCompatibility(
  ...
  return {
    warnings,
    error:
      `Registry item "${item.name}" requires shiftcut >= ${minCliVersion} ` +
      `(current: ${currentCliVersion}). Run \`npx shiftcut@latest add ${item.name}\` ` +
      "or upgrade your installed shiftcut CLI.",
  };
}
 ...
 * rejected uniformly, not just by `shiftcut add`.
 */
