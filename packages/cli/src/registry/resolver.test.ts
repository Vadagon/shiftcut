...
const MANIFEST: RegistryManifest = {
  ...
  items: [
    { name: "alpha", type: "shiftcut:example" },
    { name: "beta", type: "shiftcut:example" },
    { name: "gamma", type: "shiftcut:block" },
  ],
};

function buildItem(name: string, type: "shiftcut:example" | "shiftcut:block"): RegistryItem {
  if (type === "shiftcut:example") {
    return {
      name,
      type,
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (urlInput: string | URL) => {
      ...
      if (m && !overrides.missing?.includes(m[2]!)) {
        const type = m[1] === "examples" ? "shiftcut:example" : "shiftcut:block";
...
describe("registry resolver", () => {
  ...
  describe("listRegistryItems", () => {
    ...
    it("filters by type", async () => {
      ...
      const examples = await listRegistryItems({ type: "shiftcut:example" }, { baseUrl });
      expect(examples.map((i) => i.name)).toEqual(["alpha", "beta"]);

      const blocks = await listRegistryItems({ type: "shiftcut:block" }, { baseUrl });
  ...
  describe("resolveItem", () => {
    it("returns the full manifest for a known item", async () => {
      ...
      expect(item.type).toBe("shiftcut:example");
