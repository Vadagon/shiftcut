...
import type { FileTarget, RegistryItem } from "@shiftcut/core";
...
function isInstalledRegistryBlockComposition(item: RegistryItem, file: FileTarget): boolean {
  return (
    ...
    file.type === "shiftcut:composition" &&
    file.target.toLowerCase().endsWith(".html")
  );
}

function addRegistryItemMarker(source: string, item: RegistryItem): string {
  if (/^\s*<!--\s*shiftcut-registry-item:[^>]*-->/i.test(source.slice(0, 512))) {
    return source;
  }

  return `<!-- shiftcut-registry-item: ${item.name} -->\n${source}`;
}
