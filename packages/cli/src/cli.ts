...
// `shiftcut --version` near-instant (~10ms vs ~80ms).
...
// Verblike, ElevenLabs). Load it automatically so they don't need `source .env`.
try {
...
function reportPostRenderTerminationEvent(
  ...
  process.stderr.write(
    `  [shiftcut] Post-render ${label} (render already succeeded): ${error.message}\n`,
  );
