...
const MODELS_DIR = join(homedir(), ".cache", "shiftcut", "whisper", "models");
...
function findFromEnv(): WhisperResult | undefined {
  const envPath = process.env["SHIFTCUT_WHISPER_PATH"];
...
const BUILD_DIR = join(homedir(), ".cache", "shiftcut", "whisper", "whisper.cpp");
...
function buildFromSource(onProgress?: (msg: string) => void): WhisperResult {
  ...
  if (!existsSync(BUILD_DIR)) {
    ...
    mkdirSync(join(homedir(), ".cache", "shiftcut", "whisper"), {
      recursive: true,
    });
