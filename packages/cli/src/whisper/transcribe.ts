 ...
 * one `shiftcut transcribe` per scene at once). A `Date.now()`-based name
...
function tempWavPath(): string {
  return join(tmpdir(), `shiftcut-audio-${process.pid}-${randomUUID()}.wav`);
}
...
export function wrapWhisperTimeoutError(err: unknown, options: WrapWhisperTimeoutOptions): Error {
  ...
  const message =
    ...
    `Raise --timeout <ms> or set SHIFTCUT_TRANSCRIBE_TIMEOUT_MS. ` +
