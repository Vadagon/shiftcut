/**
 * Port utilities for the ShiftCut preview server.
 *
 ...
 * - HTTP probe for detecting existing ShiftCut instances
...
/** Localhost HTTP probe timeout — ShiftCut responds in <1ms, so 300ms is generous. */
...
interface ShiftcutConfigResponse {
  isShiftcut: boolean;
  pid?: number;
  projectName: string;
  projectDir: string;
...
/**
 * Probe an occupied port to check if it's running a ShiftCut preview server.
 * HTTP GET to /__shiftcut_config with a short timeout.
 */
export function detectShiftcutServer(
  port: number,
    ...
    const req = http.get(
      {
        hostname: "127.0.0.1",
        port,
        path: "/__shiftcut_config",
        timeout: PROBE_TIMEOUT_MS,
      },
      (res) => {
        if (res.statusCode !== 200) {
          res.resume();
          return resolveResult({ type: "not-shiftcut" });
        }
        ...
        res.on("error", () => {
          resolveResult({ type: "not-shiftcut" });
        });
        res.on("end", () => {
          try {
            const json = JSON.parse(data) as ShiftcutConfigResponse;
            if (json.isShiftcut !== true) {
              return resolveResult({ type: "not-shiftcut" });
            }
...
/**
 * Probe a single port for a ShiftCut config response.
 * Returns the full config or null if not a ShiftCut server.
 */
function probePort(port: number): Promise<ShiftcutConfigResponse | null> {
  return new Promise<ShiftcutConfigResponse | null>((resolveResult) => {
    const req = http.get(
      { hostname: "127.0.0.1", port, path: "/__shiftcut_config", timeout: PROBE_TIMEOUT_MS },
      (res) => {
        if (res.statusCode !== 200) {
          res.resume();
        ...
        res.on("end", () => {
          try {
            const json = JSON.parse(data) as ShiftcutConfigResponse;
            resolveResult(json.isShiftcut === true ? json : null);
          } catch {
            resolveResult(null);
          }
        });
...
/**
 * Scan the default port range for active ShiftCut preview servers.
...
/**
 * Kill all active ShiftCut preview servers by sending SIGTERM to their PIDs.
 ...
 *   3. If occupied and !forceNew → HTTP-probe for an existing ShiftCut server
 *      - Same project → return "already-running" (caller reopens browser)
 *      - Different project or non-ShiftCut → log and skip to next port
...
export async function findPortAndServe(
  ...
  // need LAN exposure opt in explicitly via the SHIFTCUT_PREVIEW_HOST
  // env var (e.g. SHIFTCUT_PREVIEW_HOST=0.0.0.0).
  const host = bindHost ?? (process.env.SHIFTCUT_PREVIEW_HOST?.trim() || "127.0.0.1");
  ...
  for (let port = startPort; port <= endPort; port++) {
    ...
    // Port is occupied — probe for existing ShiftCut instance
    if (!forceNew) {
      const detection = await detectShiftcutServer(
        port,
        normalizedDir,
      ...
      if (detection.type === "mismatch") {
        console.log(
          `  ${c.dim(`Port ${port} in use by ShiftCut project "${detection.projectName}" — skipping`)}`,
        );
        continue;
      }
    }
