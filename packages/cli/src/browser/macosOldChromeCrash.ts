 ...
 * launch crashes.
 *
 * Field feedback (#shiftcut-cli-feedback ts=1784227832, darwin/x64,
 * macOS 12, ShiftCut CLI 0.7.60) hit
 ...
 * detect the launch-time crash signal, surface `SHIFTCUT_BROWSER_PATH`
...
export function isMacosOldChromeCrashError(errorMessage: string): boolean {
  ...
  // mismatch, etc. — none of which are fixed by SHIFTCUT_BROWSER_PATH).
...
export function macosOldChromeCrashRemediation(errorMessage: string): string | undefined {
  ...
  return [
    ...
    "chrome-headless-shell and point shiftcut at it:",
    "",
    "  npx @puppeteer/browsers install chrome-headless-shell@150",
    '  export SHIFTCUT_BROWSER_PATH="$HOME/.cache/puppeteer/chrome-headless-shell/mac-150.0.7422.0/chrome-headless-shell-mac-x64/chrome-headless-shell"',
    "",
    ...
    "known-good macOS-12 combination. Alternatively, point SHIFTCUT_BROWSER_PATH at your",
