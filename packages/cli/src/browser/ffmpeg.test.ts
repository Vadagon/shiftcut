...
// are covered by @shiftcut/parsers ffBinaries.test.ts. These tests pin the
...
describe("findFFmpeg / findFFprobe", () => {
  it("returns undefined when the env override points at a missing file", () => {
    process.env.SHIFTCUT_FFMPEG_PATH = join(tmpdir(), "missing-ffmpeg");
    process.env.SHIFTCUT_FFPROBE_PATH = join(tmpdir(), "missing-ffprobe");
