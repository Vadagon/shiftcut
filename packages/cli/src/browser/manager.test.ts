 ...
 *      own `~/.cache/shiftcut/chrome` cache — leaving the engine without a
...
const CACHE_ROOT = join(FAKE_HOME, ".cache", "shiftcut");
const HF_CACHE = join(FAKE_HOME, ".cache", "shiftcut", "chrome");
...
describe("findBrowser — cache resolution", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env["SHIFTCUT_BROWSER_PATH"];
  ...
  it("resolves to the shiftcut-managed cache when puppeteer cache is empty", async () => {
  ...
  it("does not resolve to a shiftcut-cache build from an older CHROME_VERSION pin", async () => {
    // A build downloaded by a prior shiftcut version (this pin has moved
  ...
  it("re-downloads when the shiftcut cache manifest points at a missing binary", async () => {
  ...
  it("ensureBrowser does not leak the install lock directory after a successful download", async () => {
    ...
    // reported scenario: two `shiftcut browser ensure` runs racing) hit
  ...
  it("withInstallLock reports progress while waiting instead of staying silent", async () => {
    ...
    expect(
      warnSpy.mock.calls.some(([msg]) =>
        String(msg).includes("Waiting for another shiftcut process"),
      ),
    ).toBe(true);
  });
  ...
  it("warns and falls through when the shiftcut cache cannot be read", async () => {
  ...
  it("falls back to the puppeteer-managed cache when shiftcut cache is empty", async () => {
    // Empty shiftcut cache, populated puppeteer cache — the regression
  ...
  it("prefers the puppeteer cache over the shiftcut cache when BOTH are populated", async () => {
  ...
  it("does NOT warn when the system path happens to be chrome-headless-shell", async () => {
    // SHIFTCUT_BROWSER_PATH-style override pointing directly at a
    ...
    installPuppeteerBrowsersMock();
    process.env["SHIFTCUT_BROWSER_PATH"] = directShell;
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
  ...
  // all instruct users to set that name. Before this alias, `shiftcut
  ...
  // (win32/x64 STATUS_STACK_BUFFER_OVERRUN 3221225595, #shiftcut-cli-feedback
  ...
  it("resolves via PRODUCER_HEADLESS_SHELL_PATH when SHIFTCUT_BROWSER_PATH is unset", async () => {
    const directShell = "/opt/chrome-headless-shell/chrome-headless-shell";
    installFsMocks({ existing: new Set([directShell]) });
    installPuppeteerBrowsersMock();
  ...
  it("prefers SHIFTCUT_BROWSER_PATH over PRODUCER_HEADLESS_SHELL_PATH when both are set", async () => {
    ...
    installPuppeteerBrowsersMock();
    process.env["SHIFTCUT_BROWSER_PATH"] = hfPath;
...
// #shiftcut-cli-feedback ts 1784055194.202169 (darwin/arm64, HF CLI 0.7.57)
// hit `All providers failed for chrome-headless-shell 152.0.7928.2` at download
// time and had to discover `SHIFTCUT_BROWSER_PATH` on their own. The raw
...
describe("downloadBrowser — install failure surfaces SHIFTCUT_BROWSER_PATH hint", () => {
  const origPlatform = process.platform;
  const origArch = process.arch;
  ...
  it.each([
    ...
    "rethrows a non-corrupt install failure with an SHIFTCUT_BROWSER_PATH hint and preserves the original via cause ($label)",
    async ({ platform, arch, expectedPathHint }) => {
      ...
      expect(msg).toContain("SHIFTCUT_BROWSER_PATH");
...
// Regression guard for HF#2103: `shiftcut render` hung forever on macOS
