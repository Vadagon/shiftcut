...
async function refreshSkillsCache(): Promise<SkillsUpdateMeta> {
  ...
  // inside a shiftcut checkout) can produce a false-positive count here.
...
function skillsNoticeText(meta: SkillsUpdateMeta): string | null {
  ...
  return `\n  ${total} ShiftCut ${noun} out of date or missing.\n  Run: npx shiftcut skills update\n\n`;
}

/**
 * Print a one-line nudge to stderr if installed skills are stale. Same gating
 * as the CLI self-update notice (CI, non-TTY, dev, SHIFTCUT_NO_UPDATE_CHECK).
 */
