/**
 * Read and write `shiftcut.json` — the per-project config that tells
 * `shiftcut add` which registry to pull items from and where to drop them
 * in the user's project tree.
 *
 * The file is created by `shiftcut init` and optionally edited by users to
...
export const PROJECT_CONFIG_FILENAME = "shiftcut.json";
const PROJECT_CONFIG_SCHEMA_URL = "https://shiftcut.verblike.com/schema/shiftcut.json";

export interface ProjectConfigPaths {
  /** Where `shiftcut:block` items land, relative to project root. */
  blocks: string;
  /** Where `shiftcut:component` items land, relative to project root. */
  components: string;
...
/** Read `shiftcut.json` from a project directory. */
...
/** Write `shiftcut.json` to a project directory. Overwrites if present. */
 ...
 * direction. Falls back to the committed `shiftcut.json`
