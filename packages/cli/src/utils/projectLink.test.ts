...
// Point the module's ~/.shiftcut config dir at a throwaway home so tests use real fs
...
describe("projectLink", () => {
  ...
  beforeEach(async () => {
    ...
    projectsPath = join(osState.home, ".shiftcut", "projects.json");
    projectDirs = [];
    vi.resetModules();
  ...
  it("round-trips a project link", () => {
    ...
    const link = { projectId: "hfp_123", url: "https://shiftcut.verblike.com/p/hfp_123" };
  ...
  it("persists only the project id and URL", () => {
    const dir = makeProjectDir();
    const link = { projectId: "hfp_123", url: "https://shiftcut.verblike.com/p/hfp_123", secret: "nope" };
