...
import { resolveProxy } from "@shiftcut/studio-server/proxy-transcoder";
...
export async function preResolveHostileMediaProxies(
  ...
  try {
    ...
    console.info(
      `[shiftcut] media proxy pre-resolve: scan failed (${normalizeErrorMessage(err)})`,
    );
    return;
  }
  ...
  console.info(
    `[shiftcut] media proxy pre-resolve: ${results.length - failed}/${results.length} ready, ${failed} failed (${Date.now() - startedAt}ms)`,
  );
}
...
export async function runBrowserCheck(
  ...
  try {
    ...
    const { analyzeClipMediaFit } = await import("@shiftcut/engine");
...
const MEDIA_PROXY_MARKER_PREFIX = "[shiftcut] runtime_media_proxy_";
const MEDIA_PROXY_UNAVAILABLE_MARKER = "[shiftcut] runtime_media_proxy_unavailable";
...
async function collectLayout(
  ...
  const raw = await page.evaluate(
    (options: { time: number; tolerance: number }) => {
      const audit = Reflect.get(window, "__shiftcutLayoutAudit");
...
async function collectLayoutGeometry(page: Page): Promise<string> {
  return page.evaluate(() => {
    const geometry = Reflect.get(window, "__shiftcutLayoutGeometry");
...
async function collectGeometryCandidates(
  ...
  try {
    const raw = await page.evaluate((options: GeometryCandidateRequest) => {
      const collect = Reflect.get(window, "__shiftcutGeometryCandidates");
...
async function collectMotionFrame(
  ...
  const raw = await page.evaluate(
    (options: { selectors: string[]; livenessScopes: string[] }) => {
      const sample = Reflect.get(window, "__shiftcutMotionSample");
...
const ANNOTATION_OVERLAY_ID = "__shiftcutCheckAnnotations";
