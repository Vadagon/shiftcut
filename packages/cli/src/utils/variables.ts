 ...
 * and validation helpers used by both `shiftcut render` (in-process) and
 * `shiftcut lambda render` (distributed). The Lambda CLI mirrors the
 ...
 * `shiftcut render` and `shiftcut lambda render` so the UX is
