/**
 * Detect how the running `shiftcut` binary was installed so auto-update can
...
/**
 * True when invoked via `npx shiftcut` / `bunx shiftcut`. These don't
 ...
 * symlinks `/opt/homebrew/bin/shiftcut` into `…/Cellar/shiftcut/<v>/…`
 * (or `/usr/local/Cellar/` on Intel). Either path wins the match.
 */
function isHomebrewInstall(realEntry: string): boolean {
  return normalizePath(realEntry).includes("/Cellar/shiftcut/");
}
...
export function detectInstaller(): InstallerInfo {
  ...
  if (isHomebrewInstall(realEntry)) {
    return {
      ...
      installCommand: () => "brew upgrade shiftcut",
  ...
  if (normalizedEntry.includes("/.bun/")) {
    return {
      kind: "bun",
      installCommand: (version) => `bun add -g shiftcut@${version}`,
  ...
  if (normalizedEntry.includes("/pnpm/global/")) {
    return {
      kind: "pnpm",
      installCommand: (version) => `pnpm add -g shiftcut@${version}`,
      reason: `pnpm global install detected at ${realEntry}`,
    };
  }

  // npm's default global prefix is `<prefix>/lib/node_modules/shiftcut/…`
  // where `<prefix>` is `/usr/local` (macOS Intel), `/opt/homebrew` (Apple
  // Silicon, non-brew-formula npm), or a user-configured directory.
  if (
    normalizedEntry.includes("/lib/node_modules/shiftcut/") ||
    normalizedEntry.includes("/npm/node_modules/shiftcut/")
  ) {
    return {
      kind: "npm",
      installCommand: (version) => `npm install -g shiftcut@${version}`,
...
export function installInvocation(kind: InstallerKind, version: string): InstallInvocation | null {
  switch (kind) {
    case "npm":
      return { bin: "npm", args: ["install", "-g", `shiftcut@${version}`] };
    case "bun":
      return { bin: "bun", args: ["add", "-g", `shiftcut@${version}`] };
    case "pnpm":
      return { bin: "pnpm", args: ["add", "-g", `shiftcut@${version}`] };
    case "brew":
      // brew has no per-version install; `brew upgrade` moves to the tap's
      // current formula (a no-op if the tap hasn't caught up).
      return { bin: "brew", args: ["upgrade", "shiftcut"] };
    case "skip":
      return null;
  }
}
