 ...
 *
 * Flow across two runs of `shiftcut`:
 *
 ...
 *     via `shiftcut upgrade`.
 *   - Skip on CI, non-TTY, dev mode, unknown installer, ephemeral exec (npx),
 *     or when `SHIFTCUT_NO_AUTO_INSTALL` / `SHIFTCUT_NO_UPDATE_CHECK`
 *     is set.
 *   - If a previous install is still in flight (less than 10 min old), don't
 *     re-launch.
 *   - Installer output is redirected to `~/.shiftcut/auto-update.log` for
...
const CONFIG_DIR = join(homedir(), ".shiftcut");
...
function isAutoInstallDisabled(): boolean {
  ...
  if (process.env["SHIFTCUT_NO_AUTO_INSTALL"] === "1") return true;
  return false;
}
...
function launchDetachedInstall(
  ...
  const child = spawn(process.execPath, ["-e", nodeScript], {
    ...
    windowsHide: true,
    env: { ...process.env, SHIFTCUT_NO_UPDATE_CHECK: "1", SHIFTCUT_NO_AUTO_INSTALL: "1" },
  });
  child.unref();
...
export function reportCompletedUpdate(): void {
  ...
  if (done.ok) {
    process.stderr.write(`  shiftcut auto-updated to v${done.version}\n\n`);
  } else if (!done.reported) {
    // Failed installs are surfaced once too — the user should know why the
    // auto-update didn't take.
    process.stderr.write(
      `  shiftcut auto-update to v${done.version} failed. Run \`shiftcut upgrade\` to retry.\n\n`,
    );
  }
}
