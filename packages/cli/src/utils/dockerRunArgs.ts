/**
 * Build the argument array for `docker run` that invokes the Shiftcut
 ...
 */
import { fpsToFfmpegArg, type Fps } from "@shiftcut/core";
...
export interface DockerRenderOptions {
  /**
   * Frame rate as an exact rational; see `Fps` in @shiftcut/core. The
 ...
 *
 * Honors `SHIFTCUT_DOCKER_PLATFORM` as an escape hatch (typed loosely so
...
export function resolveDockerPlatform(
  ...
  const override = env.SHIFTCUT_DOCKER_PLATFORM;
