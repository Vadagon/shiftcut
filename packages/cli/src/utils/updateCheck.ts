...
import { readPinnedShiftcutVersions } from "./projectPin.js";
import { isSafeVersion } from "./safeVersion.js";

export { isSafeVersion } from "./safeVersion.js";

const NPM_REGISTRY_URL = "https://registry.npmjs.org/shiftcut/latest";
...
export function printDeprecationNotice(command: string): void {
  process.stderr.write(
    `'shiftcut ${command}' is deprecated and will be removed in a future release. Use 'shiftcut check' instead.\n`,
  );
}

/**
 * True when update / freshness notices should stay silent — CI, non-TTY, dev
 * mode, or the SHIFTCUT_NO_UPDATE_CHECK opt-out. Shared with the skills
...
export function updateNoticesSuppressed(): boolean {
  ...
  if (process.env["SHIFTCUT_NO_UPDATE_CHECK"] === "1") return true;
  return false;
}

/**
 * Print update notice to stderr if a newer version is available.
 * Skipped in CI, non-TTY, dev mode, or when SHIFTCUT_NO_UPDATE_CHECK is set.
 */
export function printUpdateNotice(): void {
  ...
  // universal `npx shiftcut@latest` for ephemeral/unknown installs (where a
  ...
  const command = managerCommand ?? "npx shiftcut@latest";
 ...
 * OLD shiftcut version. Unlike printUpdateNotice this DOES fire on non-TTY
...
export function printStalePinNotice(cwd: string = process.cwd()): void {
  ...
  if (process.env["SHIFTCUT_NO_UPDATE_CHECK"] === "1") return;
  ...
  const stale = readPinnedShiftcutVersions(scripts).filter((v) => {
    try {
  ...
  process.stderr.write(
    `\n  This project pins shiftcut@${stale.join(", ")} (latest ${latest}).\n` +
      `  Bump it: npx shiftcut@latest upgrade --project\n\n`,
  );
}
