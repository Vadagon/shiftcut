...
describe("printUpdateNotice — install-method-aware command", () => {
  ...
  it("shows the detected manager's command for an owned global install", async () => {
    const out = await noticeWith({ installerCommand: "brew upgrade shiftcut" });
    expect(out).toContain("Update available");
    expect(out).toContain("brew upgrade shiftcut");
    expect(out).not.toContain("npx shiftcut@latest");
  });

  it("falls back to npx shiftcut@latest when the install method is skip/unknown", async () => {
    const out = await noticeWith({ installerCommand: null });
    expect(out).toContain("npx shiftcut@latest");
  });

  it("is suppressed on a non-TTY stderr", async () => {
    const out = await noticeWith({ installerCommand: "brew upgrade shiftcut", isTTY: false });
    expect(out).toBe("");
  });

  it("is suppressed in CI", async () => {
    const out = await noticeWith({
      installerCommand: "brew upgrade shiftcut",
      env: { CI: "true" },
    });
    expect(out).toBe("");
  });

  it("is suppressed by the SHIFTCUT_NO_UPDATE_CHECK opt-out", async () => {
    const out = await noticeWith({
      installerCommand: "brew upgrade shiftcut",
      env: { SHIFTCUT_NO_UPDATE_CHECK: "1" },
    });
    expect(out).toBe("");
  });
});
...
describe("printDeprecationNotice", () => {
  it("writes exactly one line to stderr, never stdout", () => {
    ...
    expect(stderrWrites[0]).toContain("shiftcut validate");
    expect(stderrWrites[0]).toContain("shiftcut check");
  });
});
