...
vi.mock("@shiftcut/studio-server/media-codec-map", async (importOriginal) => ({
  ...(await importOriginal<typeof import("@shiftcut/studio-server/media-codec-map")>()),
  scanProjectMediaCodecMap: mocks.scanProjectMediaCodecMap,
  proxyVariantFor: (facts: { hasAlpha?: boolean }) => (facts.hasAlpha ? "vp8" : "h264"),
}));
vi.mock("@shiftcut/studio-server/proxy-transcoder", () => ({
...
afterEach(() => {
  ...
  Reflect.deleteProperty(window, "__shiftcutGeometryCandidates");
  Reflect.deleteProperty(window, "__shiftcutLayoutAudit");
...
it("surfaces the runtime's media-proxy-fallback console.info line as an info finding, ignoring unrelated info logs", async () => {
  ...
  const fallbackMessage = fakeConsoleMessage(
    "info",
    '[shiftcut] runtime_media_proxy_fallback: "assets/clip.mp4" uses a codec (hevc) this browser can\'t decode; ' +
      "auto-swapped to an authoring proxy for this preview only. Render output is unaffected.",
  );
  const unrelatedInfo = fakeConsoleMessage("info", "[shiftcut] render runtime fps 30");
...
it("surfaces the runtime's media-proxy-unavailable console.info line as its own info finding", async () => {
  ...
  const unavailableMessage = fakeConsoleMessage(
    "info",
    '[shiftcut] runtime_media_proxy_unavailable: "https://cdn.example.com/video.mp4" (cross_origin): ' +
...
describe("preResolveHostileMediaProxies", () => {
  ...
  it("does nothing (no scan, no resolveProxy) when autoProxy is off in shiftcut.json", async () => {
    const projectDir = mkProjectDir();
    writeFileSync(
