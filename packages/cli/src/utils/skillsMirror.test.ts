...
describe("mirrorGlobalSkills", () => {
  ...
  it("mirrors the store into installed agents as relative symlinks (Unix)", () => {
    const home = makeHome();
    seedStore(home, ["shiftcut", "shiftcut-core"]);
    ...
    const link = join(home, ".cursor", "skills", "shiftcut");
    expect(lstatSync(link).isSymbolicLink()).toBe(true);
    expect(isAbsolute(readlinkSync(link))).toBe(false); // relative target
    expect(realpathSync(link)).toBe(realpathSync(join(home, ".claude", "skills", "shiftcut")));
    expect(existsSync(join(link, "SKILL.md"))).toBe(true);

    // goose lands in the XDG config dir (~/.config/goose), not ~/.goose
    expect(
      existsSync(join(home, ".config", "goose", "skills", "shiftcut-core", "SKILL.md")),
    ).toBe(true);
  });
  ...
  // ShiftCut' own skills (the lock-attributed allow-list), never everything
  ...
  it("only mirrors the allow-listed skills, never other sources' (gstack)", () => {
    const home = makeHome();
    seedStore(home, ["shiftcut", "gstack"]); // gstack is a foreign skill in the store
    ...
    mirrorGlobalSkills({ skills: ["shiftcut"], home, platform: "linux", env: ENV });

    // our skill got linked
    expect(lstatSync(join(home, ".cursor", "skills", "shiftcut")).isSymbolicLink()).toBe(true);
  ...
  it("honors XDG_CONFIG_HOME for config-based agents", () => {
    ...
    expect(existsSync(join(xdg, "goose", "skills", "shiftcut", "SKILL.md"))).toBe(true);
  ...
  it("copies instead of symlinking on Windows", () => {
    ...
    mirrorGlobalSkills({ skills: ["shiftcut"], home, platform: "win32", env: ENV });
    const target = join(home, ".cursor", "skills", "shiftcut");
