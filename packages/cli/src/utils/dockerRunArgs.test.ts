...
const FIXED_INPUT = {
  imageTag: "shiftcut-renderer:0.0.0-test",
  projectDir: "/abs/proj",
  outputDir: "/abs/out",
...
describe("buildDockerRunArgs", () => {
  ...
  it("requests host GPU passthrough only when gpu is enabled", () => {
    ...
    // shiftcut CLI flag forwarded into the container — both must be set.
...
describe("resolveDockerPlatform", () => {
  ...
  it("uses process.arch and process.env when called with no arguments", () => {
    ...
    const expected = process.env.SHIFTCUT_DOCKER_PLATFORM
      ? process.env.SHIFTCUT_DOCKER_PLATFORM
      : resolveDockerPlatform(process.arch, {});
    expect(result).toBe(expected);
  });

  it("honors SHIFTCUT_DOCKER_PLATFORM override on an arm64 host (Rosetta-Node / parity-regen escape hatch)", () => {
    expect(resolveDockerPlatform("arm64", { SHIFTCUT_DOCKER_PLATFORM: "linux/amd64" })).toBe(
      "linux/amd64",
    );
  });

  it("honors SHIFTCUT_DOCKER_PLATFORM override on an amd64 host", () => {
    expect(resolveDockerPlatform("x64", { SHIFTCUT_DOCKER_PLATFORM: "linux/arm64" })).toBe(
      "linux/arm64",
    );
  });

  it("trims whitespace from SHIFTCUT_DOCKER_PLATFORM and ignores empty override", () => {
    expect(resolveDockerPlatform("arm64", { SHIFTCUT_DOCKER_PLATFORM: "  linux/amd64  " })).toBe(
      "linux/amd64",
    );
    ...
    expect(resolveDockerPlatform("arm64", { SHIFTCUT_DOCKER_PLATFORM: "" })).toBe("linux/arm64");
    expect(resolveDockerPlatform("arm64", { SHIFTCUT_DOCKER_PLATFORM: "   " })).toBe(
      "linux/arm64",
    );
  });
});
