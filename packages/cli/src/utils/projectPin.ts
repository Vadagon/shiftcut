...
// Matches `shiftcut@<semver>` as a whole token inside a script string. The
...
export const SHIFTCUT_PIN_RE =
  /\bshiftcut@([0-9]+\.[0-9]+\.[0-9]+(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?)(?=\s|$)/g;
...
export function readPinnedShiftcutVersions(scripts: Record<string, string>): string[] {
  const found = new Set<string>();
  for (const cmd of Object.values(scripts ?? {})) {
    for (const m of cmd.matchAll(SHIFTCUT_PIN_RE)) if (m[1]) found.add(m[1]);
  }
  return [...found].sort();
}
...
export function rewriteProjectPinnedScripts(
  ...
  for (const [name, cmd] of Object.entries(scripts ?? {})) {
    next[name] = cmd.replace(SHIFTCUT_PIN_RE, (_full, version: string) => {
      if (version !== targetVersion) fromVersions.add(version);
      return `shiftcut@${targetVersion}`;
    });
  }
  return {
