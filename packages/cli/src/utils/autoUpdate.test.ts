...
function setupMocks(opts: {
  ...
  vi.doMock("./installerDetection.js", () => ({
    ...
    installInvocation: (kind: string, version: string) =>
      kind === "skip" ? null : { bin: kind, args: ["add", "-g", `shiftcut@${version}`] },
  }));
  ...
  delete process.env["SHIFTCUT_NO_AUTO_INSTALL"];
  delete process.env["SHIFTCUT_NO_UPDATE_CHECK"];
...
describe("scheduleBackgroundInstall", () => {
  ...
  it("schedules an install when a newer minor/patch is available", async () => {
    const { spawnSpy, writeSpy, config } = setupMocks({
      installer: { kind: "npm", command: "npm install -g shiftcut@0.4.4" },
    });
    ...
    expect(config.pendingUpdate?.command).toBe("npm install -g shiftcut@0.4.4");
  });

  it("does NOT schedule across a major-version jump", async () => {
    const { spawnSpy } = setupMocks({
      installer: { kind: "npm", command: "npm install -g shiftcut@1.0.0" },
    });
    const { scheduleBackgroundInstall } = await import("./autoUpdate.js");
  ...
  it("skips when SHIFTCUT_NO_AUTO_INSTALL=1", async () => {
    const { spawnSpy } = setupMocks({
      installer: { kind: "npm", command: "npm install -g shiftcut@0.4.4" },
      env: { SHIFTCUT_NO_AUTO_INSTALL: "1" },
    });
    const { scheduleBackgroundInstall } = await import("./autoUpdate.js");
  ...
  it("skips when already up to date", async () => {
    const { spawnSpy } = setupMocks({
      installer: { kind: "npm", command: "npm install -g shiftcut@0.4.3" },
    });
    const { scheduleBackgroundInstall } = await import("./autoUpdate.js");
  ...
  it("does not re-launch while a fresh pending install exists for the same version", async () => {
    const { spawnSpy } = setupMocks({
      ...
      config: {
        pendingUpdate: {
          version: "0.4.4",
          command: "npm install -g shiftcut@0.4.4",
  ...
  it("surfaces failed installs once but still blocks retries for the same version", async () => {
    ...
    try {
      ...
      expect(stderrWrite).toHaveBeenCalledWith(
        expect.stringContaining("shiftcut auto-update to v0.4.4 failed"),
      );
