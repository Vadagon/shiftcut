...
const SCRIPT_DIR = join(homedir(), ".cache", "shiftcut", "tts");
...
export async function synthesize(
  ...
  if (!python) {
    throw new Error(
      "Python 3 is required for text-to-speech. Install Python 3.10+ and run: pip install kokoro-onnx soundfile (or point SHIFTCUT_PYTHON at a venv python that has them)",
    );
  }

  if (!hasPythonPackage(python, "kokoro_onnx")) {
    throw new Error(
      "The kokoro-onnx package is not installed. Run: pip install kokoro-onnx soundfile (or point SHIFTCUT_PYTHON at a venv python that has them)",
    );
  }
