...
describe("synthesize — espeak-ng language code translation", () => {
  beforeEach(() => {
    resetCapturedArgv();
    delete process.env.SHIFTCUT_PYTHON;
  });

  it("translates the public zh lang to espeak-ng's cmn at the Python boundary", async () => {
    await synthesize("你好世界", "/tmp/shiftcut-test-zh.wav", { voice: "zf_xiaobei" });
  ...
  it("translates an explicit zh override too", async () => {
    ...
    await synthesize("你好世界", "/tmp/shiftcut-test-explicit-zh.wav", {
      voice: "af_heart",
      lang: "zh",
  ...
  it("keeps every non-Mandarin public lang unchanged at the Python boundary", async () => {
    for (const lang of SUPPORTED_LANGS) {
      ...
      await synthesize("Hello world", `/tmp/shiftcut-test-${lang}.wav`, {
        voice: "af_heart",
        lang,
      });
  ...
  it("leaves voice-inferred Spanish unchanged", async () => {
    await synthesize("Hola mundo", "/tmp/shiftcut-test-es.wav", {
      voice: "ef_dora",
    });
