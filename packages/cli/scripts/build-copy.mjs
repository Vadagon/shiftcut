...
async function main() {
  ...
  for (const skill of ["shiftcut", "shiftcut-cli", "gsap"]) {
  ...
  // this, `npx shiftcut present` fails with "@shiftcut/player not found".
  const playerDist = join(REPO_ROOT, "packages", "player", "dist");
  const playerGlobals = [
    [join(playerDist, "shiftcut-player.global.js"), join(DIST, "shiftcut-player.global.js")],
    [
      join(playerDist, "slideshow", "shiftcut-slideshow.global.js"),
      join(DIST, "shiftcut-slideshow.global.js"),
    ],
  ];
