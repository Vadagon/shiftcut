...
//   - .shiftcut/examples/  vendored under Studio's dir — also listed in the
//                             file tree, also gated out of discovery.
//   - .shiftcut/backup/    Studio's internal snapshots — the only thing hidden
...
function createProjectDir(): string {
  ...
  writeFileSync(join(projectDir, ".shiftcut", "examples", "preset.html"), COMPOSITION_HTML);
  mkdirSync(join(projectDir, ".shiftcut", "backup"), { recursive: true });
  writeFileSync(join(projectDir, ".shiftcut", "backup", "snapshot.html"), COMPOSITION_HTML);
  return projectDir;
}
...
describe("registerProjectRoutes — composition discovery (#1384)", () => {
  it("excludes HTML inside dot-directories from compositions", async () => {
    ...
    expect(payload.compositions).not.toContain(".shiftcut/examples/preset.html");
  });
  ...
  it("lists vendored dot-directory files in the file tree but hides Studio backups", async () => {
    ...
    expect(payload.files).toContain(".shiftcut/examples/preset.html");
    // Only Studio's own backup snapshots are hidden from the tree (#1366).
    expect(payload.files).not.toContain(".shiftcut/backup/snapshot.html");
  });
});
