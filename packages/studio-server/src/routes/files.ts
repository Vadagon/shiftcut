...
import type { GsapAnimation } from "@shiftcut/parsers";
import { classifyPropertyGroup } from "@shiftcut/parsers/gsap-constants";
import { parseGsapScriptAcorn } from "@shiftcut/parsers/gsap-parser-acorn";
import { unrollComputedTimeline } from "@shiftcut/parsers";
import {
...
async function loadGsapParser() {
  return import("@shiftcut/parsers/gsap-parser-recast");
}
...
> {
  ...
  if (!block && (firstMutation.type === "add" || firstMutation.type === "add-with-keyframes")) {
    ...
    const { GSAP_CDN } = await import("@shiftcut/core");
    const bootstrap = [
...
async function applyGsapMutations(
  ...
  try {
    writer = resolveGsapWriter({
      SHIFTCUT_GSAP_WRITER: process.env["SHIFTCUT_GSAP_WRITER"],
    });
  } catch (error) {
...
export function registerFileRoutes(api: Hono, adapter: StudioApiAdapter): void {
  ...
  api.put("/projects/:id/files/*", async (c) => {
    ...
    const writeToken = createWriteToken(c.req.header("X-Shiftcut-Write-Token"));
    ...
    return serializeAtomicCut(async () => {
      ...
      const writeToken = createWriteToken(
        typeof body.transactionToken === "string"
          ...
          : c.req.header("X-Shiftcut-Write-Token"),
      );
