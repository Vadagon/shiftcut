...
function createProjectDir(): string {
  ...
  mkdirSync(join(projectDir, ".shiftcut"));
  writeFileSync(join(projectDir, ".shiftcut", "preset.html"), "<html><body>junk</body></html>");
  return projectDir;
}
...
describe("registerLintRoutes — dot-directory exclusion (#1384)", () => {
  it("does not lint HTML inside dot-directories", async () => {
    ...
    expect(lintedFiles).not.toContain(".shiftcut/preset.html");
  });
});
