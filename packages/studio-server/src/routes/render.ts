...
import { VALID_CANVAS_RESOLUTIONS, type CanvasResolution } from "@shiftcut/parsers";
import { formatRenderOutputTimestamp, parseFps } from "@shiftcut/core";
...
export function registerRenderRoutes(api: Hono, adapter: StudioApiAdapter): void {
  ...
  api.post("/projects/:id/render", async (c) => {
    ...
    const body = (await c.req.json().catch(() => ({}))) as {
      ...
      // window.__hfVariables — same channel as `shiftcut render --variables`.
