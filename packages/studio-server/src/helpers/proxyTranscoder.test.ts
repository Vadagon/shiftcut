...
afterEach(() => {
  ...
  vi.doUnmock("@shiftcut/parsers/ff-binaries");
  delete process.env.SHIFTCUT_PROXY_MAX_CONCURRENCY;
  delete process.env.SHIFTCUT_PROXY_MAX_QUEUE;
});
...
async function loadModule(
  ...
  vi.doMock("@shiftcut/parsers/ff-binaries", () => ({
...
describe("resolveProxy", () => {
  ...
  it("honors bounded concurrency and queue environment overrides", async () => {
    process.env.SHIFTCUT_PROXY_MAX_CONCURRENCY = "1";
    process.env.SHIFTCUT_PROXY_MAX_QUEUE = "0";
    const { spawn, calls } = createSpawnSpy();
    const { resolveProxy, ProxyCapacityError } = await loadModule(spawn, FFMPEG_PATH);
    const projectDir = tmpProject();
