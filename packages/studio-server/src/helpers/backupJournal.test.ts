...
describe("snapshotBeforeWrite", () => {
  ...
  it("prunes older backups for the same file", () => {
    ...
    expect(readdirSync(join(projectDir, ".shiftcut", "backup"))).toHaveLength(3);
  });
  ...
  it("does not prune backups for paths with colliding sanitized names", () => {
    ...
    const backups = readdirSync(join(projectDir, ".shiftcut", "backup"));
    expect(backups).toHaveLength(2);
    expect(
      backups
        .map((name) => readFileSync(join(projectDir, ".shiftcut", "backup", name), "utf-8"))
        .sort(),
