...
import { parseFps, fpsToNumber } from "@shiftcut/core";
import {
...
// `@shiftcut/aws-lambda`, which depends on @aws-sdk + @sparticuz/chromium.
...
// `@shiftcut/aws-lambda`, whose types come from `dist/index.d.ts` after
...
// pulls `regression-harness-lambda-local.ts` (and its `@shiftcut/aws-lambda`
...
type TestMetadata = {
  ...
  renderConfig: {
    ...
    /**
     * Render-time variable overrides, equivalent to `shiftcut render
...
type CliOptions = {
   ...
   * `@shiftcut/producer/distributed`. See
...
async function runTestSuite(
  ...
  // Use predictable temp location: /tmp/shiftcut-tests/{test-id}/
  const testsRoot = join(tmpdir(), "shiftcut-tests");
