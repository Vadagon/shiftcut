 ...
 *     `shiftcut render` CLI takes. This is what produced every existing
 *     `tests/<name>/output/output.mp4` golden baseline.
 *
 *   - `distributed-simulated` — calls `plan()` → `renderChunk()` per chunk
 *     → `assemble()` from `@shiftcut/producer/distributed`. No Temporal
 ...
 *   - `lambda-local` — drives the OSS `@shiftcut/aws-lambda` handler
