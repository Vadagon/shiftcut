...
} from "@shiftcut/parsers/sub-composition-validity";
...
import { createStudioPositionSeekReapplyScript } from "@shiftcut/studio-server/manual-edits-render-script";
import { getPositionEditsRenderScript } from "@shiftcut/core/runtime/position-edits-render";
 ...
 * and `shiftcut lint` use, so all three agree on what counts as usable.
 *
 ...
 * inlining logic from @shiftcut/core. This wrapper handles the
...
function injectTextRenderingRule(html: string): string {
  ...
  if (document.querySelector("style[data-shiftcut-text-rendering]")) {
    return html;
  }
 ...
 * `shiftcut publish`'s archive-time localize step.
 */
/** @internal exported for unit testing only */
