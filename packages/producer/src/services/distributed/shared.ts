...
import { type Fps } from "@shiftcut/core";
import { type VideoElement, type VideoFrameFormat, type VideoMetadata } from "@shiftcut/engine";
 ...
 * module until a `package.json` whose `name === "@shiftcut/producer"`
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (existsSync(candidate)) {
      try {
        ...
        if (pkg.name === "@shiftcut/producer" && typeof pkg.version === "string") {
