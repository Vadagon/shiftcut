...
import { type CanvasResolution, fpsToNumber } from "@shiftcut/core";
import {
  type EngineConfig,
...
export interface DistributedRenderConfig {
   ...
   * `@shiftcut/aws-lambda` SDK) also validate the shape client-side
...
/**
 * Hash the deterministic-font bundle that ships inside `@shiftcut/producer`.
...
export async function plan(
  ...
  if (probeResult.probeSession) {
    ...
    try {
      const { closeCaptureSession } = await import("@shiftcut/engine");
  ...
  // approach left a duplicate under `compiled/__shiftcut_video_frames/`)
  // halves peak disk usage during `plan()`.
  const stagedVideoFrames = join(compiledDir, "__shiftcut_video_frames");
