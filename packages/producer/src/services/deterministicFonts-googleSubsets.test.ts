 ...
 *
 * These tests inject `fetchImpl` (no network) and a temp `SHIFTCUT_FONT_CACHE_DIR`
...
beforeAll(() => {
  prevCacheEnv = process.env.SHIFTCUT_FONT_CACHE_DIR;
  cacheDir = mkdtempSync(join(tmpdir(), "hf-font-cache-"));
  process.env.SHIFTCUT_FONT_CACHE_DIR = cacheDir;
});

afterAll(() => {
  if (prevCacheEnv === undefined) delete process.env.SHIFTCUT_FONT_CACHE_DIR;
  else process.env.SHIFTCUT_FONT_CACHE_DIR = prevCacheEnv;
