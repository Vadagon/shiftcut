...
import { FONT_ALIAS_MAP } from "@shiftcut/core/fonts/aliases";
import {
  locateSystemFontVariants,
  SYSTEM_FONT_SIZE_LIMIT,
} from "@shiftcut/core/fonts/system-locator";
...
// FONT_ALIASES derives from the shared alias map in @shiftcut/core.
...
export { FONT_ALIAS_KEYS } from "@shiftcut/core/fonts/aliases";
...
function warnUnresolvedFonts(unresolved: string[]): void {
  ...
  defaultLogger.warn(
    `[Compiler] No deterministic font mapping for: ${unresolved.join(", ")}\n` +
      ...
      `  Docs: https://shiftcut.verblike.com/docs/fonts`,
  );
}
...
// running inside Lambda, and honor `SHIFTCUT_FONT_CACHE_DIR` as
// an explicit override for any environment.
function resolveFontCacheRoot(): string {
  return (
    process.env.SHIFTCUT_FONT_CACHE_DIR ??
    (process.env.AWS_LAMBDA_FUNCTION_NAME
      ? join(tmpdir(), "shiftcut", "fonts")
      : join(homedir(), ".cache", "shiftcut", "fonts"))
  );
}
...
export async function injectDeterministicFontFaces(
  ...
  styleEl.setAttribute("data-shiftcut-deterministic-fonts", "true");
