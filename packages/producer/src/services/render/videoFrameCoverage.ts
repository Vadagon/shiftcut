 ...
 * (both `#shiftcut-cli-feedback`, both `check`/`snapshot` pass /
 * final MP4 wrong):
 *
...
import type { ExtractedFrames, VideoElement } from "@shiftcut/engine";
...
export interface VideoFrameCoverageErrorDetails {
  readonly shiftcutVideoFrameCoverageError: true;
...
export class VideoFrameCoverageError extends Error {
  readonly shiftcutVideoFrameCoverageError = true as const;
  readonly threshold: number;
  readonly worst: VideoFrameCoverageReport;
  readonly failedReports: VideoFrameCoverageReport[];
  ...
  constructor(
    message: string,
    details: Omit<VideoFrameCoverageErrorDetails, "shiftcutVideoFrameCoverageError">,
  ) {
    super(message);
...
export function isVideoFrameCoverageError(err: unknown): err is VideoFrameCoverageError {
  return (
    typeof err === "object" &&
    err !== null &&
    (err as { shiftcutVideoFrameCoverageError?: unknown }).shiftcutVideoFrameCoverageError ===
      true
  );
}
