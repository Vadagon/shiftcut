...
import type { CaptureOptions, EngineConfig, ExtractedFrames } from "@shiftcut/engine";
import { executeParallelCapture, mergeWorkerFrames } from "@shiftcut/engine";
...
vi.mock("@shiftcut/engine", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@shiftcut/engine")>();
...
describe("createCompiledFrameSrcResolver", () => {
  it("maps extracted frame paths under compiledDir to encoded server URLs", () => {
    ...
    expect(
      resolver("/tmp/hf job/compiled/__shiftcut_video_frames/video 1/frame_00001.jpg"),
    ).toBe("/__shiftcut_video_frames/video%201/frame_00001.jpg");
  });
  ...
  it("resolves symlinked cache frames when materialized under compiledDir", () => {
    ...
    expect(resolver("/tmp/hf-job/compiled/__shiftcut_video_frames/vid1/frame_00001.jpg")).toBe(
      "/__shiftcut_video_frames/vid1/frame_00001.jpg",
    );
  ...
  it("encodes reserved characters in frame path segments", () => {
    ...
    expect(
      resolver("/tmp/hf-job/compiled/__shiftcut_video_frames/video#1/frame_00001.jpg"),
    ).toBe("/__shiftcut_video_frames/video%231/frame_00001.jpg");

    expect(
      resolver("/tmp/hf-job/compiled/__shiftcut_video_frames/video?q=1/frame_00001.jpg"),
    ).toBe("/__shiftcut_video_frames/video%3Fq%3D1/frame_00001.jpg");
  });
});
...
describe("materializeExtractedFramesForCompiledDir", () => {
  ...
  it("leaves Windows frame paths already under compiledDir unchanged", () => {
    ...
    const outputDir = win32.join(compiledDir, "__shiftcut_video_frames", "video-1");
  ...
  it("remaps Windows cache frames under compiledDir using only the frame basename", () => {
    ...
    const linkPath = win32.join(compiledDir, "__shiftcut_video_frames", "video-1");
