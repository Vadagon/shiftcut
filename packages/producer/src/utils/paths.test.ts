...
describe("isPathInside", () => {
  ...
  it("handles Windows paths under the parent directory", () => {
    expect(
      isPathInside(
        win32.resolve("C:\\compiled\\__shiftcut_video_frames\\video\\frame_000001.jpg"),
...
describe("toExternalAssetKey", () => {
  ...
  it("converts Windows drive-letter paths to a colonless, slash-delimited key", () => {
    // GH #321: `D:\coder\reactGin\shiftcut\reading\assets\segment_001.wav`
    // used to become `hf-ext/D:\coder\...`, which makes the downstream
    // `path.join(compileDir, key)` absolute on Windows (drive letter wins).
    expect(
      toExternalAssetKey("D:\\coder\\reactGin\\shiftcut\\reading\\assets\\segment_001.wav"),
    ).toBe("hf-ext/D/coder/reactGin/shiftcut/reading/assets/segment_001.wav");
  });
