...
} from "@shiftcut/core/runtime/position-edits";
...
function warnTaintOnce(src: string): void {
  ...
  console.warn(
    `[shiftcut] image-alpha hit-test unavailable for cross-origin/tainted image; treating as opaque: ${src}`,
  );
}
  ...
  attachSync(comp: Composition): () => void {
    ...
    const syncOverrides = (): void => {
      ...
      try {
        ...
        console.warn("[shiftcut] attachSync: override sync failed:", err);
      }
    };
