/**
 * SDK document model — adaptation layer on top of @shiftcut/core.
 *
 ...
 *   ShiftCutElement is the editing view (ALL editable elements, with raw attrs).
 */

import { parseHTML } from "linkedom";
import { ensureHfIds, isCompositionTemplate } from "@shiftcut/parsers/hf-ids";
import { parseGsapScriptAcornForWrite } from "@shiftcut/core/gsap-parser-acorn";
import {
  findRoot,
  getElementStyles,
  getGsapScripts,
...
import type { ShiftCutElement, SdkDocument } from "./types.js";
 ...
 */
export function buildRoots(document: Document): ShiftCutElement[] {
...
export function flatElements(roots: readonly ShiftCutElement[]): ShiftCutElement[] {
  const result: ShiftCutElement[] = [];
  function walk(el: ShiftCutElement) {
    result.push(el);
