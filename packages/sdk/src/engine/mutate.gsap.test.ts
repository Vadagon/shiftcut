...
describe("addGsapKeyframe", () => {
  ...
  it("backfills a NEW property into the other keyframes, matching the recast writer", async () => {
    ...
    const { parseGsapScript } = await import("@shiftcut/core/gsap-parser");
    const { addKeyframeToScript } = await import("@shiftcut/core/gsap-writer-acorn");
...
describe("addLabel", () => {
  ...
  it("addLabel output is not blocked by GSAP validator", async () => {
    const { validateCompositionGsap } = await import("@shiftcut/core/gsap-parser");
    const parsed = fresh();
