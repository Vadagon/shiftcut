...
const SLIDESHOW_HTML = `<html><body><script type="application/shiftcut-slideshow+json">{"slides":[]}</script></body></html>`;
...
describe("useSlideshowTabState", () => {
  ...
  it("still detects a malformed island — presence-only, not full manifest validation", () => {
    const malformed = `<html><body><script type="application/shiftcut-slideshow+json">{not valid json</script></body></html>`;
