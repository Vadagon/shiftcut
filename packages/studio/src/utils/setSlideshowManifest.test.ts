...
import { parseSlideshowManifest } from "@shiftcut/core/slideshow";
...
describe("buildSlideshowIslandHtml", () => {
  it("serializes a manifest into a script island", () => {
    ...
    expect(html).toContain('type="application/shiftcut-slideshow+json"');
...
describe("persistSlideshowManifest — op construction", () => {
  ...
  it("inserts the island when none exists in the serialized HTML", async () => {
    ...
    expect(written).toContain('type="application/shiftcut-slideshow+json"');
  });
  ...
  it("collapses two stale islands into exactly one after persist", async () => {
    ...
    const islandCount = (written.match(/type="application\/shiftcut-slideshow\+json"/g) ?? [])
      .length;
