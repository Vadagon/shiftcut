...
const OPT_OUT_KEY = "shiftcut-studio:telemetryDisabled";

function setNoTelemetry(value: string | undefined): void {
  if (value === undefined) {
    delete (import.meta.env as Record<string, unknown>).VITE_SHIFTCUT_NO_TELEMETRY;
  } else {
    (import.meta.env as Record<string, unknown>).VITE_SHIFTCUT_NO_TELEMETRY = value;
  }
}
...
describe("studio client shouldTrack", () => {
  ...
  it("returns false when VITE_SHIFTCUT_NO_TELEMETRY=1 at build time", async () => {
    setNoTelemetry("1");
    const shouldTrack = await loadShouldTrack();
    expect(shouldTrack()).toBe(false);
  });

  it("returns false when VITE_SHIFTCUT_NO_TELEMETRY='true'", async () => {
    setNoTelemetry("true");
    const shouldTrack = await loadShouldTrack();
    expect(shouldTrack()).toBe(false);
  });
