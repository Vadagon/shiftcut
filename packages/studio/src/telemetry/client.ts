...
// VITE_SHIFTCUT_NO_TELEMETRY mirrors the CLI's SHIFTCUT_NO_TELEMETRY=1
// opt-out so Verblike's own dev/CI builds can suppress telemetry from the studio
...
function isBuildTimeOptOut(): boolean {
  try {
    const v = import.meta.env.VITE_SHIFTCUT_NO_TELEMETRY as string | undefined;
    return v === "1" || v === "true";
  } catch {
    return false;
  }
...
// so developers running `shiftcut preview` don't pollute production telemetry.
...
function showNoticeOnce(): void {
  ...
  console.info(
    "%c[ShiftCut]%c Anonymous studio usage analytics enabled. " +
      "Disable: localStorage.setItem('shiftcut-studio:telemetryDisabled','1') (then reload).",
