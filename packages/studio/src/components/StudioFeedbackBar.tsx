...
function isFeedbackDisabled(): boolean {
  try {
    return import.meta.env.VITE_SHIFTCUT_NO_FEEDBACK === "1";
  } catch {
    return false;
  }
}
...
function getFeedbackInterval(): number {
  try {
    const v = import.meta.env.VITE_SHIFTCUT_FEEDBACK_INTERVAL as string | undefined;
    if (v) {
...
const STORAGE_KEYS = {
  sessionCount: "shiftcut-studio:feedbackSessionCount",
  lastPromptedAt: "shiftcut-studio:feedbackLastPromptedAt",
} as const;
...
const SESSION_COUNTED_KEY = "shiftcut-studio:feedbackSessionCounted";
  ...
  return (
    ...
    >
      {submitted ? (
        ...
        <>
          <span className="text-neutral-500 flex-shrink-0">Recommend ShiftCut?</span>
