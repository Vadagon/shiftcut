...
function buildAgentPrompt(
  ...
  const isComponent = blockType === "shiftcut:component";
  const kind = isComponent ? "component" : "block";
  const compositionInfo = formatCompositionContext(context);

  const categoryPrompts: Record<string, string> = {
    captions: [
      `Using /shiftcut, add the "${title}" caption style (registry: ${name}) to my composition.`,
      `${description}`,
    ...
    vfx: [
      `Using /shiftcut, add the "${title}" VFX (registry: ${name}) as a full-screen overlay on my composition.`,
      `${description}`,
    ...
    transitions: [
      `Using /shiftcut, add the "${title}" transition (registry: ${name}) between my scenes.`,
      `${description}`,
    ...
    effects: [
      `Using /shiftcut, add the "${title}" effect (registry: ${name}) as an overlay on my composition.`,
      `${description}`,
    ...
    social: [
      `Using /shiftcut, add the "${title}" template (registry: ${name}) to my composition.`,
      `${description}`,
    ...
    data: [
      `Using /shiftcut, add the "${title}" visualization (registry: ${name}) to my composition.`,
      `${description}`,
    ...
    scenes: [
      `Using /shiftcut, add the "${title}" scene (registry: ${name}) to my composition.`,
      `${description}`,
  ...
  const instruction =
    ...
    [
      `Using /shiftcut, add the "${title}" ${kind} (registry: ${name}) to my composition.`,
      `${description}`,
