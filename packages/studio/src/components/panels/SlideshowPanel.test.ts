...
import type { SlideshowManifest } from "@shiftcut/core/slideshow";
...
describe("safeParseManifest", () => {
  it("parses a valid slideshow island", () => {
    ...
    const island = `<script type="application/shiftcut-slideshow+json">${JSON.stringify(manifest)}</script>`;
  ...
  it("returns {slides:[]} for malformed JSON in the island", () => {
    const html = `<html><body><script type="application/shiftcut-slideshow+json">NOT_JSON</script></body></html>`;
    const result = safeParseManifest(html);
