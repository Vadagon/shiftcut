...
function getDocumentScriptSignature(doc: Document): string {
  const projectSignature = Array.from(
    doc.querySelectorAll<HTMLMetaElement>('meta[name="shiftcut-project-signature"]'),
  )
  ...
  const scriptText = Array.from(doc.querySelectorAll<HTMLScriptElement>("script"))
    ...
    .map((script) => {
      const attrs = [
        ...
        script.getAttribute("data-shiftcut-runtime") || "",
      ].join(":");
...
function createSnapshotLoadingOverlay(
  ...
  overlay.setAttribute("data-shiftcut-ignore", "");
  overlay.setAttribute("data-shiftcut-picker-block", "");
...
export function init(config: HyperShaderConfig): GsapTimeline {
  ...
  // Locally redeclared (not imported) because @shiftcut/shader-transitions
  // ships as a standalone CDN bundle and must not depend on @shiftcut/engine.
  ...
  // The Shiftcut engine injects a virtual-time shim (window.__HF_VIRTUAL_TIME__)
