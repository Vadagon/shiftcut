# @shiftcut/sdk-playground

Interactive browser playground for the `@shiftcut/sdk` API. Open a composition, edit it through the full SDK op surface, watch the preview update live.
...
Composition state is persisted to `packages/sdk-playground/composition.html` via a Vite dev-server plugin backed by `@shiftcut/sdk/adapters/fs`. Every save writes a timestamped snapshot to `.hf-versions/composition.html/` (capped at 20). Reload the page and your last state is restored.
...
- Render to video via `@shiftcut/producer` integration
