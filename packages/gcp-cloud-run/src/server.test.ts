...
describe("bucket allowlist guard", () => {
  it("throws GCS_URI_NOT_ALLOWED for an off-bucket URI", async () => {
    const gcs = new FakeGcs();
    const prev = process.env.SHIFTCUT_RENDER_BUCKET;
    process.env.SHIFTCUT_RENDER_BUCKET = "allowed-bucket";
    try {
      const event: RenderChunkEvent = {
        Action: "renderChunk",
      ...
      if (prev === undefined) delete process.env.SHIFTCUT_RENDER_BUCKET;
      else process.env.SHIFTCUT_RENDER_BUCKET = prev;
    }
  });

  it('treats SHIFTCUT_RENDER_BUCKET="*" as an explicit opt-out (off-bucket allowed)', async () => {
    const gcs = new FakeGcs();
    await seedPlanTar(gcs, "gs://any-bucket/renders/r1/plan.tar.gz", PLAN_HASH);
    const prev = process.env.SHIFTCUT_RENDER_BUCKET;
    process.env.SHIFTCUT_RENDER_BUCKET = "*";
    try {
      const event: RenderChunkEvent = {
        Action: "renderChunk",
