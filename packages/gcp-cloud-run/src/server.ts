/**
 * Cloud Run request handler for ShiftCut distributed rendering.
 *
 ...
 * primitive from `@shiftcut/producer/distributed`.
 *
 ...
 * deliberately tracks `@shiftcut/aws-lambda`'s `handler.ts` so the two
...
import {
  ...
  type PlanResult,
  renderChunk,
} from "@shiftcut/producer/distributed";
 ...
 * arbitrary GCS data.
 *
 * Opt-out is explicit: set `SHIFTCUT_RENDER_BUCKET="*"` to disable the
...
function validateEventGcsUris(event: PlanEvent | RenderChunkEvent | AssembleEvent): void {
  const allowedBucket = process.env.SHIFTCUT_RENDER_BUCKET?.trim();
  ...
  if (!allowedBucket) {
    if (!warnedAllowlistDisabled) {
      ...
      logEvent({
        ...
        level: "WARNING",
        message:
          "SHIFTCUT_RENDER_BUCKET is unset — the GCS bucket-allowlist guard is DISABLED. " +
