# @shiftcut/gcp-cloud-run

Google Cloud Run + Cloud Workflows adapter for ShiftCut distributed
...
the GCP counterpart to [`@shiftcut/aws-lambda`](../aws-lambda).
...
The package is **not** a dependency of `@shiftcut/producer`; install it
separately.
...
fixed path, and exports `SHIFTCUT_CHROME_PATH`. CDP-level `BeginFrame`
...
gcloud builds submit . \
  --tag REGION-docker.pkg.dev/PROJECT/REPO/shiftcut-render:TAG

# 2. Apply the module.
terraform -chdir=node_modules/@shiftcut/gcp-cloud-run/terraform init
terraform -chdir=node_modules/@shiftcut/gcp-cloud-run/terraform apply \
  -var project_id=PROJECT \
  -var region=us-central1 \
  -var image=REGION-docker.pkg.dev/PROJECT/REPO/shiftcut-render:TAG
```
...
```ts
import { renderToCloudRun, getRenderProgress } from "@shiftcut/gcp-cloud-run/sdk";

const handle = await renderToCloudRun({
  projectDir: "./my-composition",
  config: { fps: 30, width: 1920, height: 1080, format: "mp4" },
  bucketName: "shiftcut-render-my-project", // from terraform output
  projectId: "my-project",
  location: "us-central1",
  workflowId: "shiftcut-render",
  serviceUrl: "https://shiftcut-render-abc.us-central1.run.app",
});
