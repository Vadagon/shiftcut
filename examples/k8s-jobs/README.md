...
This directory ships a reference `Dockerfile.example` for adopters who want to run ShiftCut distributed renders **outside AWS Lambda**. The image bakes Node 22 + `chrome-headless-shell` + `ffmpeg` + the producer source, and works on Kubernetes Jobs, Argo Workflows, Cloud Run Jobs, ECS Fargate, or plain `docker run`.
...
```bash
docker build -t shiftcut-chunk-runner:local -f examples/k8s-jobs/Dockerfile.example .
```
...
```ts
import { plan, renderChunk, assemble } from "@shiftcut/producer/distributed";
...
If you want AWS Lambda specifically, use `shiftcut lambda deploy` instead — it ships a turnkey deployment. See [docs/deploy/aws-lambda.mdx](../../docs/deploy/aws-lambda.mdx).
