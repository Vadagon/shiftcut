...
Reference SAM template for deploying ShiftCut distributed rendering on
...
```bash
STACK_NAME=shiftcut-render          # whatever you picked at deploy
...
| ------------------------------- | ------------- | ----------------------------------------------------------------------------------------------- |
| `ProjectName`                   | `shiftcut` | Prefix for created resource names.                                                              |
...
```bash
sam delete --stack-name shiftcut-render
```
...
- `shiftcut lambda deploy / render / progress / destroy` CLI — follow-up.
