#!/usr/bin/env bash
# Real-AWS smoke + benchmark for the ShiftCut Lambda adapter.
#
...
#   --stack-name <name>        (default: shiftcut-lambda-smoke-<timestamp>)
...
STACK_NAME="${STACK_NAME:-shiftcut-lambda-smoke-$(date +%s)}"
...
Real-AWS smoke + benchmark for the ShiftCut Lambda adapter. Builds the
...
Flags:
  ...
  --stack-name <name>           SAM stack name (default: shiftcut-lambda-smoke-<timestamp>)
