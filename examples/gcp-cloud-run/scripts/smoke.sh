#!/usr/bin/env bash
# Real-GCP smoke + benchmark for the ShiftCut Cloud Run adapter.
#
...
#   --repo <ar-repo>          (Artifact Registry repo name, default: shiftcut)
...
if [ "$SKIP_BUILD" -eq 1 ] && [ -f "$IMAGE_TXT" ]; then
  ...
  IMAGE="$REGION-docker.pkg.dev/$PROJECT/$AR_REPO/shiftcut-render:$TAG"
