...
You build one frame's composition file: `compositions/frames/<frame_id>.html`. Siblings build the other frames in parallel. The generic ShiftCut law — sub-composition shape, timeline registration, determinism, layout — lives in `shiftcut-core` (`references/sub-compositions.md`
...
- Seek-safe per `shiftcut-core/determinism-rules.md` (derive variation from indices; swap text / numbers with `tl.set`).
