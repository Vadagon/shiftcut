...
//
// Reads:  --storyboard STORYBOARD.md, --shiftcut <root>, [--audiomap audiomap.json],
...
const audiomapPath = resolve(flag("audiomap", join(shiftcutDir, "audiomap.json")));
...
for (const m of mounted) {
  ...
  if (v?.path && existsSync(join(shiftcutDir, v.path))) {
    body.push(
...
if (existsSync(join(shiftcutDir, bgmRel))) {
