...
Initialize only if `shiftcut.json` is missing. Name `<project>` from the brief in kebab-case, such as `midnight-drive-loop` — never a timestamp. `init` checks the installed skills against the latest on GitHub and updates the global set if any are out of date.
...
node <SKILL_DIR>/scripts/stage-assets.mjs --from <dir> --shiftcut "$PROJECT_DIR" --into public
```
...
**Gate:** `shiftcut.json` + `assets/bgm.mp3` exist; aspect / length / fps and (if any) the asset inventory are noted.
...
1. **Pick the brand.** Choose one preset from `../shiftcut-creative/frame-presets/` using the table in `../shiftcut-creative/references/design-spec.md` (match the track's mood; **only its fonts and colors matter** — templates own composition). Copy it into `frame.md` **unmodified** and fill the frontmatter `style` (font + a ≤4–6 swatch palette) from it.
...
Create `compositions/frames/`. Read [`sub-agents/frame-worker.md`](sub-agents/frame-worker.md) and `../shiftcut-core/references/subagent-dispatch.md`. Dispatch **one frame-worker per frame**, in parallel where possible (otherwise in waves). Each worker gets exactly one frame and this context:
...
node <SKILL_DIR>/scripts/assemble-index.mjs --storyboard "$PROJECT_DIR/STORYBOARD.md" \
  --shiftcut "$PROJECT_DIR" --audiomap "$PROJECT_DIR/audiomap.json"
```
...
```bash
( cd "$PROJECT_DIR" && npx shiftcut check . --snapshots )
```
...
```bash
( cd "$PROJECT_DIR" && npx shiftcut render . --skill=music-to-video -q draft -o renders/video.mp4 --fps 30 )
```
...
**Scripts** under `scripts/`: `analyze-beatgrid.py` (the one analyzer), `validate-plan.mjs` (plan check), `assemble-index.mjs` (index assembly), `stage-assets.mjs` (stage user media), `lib/storyboard.mjs` (vendored parser). Everything else is the `shiftcut` CLI.
...
| `../shiftcut-core/references/subagent-dispatch.md`                                                          | Step 4: dispatch sub-agents safely                      |
| `../shiftcut-creative/references/design-spec.md`                                                            | Step 3: pick the preset (the brand)                     |
