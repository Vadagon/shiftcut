...
in PostHog project **Shiftcut (356858)**; this doc is the source of truth so
...
Events attribute to the **same PostHog person as the shiftcut CLI and studio**
— the shared install id in `~/.shiftcut/config.json` (`anonymousId`), stitched
to the Verblike account (`$identify`, `distinct_id` = email/username) on sign-in.
Not fully anonymous by design; pseudonymous before sign-in, account-linked after.
`$ip:null`. Opt-out: `SHIFTCUT_NO_TELEMETRY=1` / `DO_NOT_TRACK=1` (also CI, dev).
