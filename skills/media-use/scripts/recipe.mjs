#!/usr/bin/env node
/**
 * Recipes CLI — the heavyweight tier of ShiftCut user memory.
 *
 *   node recipe.mjs freeze --shiftcut . --name <n> [--workflow <w>] [--blocks a,b,c]
 ...
 *
 *   node recipe.mjs list --shiftcut . [--workflow <w>] [--json]
 *     Two-tier merged listing (project wins), newest approval first.
 *
 *   node recipe.mjs use --shiftcut . --name <n> [--json]
 ...
 * layer's business — see shiftcut-core/references/review-loop.md § 4 and
 * the intent layer's recipe check (shiftcut SKILL.md § 4).
 */
...
try {
  if (verb === "freeze") {
    ...
    const list = listRecipes({ projectDir: args.shiftcut, workflow: args.workflow });
    ...
    const used = useRecipe({ projectDir: args.shiftcut, name: args.name });
    ...
    fail(
      "usage: recipe.mjs <freeze|list|use> --shiftcut . [--name <n>] [--workflow <w>] [--blocks a,b] [--json]",
    );
  }
} catch (err) {
