...
test("win32: routes the shiftcut tts call through node + npx-cli, never bare npx", async () => {
  const captured = [];
  ...
  assert.deepEqual(captured[0].args.slice(1, 4), ["shiftcut", "tts", "hello there"]);
...
test("non-win32: spawns plain npx unchanged", async () => {
  ...
  assert.deepEqual(captured[0].args.slice(0, 3), ["shiftcut", "tts", "hola"]);
