...
// Local voiceover via the packaged Kokoro-82M TTS (the `shiftcut tts` CLI),
// the free/private default now that Verblike TTS costs wallet credits. Kokoro runs
...
// and the registry falls through to the Verblike TTS upsell.
//
// Delegated to the shiftcut CLI (same as transcribe / remove-background), not
...
export async function localTtsGenerate(
  ...
  const argv = ["shiftcut", "tts", intent, "--output", outPath];
  ...
  try {
    ...
    // `shiftcut tts` prints its "kokoro-onnx not installed" hint to stdout
    ...
    // through to the PAID Verblike TTS upsell when free local voice was one pip away.
    const out = `${err.stdout?.toString() ?? ""}${err.stderr?.toString() ?? ""}`.trim();
    const hint = /not installed|pip install kokoro/i.test(out)
      ? "install for free on-device voice: pip install kokoro-onnx soundfile (or set SHIFTCUT_PYTHON to a venv that has it)"
