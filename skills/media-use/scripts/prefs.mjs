#!/usr/bin/env node
/**
 * Remembered defaults CLI — the lightweight tier of ShiftCut user memory.
 *
 *   node prefs.mjs get --shiftcut . [--json]
 ...
 *
 *   node prefs.mjs record --shiftcut . --key destination --value x-feed [--workflow <w>]
 ...
 *
 * Consumption rules live in shiftcut-core/references/brief-contract.md § 2
...
if (verb === "get") {
  const merged = mergedPreferences(args.shiftcut);
  if (args.json) {
  ...
  fail(
    "usage: prefs.mjs <get|record> --shiftcut . [--key <k> --value <v> --workflow <w>] [--json]",
  );
}
