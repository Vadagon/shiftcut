...
// `shiftcut transcribe` (99 languages; the CLI resolves/builds whisper.cpp
...
// whisper.cpp via the shiftcut CLI (fetched/built on first use — see
...
function runWhisper() {
  ...
  try {
    ...
    const resolved = resolveNpxInvocation(
      ["shiftcut", "transcribe", inputPath, "--dir", workDir],
      { stdio: ["ignore", "pipe", "pipe"], timeout: 1_800_000 },
    );
...
try {
  ...
  if (engine === "parakeet") {
    if (!parakeetBin) {
      throw new Error(
        "parakeet-mlx not found (checked $SHIFTCUT_PARAKEET, ~/.venvs/parakeet, and PATH). Install: uv venv ~/.venvs/parakeet && VIRTUAL_ENV=~/.venvs/parakeet uv pip install parakeet-mlx (or use --engine whisper)",
      );
    }
