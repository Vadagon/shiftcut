...
```bash
npx shiftcut remove-background subject.mp4 -o transparent.webm          # default: VP9 + alpha
npx shiftcut remove-background subject.mp4 -o transparent.mov           # ProRes 4444 (editing)
npx shiftcut remove-background portrait.jpg -o cutout.png               # single-image cutout
npx shiftcut remove-background subject.mp4 -o subject.webm \
...
npx shiftcut remove-background --info                                   # detected providers
```
...
`auto` (default) picks CoreML on Apple Silicon, CUDA when available, otherwise CPU. Force with `--device cpu | coreml | cuda`. CUDA requires `SHIFTCUT_CUDA=1` plus a GPU-enabled `onnxruntime-node` build. Use `--info` to inspect detected providers without rendering.
...
<h1 id="headline" style="z-index:2; ...">MAKE IT IN SHIFTCUT</h1>
