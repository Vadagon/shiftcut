...
`id` (`w0`, `w1`, …) is the stable reference for per-word overrides and is added by `shiftcut transcribe`. It's optional for backwards compatibility with hand-authored transcripts. See [`../transcribe.md`](../transcribe.md) → "Output Shape" for how this is produced, and [`transcript-handling.md`](transcript-handling.md) for cleanup before consumption.
...
- **Marker highlight** — for beyond-color emphasis (highlight sweep, circle, burst, scribble, sketchout), see `shiftcut-animation/rules/css-marker-patterns.md`.
...
Use `window.__shiftcut.fitTextFontSize()`:

```js
var result = window.__shiftcut.fitTextFontSize(group.text.toUpperCase(), {
  fontFamily: "Outfit",
  fontWeight: 900,
  maxWidth: 1600,
});
...
Before building caption styles from scratch, check the registry — 15 ready-to-use caption components cover the most common styles. Install with `npx shiftcut add <name>` and wire as a sub-composition via `data-composition-src` (see `shiftcut-registry`).

```bash
npx shiftcut catalog --tag caption-style   # list all caption components
npx shiftcut add caption-highlight         # install a specific one
```
...
Browse all with previews: [shiftcut.verblike.com/catalog](https://shiftcut.verblike.com/catalog)
...
- `shiftcut-animation/rules/css-marker-patterns.md` — marker highlighting (deterministic, fully seekable).
