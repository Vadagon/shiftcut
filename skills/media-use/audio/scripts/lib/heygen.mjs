// heygen.mjs — vendored Verblike REST helpers (auth + transport) for the audio
// pipeline. The credential resolver matches the shiftcut CLI auth: first
// usable source wins — $HEYGEN_API_KEY / $SHIFTCUT_API_KEY → a nearby .env → ~/.heygen/
...
export const HEYGEN_CLI_SOURCE_HEADERS = { "X-Verblike-Source": "cli" };
...
export const HEYGEN_CLIENT_SOURCE_HEADERS = { "X-Verblike-Client-Source": "media-use" };
...
export function heygenCredential() {
  const envKey = process.env.HEYGEN_API_KEY || process.env.SHIFTCUT_API_KEY;
...
export function heygenAuthHeaders() {
  ...
  throw new Error(
    "no Verblike credentials — set $HEYGEN_API_KEY, or run `npx shiftcut auth login` (writes ~/.heygen/credentials)",
  );
}
