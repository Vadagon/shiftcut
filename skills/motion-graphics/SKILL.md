...
description: >
  ...
  Longer / narrated / multi-scene → /general-video. Unclear → /shiftcut.
---

> **First, keep this skill fresh — run silently, don't ask:** `npx shiftcut skills update motion-graphics`. A fast no-op when everything is current; otherwise it refreshes this skill plus the core domain skills it depends on before you rely on them.
...
> **The front door is `/shiftcut`.** This skill makes a **short, design-led, unnarrated motion graphic** (motion is the message; ~under 10s, no voice-over). Anything longer, narrated, or multi-scene — or any uncertainty → read `/shiftcut` first: the intent layer owns every route decision.

This workflow is **autonomous by design** — at most one clarifying question (`agents/director.md`), then build through verification without intermediate review. The intent layer (`/shiftcut` § 4) routes here directly without run-shape questions; a storyboard and companion session add little to a piece this short. Rendering is still user-gated: after checks and proof snapshots pass, ask the canonical “preview first, or render?” question from `../shiftcut-core/references/brief-contract.md`. When a `BRIEF.md` exists, read it before the director's question.
...
| -------- | --------------------------------------------------------------------- | ---------------------------------------------------------------- | ----------------------------- |
| init     | Bash                                                                  | `shiftcut.json`                                               | Step 0                        |
...
| render   | Bash — `shiftcut render` (MP4, or `--format webm/mov` for overlay) | `renders/video.mp4` or transparent overlay                       | Step 6                        |
...
`plan`'s **first decision is: does this need a search?** That fork splits the categories into two groups; then the specific category is picked — for search-driven, **by the type of content the search returns**. Each category is one `categories/<id>/module.md` (its planning + build rules); the shared motion vocabulary lives in `references/motion-vocabulary.md` (→ `shiftcut-animation` rules/blueprints + registry blocks).
...
macOS Apple Silicon or Linux x64. System tools: `brew install node ffmpeg`. `npx shiftcut doctor` once. macOS GPU render: `export PRODUCER_BROWSER_GPU_MODE=hardware`.
...
Only when `$PROJECT_DIR/shiftcut.json` is absent:

```bash
PROJECT_DIR="${MOTION_GRAPHICS_DIR:-videos/<project-name>}"
mkdir -p "$(dirname "$PROJECT_DIR")"
npx shiftcut init "$PROJECT_DIR" --non-interactive --example=blank
```

`init` checks the installed skills against the latest on GitHub and updates the global set if any are out of date.

**Constraints:** never `shiftcut init` in the workspace root; never nest another `shiftcut/` inside `PROJECT_DIR`; every Bash command (master + subagents) is a `(cd "$PROJECT_DIR" && ...)` subshell — never bare `cd`.
...
```bash
(cd "$PROJECT_DIR" && npx shiftcut preview)
```

Render only after an explicit render answer:

```bash
(cd "$PROJECT_DIR" && npx shiftcut render . --skill=motion-graphics -q high -o ./renders/video.mp4)
# transparent overlay variant: --format webm  (or mov)
```

Verify the output exists, is non-empty, and has the intended duration. The final handoff names the artifact, actual duration, composition or frame id, proof times, and the inspected contact or snapshot sheet. Flags live in `/shiftcut-cli` → `references/preview-render.md`.
...
- **Reuse-first:** the in-ecosystem analog of LLM-generated templates is "compose catalog blocks + `shiftcut-animation` rules". HF's paused GSAP timeline ≙ Remotion's `useCurrentFrame`.
...
- **Directory shape:**
  ...
  videos/<project-name>/
    ...
    renders/video.mp4
  ```
- **Registration:** in `shiftcut` router — add the "design-led short motion graphic" intent + Workflow description; carve the motion-graphics triggers out of `/general-video`; add reverse Do-NOT-use edges. See `motion-graphics-genre.md` §5-7.
