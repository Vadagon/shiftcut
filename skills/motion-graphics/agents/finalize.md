...
1. **Snapshots** — run `npx shiftcut snapshot --at <proof-times>` and inspect overflow, off-canvas content, text collisions, empty frames, wrong content, and unreadable motion.
...
3. **Recheck** — rerun `npx shiftcut lint`, `npx shiftcut check`, and the affected snapshots. Return the result to the orchestrator without rendering.
