...
Turn `shot-plan.json` into one renderable ShiftCut composition (`compositions/index.html`). Everything stays in the HF ecosystem — HTML is the source of truth; a single **paused** GSAP timeline carries all motion; the engine seeks it. Category-specific build rules live in `categories/<id>/module.md`; this file is the shared contract.

## Reuse-first (the default)

Default = **compose existing catalog capabilities, not hand-author**:

- `npx shiftcut add <block>` (registry) → customize in place. Most blocks bake content/data into their own script (only a few expose CSS-var params), so reuse = **add + edit**.
- `shiftcut-animation` rules / blueprints / transitions for motion; runtime adapters (GSAP default).
...
- `content.block` → `shiftcut add` it (or inline) + apply `content.customize`.
...
Opacity-gate delayed elements (set hidden until their entrance). Clamp at tween bounds (no overshoot past a held value). Allowed eases: `power1–4`, `back`, `bounce`, `circ`, `elastic`, `expo`, `sine` (`.in/.out/.inOut`). One motif per scene. Run `shiftcut check` for overflow / collisions.

## Hand off for verification

Self-check the authored file, then return it to the orchestrator. Step 5 runs `shiftcut lint`, `shiftcut check`, and proof snapshots on the assembled project. Do not render. When redispatched with a finding, fix the offending element and never change a fixed `data-duration` during repair. Remotion-source migrations use `/remotion-to-shiftcut` and its SSIM harness instead.
