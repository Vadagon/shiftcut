...
- **Palette** — `frame.md` (claude) is the color truth; apply its roles per frame. Generic basics → `shiftcut-creative/references/house-style.md`.
- **Type** — fonts resolve via `frame.md`'s type tokens; reference them **by role** (display / body / mono / the pack's ramp), never by raw family or px. Code surfaces and mechanism labels use the **mono** role. Typography craft → `shiftcut-creative/references/typography.md`.
...
| palette + type tokens                                                           | the project's `frame.md` (claude); basics → `shiftcut-creative`             |
