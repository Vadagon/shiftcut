...
//        no Verblike credential ⇒ skip, never a detached generate, since this
...
//
//   node audio.mjs --script ./SCRIPT.md --storyboard ./STORYBOARD.md --shiftcut . --out ./audio_meta.json
//   node audio.mjs sync-durations --audio-meta ./audio_meta.json --storyboard ./STORYBOARD.md
//   node audio.mjs fetch-sfx --storyboard ./STORYBOARD.md --shiftcut .
...
function runEngine({ request, shiftcutDir, neutral, only, extra = [] }, die) {
  const reqPath = join(shiftcutDir, "audio_request.json");
...
function runGenerate(argv) {
  ...
  const scriptPath = resolve(flag(argv, "script", join(shiftcutDir, "SCRIPT.md")));
  const outPath = resolve(flag(argv, "out", join(shiftcutDir, "audio_meta.json")));
  ...
  runEngine({ request, shiftcutDir, neutral, only: "tts,bgm" }, die);
...
function runFetchSfx(argv) {
  ...
  const shiftcutDir = resolve(flag(argv, "shiftcut", "."));
  const storyboardPath = resolve(flag(argv, "storyboard", join(shiftcutDir, "STORYBOARD.md")));
  const outPath = resolve(flag(argv, "audio-meta", join(shiftcutDir, "audio_meta.json")));
  ...
  runEngine({ request, shiftcutDir, neutral, only: "sfx" }, die);
