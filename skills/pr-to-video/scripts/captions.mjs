...
//
//   node captions.mjs build --storyboard ./STORYBOARD.md --audio-meta ./audio_meta.json --shiftcut . --out ./caption_groups.json
//
// CAPTION LOOK — two sources, picked automatically:
//   1. PRESET SKIN (preferred). If a project-local `.shiftcut/caption-skin.html`
...
function runBuild(argv) {
  ...
  const shiftcutDir = resolve(flag(argv, "shiftcut", "."));
  const storyboardPath = resolve(flag(argv, "storyboard", join(shiftcutDir, "STORYBOARD.md")));
  const audioMetaPath = resolve(flag(argv, "audio-meta", join(shiftcutDir, "audio_meta.json")));
  const outPath = resolve(flag(argv, "out", join(shiftcutDir, "caption_groups.json")));
  const htmlPath = join(shiftcutDir, "compositions/captions.html");
  const overridesPath = join(shiftcutDir, "caption-overrides.json");
  const skinArg = flag(argv, "skin", null);
  const hiddenSkinPath = join(shiftcutDir, ".shiftcut", "caption-skin.html");
  const legacySkinPath = join(shiftcutDir, "caption-skin.html");
  const skinPath = resolve(
    skinArg ?? (existsSync(hiddenSkinPath) ? hiddenSkinPath : legacySkinPath),
  );
  const framePath = resolve(flag(argv, "frame", join(shiftcutDir, "frame.md")));
  ...
  if (existsSync(skinPath)) {
    ...
    const faces = brandFontFaces(framePath, shiftcutDir);
    ...
    source = `preset skin (${skinPath.replace(shiftcutDir + "/", "")})`;
  } else {
...
function brandFontFaces(framePath, shiftcutDir) {
  ...
  const dirs = [
    ...
    { abs: join(shiftcutDir, "assets/fonts"), rel: "assets/fonts" },
    { abs: join(shiftcutDir, "capture/assets/fonts"), rel: "capture/assets/fonts" },
...
// + style AND the <script> (the ShiftCut loader only executes scripts INSIDE the
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (sub === "build" || sub === undefined) runBuild(process.argv.slice(sub === "build" ? 3 : 2));
    ...
    console.error(
      "usage: node captions.mjs build [--storyboard …] [--audio-meta …] [--shiftcut .]",
    );
    process.exit(2);
  }
}
