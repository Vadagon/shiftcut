...
test("assembler rejects malformed worker output before writing index.html", () => {
  ...
  assert.throws(
    () =>
      execFileSync(
        process.execPath,
        [assembleScript, "--storyboard", join(project, "STORYBOARD.md"), "--shiftcut", project],
...
test("Claude preset stages renderer-parity fonts for an empty PR token set", () => {
  ...
  execFileSync(
    process.execPath,
    [buildFrameScript, "--preset", "claude", "--shiftcut", project],
    { encoding: "utf8" },
  );
...
test("bundled Claude font licenses are shipped beside the assets", () => {
  const fontDir = resolve(scriptDir, "../../shiftcut-creative/frame-presets/claude/fonts");
