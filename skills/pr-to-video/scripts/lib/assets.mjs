...
export function stageAssets({ shiftcutDir, frames }) {
  const wanted = new Set();
  for (const f of frames) {
    for (const b of basenamesFromCandidates(f.extra?.asset_candidates)) wanted.add(b);
  }
  const captureDirs = [
    join(shiftcutDir, "capture/assets"),
    join(shiftcutDir, "capture/assets/videos"), // videos download into a subdir
    join(shiftcutDir, "capture/screenshots"),
  ];
  const assetsDir = join(shiftcutDir, "assets");
  const anomalies = [];
  let staged = 0;
  if (wanted.size > 0) {
