...
//
//   node build-frame.mjs --preset capsule --shiftcut .
...
const presetDir = resolve(
  flag("preset-dir", join(__dirname, "../../shiftcut-creative/frame-presets")),
);
const tokensPath = resolve(flag("tokens", join(shiftcutDir, "capture/extracted/tokens.json")));
...
if (brandFonts.length) {
  ...
  const srcDirs = [
    join(shiftcutDir, "capture/assets/fonts"),
    join(shiftcutDir, "assets/fonts"),
  ...
  const outDir = join(shiftcutDir, "assets/fonts");
  const faces = [];
...
if (existsSync(presetSkin)) {
  const skinDir = join(shiftcutDir, ".shiftcut");
...
console.log(
  `  .shiftcut/caption-skin.html: ${skinCopied ? "copied" : "preset ships none — captions will use the default pill"}`,
);
