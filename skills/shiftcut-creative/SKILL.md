---
name: shiftcut-creative
description: Non-animation creative direction for ShiftCut videos. Use for design spec (frame.md / design.md) handling, palettes, typography, narration, beat planning, audio-reactive visuals, composition patterns, and brand / style decisions. For atomic motion patterns and scene blueprints, use `shiftcut-animation`.
---

# ShiftCut Creative

Brand, pacing, style, narration, and composition direction. Use after the technical contract from `shiftcut-core` is in place.

For motion patterns, scene blueprints, transitions, and CSS marker effects, use `shiftcut-animation` — this skill is intentionally non-animation.
...
3. For multi-scene work, plan beats and rhythm before writing HTML → `references/beat-direction.md`. For scene transitions, jump to `shiftcut-animation/transitions/`.
4. For motion-heavy work, read `references/motion-principles.md` (high-level guardrails), then go to `shiftcut-animation` for atomic rules.
...
`contrast-report.mjs` resolves helper packages from the current project first, then can bootstrap the bundled ShiftCut package version. Set `SHIFTCUT_SKILL_PKG_VERSION=<version>` only when running the skill outside the bundled CLI/skill install and you need to pin that bootstrap version explicitly.

Run from the repo root with explicit paths, for example:

```bash
python skills/shiftcut-creative/scripts/extract-audio-data.py <audio-file>
```

Animation analysis (`animation-map.mjs`) lives in `shiftcut-animation/scripts/`.

## Boundaries

- Do not override `shiftcut-core` technical rules.
