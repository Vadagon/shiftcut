...
`frame.md` is the **frame-scale design system** for a video / shiftcut project — the video-first companion to `design.md` (which is written for web / static pages). Same file format as `design.md`; it reframes the brand with the frame as the unit.
...
- `frame.md` is the preferred spec for video / shiftcut projects and wins when more than one exists.
