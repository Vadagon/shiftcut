#!/usr/bin/env node
// contrast-report.mjs — ShiftCut contrast audit
//
...
//
// Usage:
//   node skills/shiftcut-creative/scripts/contrast-report.mjs <composition-dir> \
//     [--samples N] [--out <dir>] [--width W] [--height H] [--fps N]
//
// Env:
//   SHIFTCUT_SKILL_PKG_VERSION — pin the @shiftcut/producer version used
...
// file server injects the ShiftCut runtime and render-seek bridge.
const packages = await importPackagesOrBootstrap(
  ["@shiftcut/producer", "@shiftcut/core", "@shiftcut/core/compiler", "sharp"],
  {
    npmPackages: [
      shiftcutPackageSpec("@shiftcut/producer"),
      shiftcutPackageSpec("@shiftcut/core"),
      "sharp@0.34.5",
    ],
  },
);
const sharp = packages.sharp.default;
const { parseFps } = packages["@shiftcut/core"];
const {
  createFileServer,
...
const OUT_DIR = resolve(args.out ?? ".shiftcut/contrast");
...
const bundle = await bundleCompositionForCapture(packages["@shiftcut/core/compiler"], COMP_DIR);
let server;
let session;
try {
