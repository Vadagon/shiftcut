...
```bash
npx shiftcut preview                   # serve current directory
npx shiftcut preview --port 4567       # custom port (default 3002)
npx shiftcut preview --selection --json # print the current Studio selection and exit
npx shiftcut preview --context --json  # print compact agent context from Studio
```
...
Use the actual port and project directory name; treat `index.html` as source-code context, not the preview surface. For example, after `npx shiftcut preview --port 3017` in `codex-openai-video`, report `http://localhost:3017/#project/codex-openai-video`.
...
```bash
npx shiftcut preview --context --json --context-fields selection
npx shiftcut preview --context --json --context-fields lint
npx shiftcut preview --context --json --context-fields selection,lint
```
...
```bash
npx shiftcut preview --context --json --context-fields selection --context-detail full
```
...
| Code                       | Meaning                                                                    |
| -------------------------- | -------------------------------------------------------------------------- |
| `preview-not-running`      | Start Studio first with `npx shiftcut preview`.                         |
...
Both `preview` and `play` can open inside an explicit Chromium-compatible browser instead of the OS default. Two use cases: isolated Chromium profile, or external CDP attach (DevTools / Playwright / Puppeteer / browser-MCP). **ShiftCut itself does not own CDP automation** — this only exposes the endpoint; whatever connects to it is your problem. Not to be confused with `--browser-gpu` (a `render` flag controlling Chrome GPU access during capture).
...
npx shiftcut preview --browser-path /usr/bin/chromium --user-data-dir /tmp/hf-profile

# Same plus a CDP endpoint on :9222 (attach DevTools / Playwright / etc.)
npx shiftcut play --browser-path /usr/bin/chromium --user-data-dir /tmp/hf-profile --remote-debugging-port 9222
```
...
```bash
npx shiftcut render                                # standard MP4 from cwd
npx shiftcut render ./my-video --output ./out.mp4  # render from outside the project dir
npx shiftcut render --output final.mp4             # named output (no timestamp)
npx shiftcut render -c compositions/intro.html -o intro.mp4  # render a specific sub-composition file
npx shiftcut render --quality draft                # fast iteration
npx shiftcut render --fps 60 --quality high        # final delivery
npx shiftcut render --format webm                  # transparent WebM
npx shiftcut render --docker                       # byte-identical
```
...
**Parametrized renders:** the composition declares its variables on the `<html>` root with **`data-composition-variables`** — a JSON **array of declarations** (`{id, type, label, default}` per entry) that defines the schema. Scripts inside read the resolved values via `window.__shiftcut.getVariables()`. The CLI `--variables '{"title":"Q4 Report"}'` is a JSON **object keyed by id** that overrides those declared defaults for one render; missing keys fall through, so the same composition runs unchanged in dev preview and in production. Sub-comp hosts can also override per-instance with `data-variable-values`. See the `shiftcut-core` skill for the full pattern.
...
```bash
npx shiftcut feedback --rating 10                              # clean run, no notes
npx shiftcut feedback --rating 6 --comment "bg <video> renders grey in multi-scene; worked around with --format png-sequence"
```
...
```text
REPRO COMMAND: cd <project path> && <HF_*/PRODUCER_* env> npx shiftcut <exact command>
...
```bash
npx shiftcut publish              # upload current project, return public URL
npx shiftcut publish ./my-video   # specific project
npx shiftcut publish --yes        # skip the confirmation prompt (scripts/CI)
```
