...
```bash
npx shiftcut lint                  # current directory
npx shiftcut lint ./my-project     # specific project
npx shiftcut lint --verbose        # info-level findings
npx shiftcut lint --json           # machine-readable
```
...
**Blind spot — media inside a sub-composition (not yet a lint rule).** A `<video>`/`<audio>` inside a `compositions/*.html` `<template>` (or nested in a wrapper `<div>` anywhere) is never seeked/decoded and renders blank/black; the automated checks all pass. Media must be a direct child of the host root (`index.html`) — see `shiftcut-core` → `variables-and-media.md`. Until a rule exists, check manually before render:
...
```bash
npx shiftcut check                    # current directory: the full browser gate
npx shiftcut check ./my-project       # specific project
npx shiftcut check --json             # agent-readable envelope {ok, lint, runtime, layout, motion, contrast, snapshots}
npx shiftcut check --snapshots        # also write overview frames (annotated) + per-finding crops
npx shiftcut check --samples 15       # denser timeline sweep (default 9)
npx shiftcut check --at 1.5,4,7.25    # explicit hero-frame timestamps
npx shiftcut check --at-transitions   # also sample every tween start/end boundary
npx shiftcut check --tolerance 4      # allowed overflow px before reporting (default 2)
npx shiftcut check --timeout 5000     # ms for the initial settle (default 3000)
npx shiftcut check --no-contrast      # skip the WCAG audit while iterating
npx shiftcut check --strict           # exit non-zero on warnings too (default: only errors)
```
...
```bash
npx shiftcut check --caption-zone "x0=0;y0=.82;x1=1;y1=1;severity=error;seek=.25,1"
npx shiftcut check --frame-check     # media (img/svg/video/canvas) out-of-frame detection
```
...
```bash
npx shiftcut snapshot                       # 5 key frames as PNG
npx shiftcut snapshot ./my-project          # specific project
npx shiftcut snapshot --frames 10           # evenly-spaced N frames
```
...
`shiftcut check --snapshots` already writes a `finding-NN-<code>.png` crop for every error finding that carries a bbox, but the same zoom is available standalone once you know what to look at:

```bash
npx shiftcut check --snapshots               # reports a finding, e.g. content_overlap on "#cta"
npx shiftcut snapshot --zoom "#cta"           # crop the element to verify the defect, at 3x density
npx shiftcut snapshot --zoom "100,50,400,300" --zoom-scale 2   # or an exact pixel region
