...
```bash
npx shiftcut info                   # project metadata
npx shiftcut info ./my-video        # specific project
npx shiftcut info --json
```
...
```bash
npx shiftcut upgrade                # check + interactive prompt
npx shiftcut upgrade --check        # check and exit, no prompt (agent-friendly)
npx shiftcut upgrade --check --json # machine-readable: current / latest / updateAvailable
npx shiftcut upgrade --yes          # print upgrade commands without prompting
```

Compares the installed CLI version against npm latest.

`--project [dir]` bumps a **project's** pinned scripts instead of the global install: it rewrites every `npx …shiftcut@<version>…` in `<dir>/package.json` (default cwd) to npm-latest. Always invoke it unpinned (`npx shiftcut@latest upgrade --project`) — a project scaffolded on an old CLI stays frozen otherwise. `--project . --check` reports the delta without writing; add `--json` for `{ changed, from, to, path }`. Pass the dir explicitly whenever another flag follows `--project` — on older releases a bare `--project` consumes the next flag as its directory value.

## compositions, docs

```bash
npx shiftcut compositions           # list compositions in project
npx shiftcut compositions --json
npx shiftcut docs                   # list available topics
npx shiftcut docs rendering         # print one topic inline in the terminal
```
...
```bash
npx shiftcut benchmark              # run the preset matrix in current project
npx shiftcut benchmark ./my-video   # specific project
npx shiftcut benchmark --runs 5     # repeat each config N times (default 3)
npx shiftcut benchmark --json
```
...
```bash
npx shiftcut telemetry status      # show telemetry state
npx shiftcut telemetry disable     # disable anonymous usage telemetry
npx shiftcut telemetry enable      # re-enable telemetry
```

Telemetry is anonymous usage counters only. Disable globally with `SHIFTCUT_NO_TELEMETRY=1` if env-var control is preferred over the subcommand.
...
```bash
npx shiftcut tts
npx shiftcut transcribe
npx shiftcut remove-background
```
