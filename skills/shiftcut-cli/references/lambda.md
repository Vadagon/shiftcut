...
Use `shiftcut lambda` when the user explicitly wants self-managed AWS infrastructure or needs distributed rendering. It wraps `@shiftcut/aws-lambda` and AWS SAM.
...
```bash
npx shiftcut lambda deploy
npx shiftcut lambda render ./my-project --width 1920 --height 1080 --wait
npx shiftcut lambda destroy
```
...
Builds `packages/aws-lambda/dist/handler.zip` and SAM-deploys the stack (Lambda + Step Functions + S3 + IAM). Idempotent — re-running on the same `--stack-name` is a no-op when nothing changed. Writes `<cwd>/.shiftcut/lambda-stack-<name>.json` so later subcommands don't need to call `describe-stacks`.

| Flag              | Default                         | Description                            |
| ----------------- | ------------------------------- | -------------------------------------- |
| `--stack-name`    | `shiftcut-default`           | CloudFormation stack name              |
...
```bash
npx shiftcut lambda sites create ./my-project
# → siteId: abc1234deadbeef0  (stable across re-runs of the same tree)

npx shiftcut lambda render ./my-project --site-id=abc1234deadbeef0 ...
```
...
```bash
npx shiftcut lambda render ./my-project \
...
For variable-driven templates, declare the schema in the composition and pass either `--variables` or `--variables-file`, never both. `--strict-variables` checks local project input before any render starts. Also read [`variables-and-media.md`](../../shiftcut-core/references/variables-and-media.md#variables).
...
```bash
npx shiftcut lambda render-batch ./template \
  --batch ./users.jsonl \
...
```bash
npx shiftcut lambda progress hf-render-abcd1234
npx shiftcut lambda progress arn:aws:states:us-east-1:...:execution:...
```
...
```bash
npx shiftcut lambda policies user                                  # inline policy for an IAM user
npx shiftcut lambda policies role                                  # { TrustRelationship, InlinePolicy }
npx shiftcut lambda policies validate ./infra/iam/hf-deploy.json   # CI gate
```
...
`shiftcut lambda` stores per-stack metadata under `<cwd>/.shiftcut/lambda-stack-<name>.json` (bucket name, state-machine ARN, region). Not secret, but AWS-account-identifying. Commit it to a repo or `.gitignore` it per your workflow.
