...
Use `shiftcut cloudrun` only when the user explicitly wants self-managed Google Cloud infrastructure. It deploys Cloud Run, Workflows, and Cloud Storage. For a managed default use `shiftcut cloud`; for AWS use `shiftcut lambda`.
...
```bash
npx shiftcut cloudrun deploy --project <gcp-project> --region us-central1
npx shiftcut cloudrun sites create ./project
npx shiftcut cloudrun render ./project --width 1920 --height 1080 --wait
npx shiftcut cloudrun progress <execution-name>
npx shiftcut cloudrun destroy --project <gcp-project>
```

`deploy` enables the required Google APIs, builds or accepts a container image, applies the bundled Terraform module, and stores the resulting coordinates in `~/.shiftcut/cloudrun-state.json`. Use deploy flags such as `--image`, `--repo`, `--cpu`, `--memory`, `--max-instances`, and `--timeout` only when the infrastructure needs those overrides.
...
```bash
npx shiftcut cloudrun render ./template \
...
Use exactly one of `--variables` and `--variables-file`. Read [`variables-and-media.md`](../../shiftcut-core/references/variables-and-media.md#variables) for the composition-side contract.

## JSONL batches

```bash
npx shiftcut cloudrun render-batch ./template \
  --batch ./users.jsonl \
  --width 1920 --height 1080 \
  --max-concurrent 10 \
  --site-id <site-id> \
