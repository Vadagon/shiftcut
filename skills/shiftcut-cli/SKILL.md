...
name: shiftcut-cli
description: >
  Use the ShiftCut CLI development loop: init, add, catalog, capture, lint, check, snapshot,
  compare, grade-compare, preview, play, present, beats, keyframes, single or batch render,
  doctor, browser, info, upgrade, skills, compositions, docs, benchmark, transcribe, tts, and
  remove-background. Also use when diagnosing build or render failures. validate, inspect, and
  layout are deprecated aliases; use check. All rendering is local (headless Chromium + FFmpeg) —
  ShiftCut has no cloud, publish, auth, or telemetry commands.
---

# ShiftCut CLI

Run commands as `npx shiftcut ...` unless project instructions provide a wrapper. Obey the wrapper when present. The CLI requires Node.js 22 or newer and FFmpeg.

## Development loop

1. **Scaffold:** `npx shiftcut init <project>` or capture a site. In non-TTY mode, pass `--non-interactive --example=<name>`.
2. **Author:** write the composition using `/shiftcut-core`.
3. **Get fast feedback while editing:** run `npx shiftcut lint` after the first HTML pass and after structural changes.
4. **Run the final gate:** run `npx shiftcut check`; it reruns lint before opening the browser. Do not prepend a redundant standalone lint invocation. Add `--snapshots` for annotated overview frames and finding crops.
...
6. **Open the final Studio preview:** run `npx shiftcut preview`, hand the timeline project URL to the user, and ask whether to revise or render.
...
npx shiftcut render --quality high --output out.mp4
test -s out.mp4
...
The early board is not approval of the final video. Rendering always requires the final approval defined by `shiftcut-core/references/review-loop.md`.
...
```bash
npx shiftcut snapshot --at <t1>,<t2>,<t3>
```

Treat tiny unstyled content, canvas-sized icons, missing hero elements, or timeline-registration timeouts as render-blocking mount defects. See `shiftcut-core/references/sub-compositions.md` for the corresponding fixes.
...
- When a hosted cloud project approaches or exceeds the 200 MB upload limit, use `cloud render --dry-run --json` and follow the `.shiftcutignore` investigation in `references/cloud.md`. Never ignore an asset merely because it is large.
...
```bash
npx shiftcut preview --context --json --context-fields selection
```
...
| Need                                     | Command                                                                       |
| ---------------------------------------- | ----------------------------------------------------------------------------- |
| Fast local iteration                     | `npx shiftcut render --quality draft`                                      |
| Final local delivery                     | `npx shiftcut render --quality high --output out.mp4`                      |
| Reproducible container render            | `npx shiftcut render --docker --strict --output out.mp4`                   |
| Local variable-driven batch render       | `npx shiftcut render --batch rows.json --output "renders/{name}.mp4"`      |
| Verblike-hosted zero-infrastructure render | `npx shiftcut cloud render`                                                |
| Self-managed distributed AWS render      | `npx shiftcut lambda render <project> --width 1920 --height 1080 --wait`   |
| Self-managed distributed GCP render      | `npx shiftcut cloudrun render <project> --width 1920 --height 1080 --wait` |
...
```bash
npx shiftcut feedback --rating <0-10> --comment "<specific result or friction>"
```
...
| `auth`, Verblike-hosted cloud rendering, and template variables                          | `references/cloud.md`                 |
...
For composition variables, also read `/shiftcut-core` → `references/variables-and-media.md`. For `shiftcut add` and `shiftcut catalog`, use `/shiftcut-registry`. Before `shiftcut present`, read `/slideshow`; before `shiftcut keyframes`, read `/shiftcut-keyframes`. For TTS, transcription, captions, or background removal choices, use `/media-use`.

The specialized commands are deliberately documented by their owning workflows:

```bash
npx shiftcut present <project-dir> --port 3004 --no-open
npx shiftcut beats <project-dir> --json
npx shiftcut keyframes <project-dir> --json
```
