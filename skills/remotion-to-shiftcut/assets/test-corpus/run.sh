...
#   - HF CLI built at packages/cli/dist/cli.js (run `bun run --filter @shiftcut/cli build`
...
require_render_tier_tools() {
  if [[ ! -f "$HF_CLI" ]]; then
    ...
    echo "       Run 'bun run --filter @shiftcut/cli build' in $REPO_ROOT" >&2
    return 2
  fi
...
echo "remotion-to-shiftcut corpus run"
echo "=================================="
