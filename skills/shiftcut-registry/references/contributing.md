...
- **Block** (`registry/blocks/`, type `shiftcut:block`) — a full standalone composition with fixed dimensions and duration. Caption styles, VFX effects, title cards, lower thirds.
- **Component** (`registry/components/`, type `shiftcut:component`) — a reusable snippet with no fixed dimensions or duration. CSS effects, text treatments, overlays that adapt to any composition size.
...
- Overflow: call `window.__shiftcut.fitTextFontSize()` on every group
...
```bash
shiftcut lint                    # 0 errors required
shiftcut check --no-contrast  # 0 console errors required
```
...
shiftcut snapshot --at "1.0,3.0,5.0,7.0"

# Publish to shiftcut.verblike.com for review
...
- **Verblike internal contributors:** run `scripts/upload-docs-images.sh` (requires AWS profile `engineering-767398024897`)
...
#    { "name": "{name}", "type": "shiftcut:block" }  (or "shiftcut:component")

# 4. Generate catalog docs page
npx tsx scripts/generate-catalog-pages.ts

# 5. Publish to shiftcut.verblike.com so reviewers can preview
npx shiftcut publish
...
# 8. Push and open PR with shiftcut.verblike.com link
git push origin feat/registry-{name}
gh pr create --title "feat(registry): {name}" --body "preview: {shiftcut.verblike.com-url}"
```
...
- [ ] `shiftcut lint` → 0 errors
- [ ] `shiftcut check` → 0 console errors
...
- [ ] `npx shiftcut publish` run (claim your project URL)
