---
name: shiftcut-keyframes
description: >
  Use when a ShiftCut composition needs seek-safe 2D/3D keyframes, GSAP
  timelines, CSS keyframes, Anime.js, WAAPI, FLIP, paths, masks, SVG morph/draw,
  text trails, 3D depth, or `shiftcut keyframes` diagnostics.
...
Use `shiftcut-animation` for broad scene recipes. Use `shiftcut-cli` for full command docs. Use `references/keyframe-patterns.md` only when choosing implementation mechanisms, not visual style.
...
4. Verify with `shiftcut lint`, `shiftcut check`, `shiftcut keyframes`, one focused `--shot`, and snapshots at proof times.
...
Keyframe camera position, camera target, object transform, material opacity, shader uniforms, and postprocess intensity through deterministic state. Render from ShiftCut time. Use `--ghost` because marker boxes cannot see internal canvas motion.
...
npx shiftcut lint
npx shiftcut check
npx shiftcut keyframes .
npx shiftcut keyframes . --json
npx shiftcut keyframes . --runtime all
npx shiftcut keyframes . --selector "<selector>" --shot "<file>" --samples <n>
npx shiftcut keyframes . --selector "<selector>" --shot "<file>" --layout strip --from <t0> --to <t1>
npx shiftcut keyframes . --shot "<file>" --ghost --angle <angle>
npx shiftcut snapshot . --at <times>
```
...
Run `shiftcut lint`, `shiftcut check`, `shiftcut keyframes`, one focused `--shot`, and snapshots. Confirm first frame, proof poses, final-minus-hold, exact final, subject-owned motion, and no debug overlays.
