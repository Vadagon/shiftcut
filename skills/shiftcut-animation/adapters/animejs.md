---
name: shiftcut-animejs
description: Anime.js adapter patterns for ShiftCut. Use when writing Anime.js animations or timelines inside ShiftCut compositions, registering animations on window.__hfAnime, making Anime.js seek-driven and deterministic, or translating Anime.js examples into render-safe ShiftCut HTML.
---

# Anime.js for ShiftCut

ShiftCut can seek Anime.js instances through its `animejs` runtime adapter. The composition owns the animation objects; ShiftCut owns the clock.
...
The adapter seeks every registered instance with `instance.seek(timeMs)`, where `timeMs` is ShiftCut time in milliseconds.
...
Use GSAP for complex scene sequencing unless the user specifically asks for Anime.js. GSAP is still the primary ShiftCut authoring path.
...
- ShiftCut adapter source: `packages/core/src/runtime/adapters/animejs.ts`.
