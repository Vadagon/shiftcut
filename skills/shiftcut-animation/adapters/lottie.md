---
name: shiftcut-lottie
description: Lottie and dotLottie adapter patterns for ShiftCut. Use when embedding lottie-web JSON animations, .lottie files, @lottiefiles/dotlottie-web players, registering instances on window.__hfLottie, or making After Effects exports deterministic in ShiftCut.
---

# Lottie for ShiftCut

ShiftCut can seek both `lottie-web` and dotLottie players through its `lottie` runtime adapter. Lottie is a strong fit because the animation timeline is already encoded in the asset; ShiftCut only needs a player object it can seek.
...
ShiftCut seeks them all to the same composition time.
...
- Translating Remotion Lottie usage into plain ShiftCut HTML.
...
- Loading a player asynchronously and registering it after ShiftCut validation has already inspected the page.
...
- ShiftCut adapter source: `packages/core/src/runtime/adapters/lottie.ts`.
