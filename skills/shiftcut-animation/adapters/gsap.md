---
name: shiftcut-gsap-adapter
description: GSAP animation API reference for ShiftCut. Use when writing seekable GSAP timelines in ShiftCut compositions, including gsap.to(), from(), fromTo(), set(), timeline position parameters, labels, easing, stagger, finite repeats, and transform performance.
---

# ShiftCut GSAP

GSAP usage scoped to ShiftCut' seek-driven render model. This skill is the GSAP reference _as constrained by ShiftCut_ — for the framework's broader composition contract see `shiftcut-core`.

## ShiftCut Contract

ShiftCut controls GSAP through its `gsap` runtime adapter. Create a paused timeline synchronously, register it on `window.__timelines` with the exact `data-composition-id`, and let ShiftCut seek it.
...
- Keep loops finite. ShiftCut renders finite video durations.
- **Render duration comes from `data-duration` on the composition root, not from GSAP timeline length.** Do not pad the timeline with empty tweens like `tl.set({}, {}, 283)` to "extend" it. (Some external docs show this trick; in ShiftCut it conflicts with the seek-driven duration model — set `data-duration` instead.)
...
- **repeat** — finite number; never `-1` in ShiftCut. Compute repeats from the visible duration.
...
ShiftCut is stricter than vanilla GSAP. Animate only:
...
> **Note**: `docs/guides/gsap-animation.mdx` lists `width`/`height`/`visibility` in its "Supported Properties" — that list is too permissive for ShiftCut composition rules. This allowlist is the canonical one. See `shiftcut-core/references/determinism-rules.md` for the full deterministic-render contract.
...
- `../rules/gsap-effects.md` — drop-in recipes: typewriter (with cursor / backspace / word rotation) + audio visualizer (uses `skills/shiftcut-creative/scripts/extract-audio-data.py`).
...
- Use infinite `repeat: -1` in ShiftCut compositions — use finite repeat counts computed from the visible duration.

## Credits And References

- ShiftCut adapter source: `packages/core/src/runtime/adapters/gsap.ts`.
