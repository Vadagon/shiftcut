---
name: shiftcut-css-animations
description: CSS animation adapter patterns for ShiftCut. Use when authoring CSS keyframes, animation-delay based timing, animation-fill-mode, animation-play-state, or CSS-only motion that ShiftCut must seek deterministically during preview and rendering.
---

# CSS Animations for ShiftCut

ShiftCut can seek CSS keyframe animations through its `css` runtime adapter. Use this for simple repeated motifs, background motion, shimmer, glow, masks, and non-sequenced decoration.
...
- ShiftCut adapter source: `packages/core/src/runtime/adapters/css.ts`.
