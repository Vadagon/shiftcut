---
name: shiftcut-three
description: Three.js and WebGL adapter patterns for ShiftCut. Use when creating deterministic Three.js scenes, WebGL canvas layers, AnimationMixer timelines, camera motion, shader-driven visuals, or canvas renders that respond to ShiftCut hf-seek events.
---

# Three.js for ShiftCut

ShiftCut supports Three.js through its `three` runtime adapter. The adapter does not own your scene. It publishes ShiftCut time and dispatches a seek event so your composition can render the exact frame.

## Contract

- Create the scene, camera, renderer, materials, and assets synchronously when possible.
- Render from ShiftCut time, not wall-clock time.
...
For anything under `three/addons/`, use an importmap so bare specifiers resolve. The ShiftCut lint recognizes both this form and the inline `+esm` import above — pick whichever your composition needs.
...
- ShiftCut adapter source: `packages/core/src/runtime/adapters/three.ts`.
