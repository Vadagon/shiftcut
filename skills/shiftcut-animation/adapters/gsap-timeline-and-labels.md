...
ShiftCut is a seek-driven runtime. Build one paused timeline per composition, attach it to `window.__timelines["<composition-id>"]`, and let ShiftCut seek it. Never call `.play()` for render-critical motion.
...
- **paused: true** — required in ShiftCut. The framework drives the playhead.
...
In ShiftCut, **do not** nest sub-composition timelines into the host. Sub-compositions loaded via `data-composition-src` are seeked independently by ShiftCut from their own `data-start`. Nesting is only for grouping pieces of the _same_ composition's timeline.
...
These are useful when previewing in the browser. In rendered output ShiftCut calls `seek()` internally — your timeline must produce identical state for the same time value every time it is seeked.
