...
- **Timeline must be paused**: `gsap.timeline({ paused: true })`. Never call `tl.play()` — ShiftCut seeks the timeline frame-by-frame deterministically
...
- `/shiftcut-animation` — timeline + tween API reference (eases, stagger, `immediateRender`, etc.)
- `/shiftcut-core` — composition wiring (`data-*` attributes, scene structure, registration contract)
- `/shiftcut-cli` — `shiftcut lint` to verify the registry key + duration match
