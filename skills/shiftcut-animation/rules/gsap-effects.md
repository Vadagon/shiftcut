# GSAP Effects for ShiftCut

Drop-in animation patterns. Each effect is self-contained (HTML + CSS + JS) and follows the ShiftCut seek-driven contract — deterministic, no randomness, timeline registered on `window.__timelines`.
...
```bash
python skills/shiftcut-creative/scripts/extract-audio-data.py audio.mp3 -o audio-data.json
python skills/shiftcut-creative/scripts/extract-audio-data.py video.mp4 --fps 30 --bands 16 -o audio-data.json
```
...
**Do NOT use async `fetch()`.** ShiftCut reads `window.__timelines` synchronously after page load — building the timeline inside `.then()` means the timeline isn't ready when capture starts.
...
**WebGL / Three.js** — ShiftCut patches `THREE.Clock` for deterministic time. Update uniforms from audio data each frame.
