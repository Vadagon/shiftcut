...
For real ASR-driven scenes, replace hardcoded TIMINGS with transcript JSON (each entry has `word`, `start_ms`, `end_ms`). Convert to seconds and feed in identically. The shape `{ [wordKey]: { start, end } }` is the same whether hand-authored or derived from `shiftcut transcribe`.
...
- `/shiftcut-animation` — single driver, multi-element envelope
- `/media-use` — `shiftcut transcribe` outputs real ASR data
- `/media-use` — pair with caption rendering
- `/shiftcut-core` — composition wiring
- `/shiftcut-cli` — `shiftcut lint`
