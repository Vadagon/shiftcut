...
- `/shiftcut-animation` — `out`-family easing, proxy-driven `onUpdate` attribute tweens, and locked-envelope coordination (`../../adapters/gsap-easing-and-stagger.md`)
- `/shiftcut-creative` — `references/typography.md` (embedded display face for a text streak), `references/video-composition.md` (solid field behind the smear)
- `/shiftcut-core` — composition wiring, determinism (finite tweens, no `Math.random`)
- `/shiftcut-cli` — `shiftcut lint` / `shiftcut check` (`check` catches a missing `#streak-blur` node or an unreferenced filter)
