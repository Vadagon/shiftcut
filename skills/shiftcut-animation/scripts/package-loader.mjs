...
//   • requires an interactive y/N (or an explicit $SHIFTCUT_SKILL_BOOTSTRAP_DEPS=1)
...
const VERSION_OVERRIDE_ENV = "SHIFTCUT_SKILL_PKG_VERSION";
const BOOTSTRAP_ENV = "SHIFTCUT_SKILL_DEPS_BOOTSTRAPPED";
const BOOTSTRAP_CONFIRM_ENV = "SHIFTCUT_SKILL_BOOTSTRAP_DEPS";
const NODE_MODULES_ENV = "SHIFTCUT_SKILL_NODE_MODULES";
...
export async function bundleCompositionForCapture(compiler, projectDir) {
  const compiledDir = mkdtempSync(join(tmpdir(), "shiftcut-skill-bundle-"));
  try {
...
// @shiftcut/producer predates the isTransientBrowserError re-export. The
 ...
 * @shiftcut/producer): on a transient failure, close the crashed session
 ...
 * "Runtime ready: false").
 *
 * `producer` is the imported @shiftcut/producer namespace;
...
export function shiftcutPackageSpec(packageName) {
  const override = process.env[VERSION_OVERRIDE_ENV]?.trim();
  if (override) return `${packageName}@${override}`;

  const version = readBundledShiftcutVersion();
  if (version) return `${packageName}@${version}`;

  // Global skill installs (e.g. ~/.claude/skills) have no shiftcut package.json
  ...
  process.stderr.write(
    [
      `shiftcut: could not determine the bundled version for ${packageName}; using @latest.`,
...
function resolvePackageEntry(packageName) {
  ...
  for (const base of bases) {
    ...
    try {
      return createRequire(join(normalized, "__shiftcut_skill_loader__.cjs")).resolve(
        packageName,
      );
    } catch {
...
function readBundledShiftcutVersion() {
...
function readPackageVersion(packageJsonPath) {
  try {
    ...
    if (manifest.name === "shiftcut" || manifest.name === "@shiftcut/cli") {
...
async function confirmBootstrap(packageSpecs) {
  ...
  try {
    const answer = await rl.question(
      [
        "ShiftCut helper package(s) are missing.",
...
function bootstrapWithNpmInstall(packageNames) {
  const installRoot = mkdtempSync(join(tmpdir(), "shiftcut-skill-deps-"));
