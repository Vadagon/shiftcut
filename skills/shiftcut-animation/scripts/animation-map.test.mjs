...
const HELPERS = [
  join(REPO_ROOT, "skills", "shiftcut-animation", "scripts", "animation-map.mjs"),
  join(REPO_ROOT, "skills", "shiftcut-creative", "scripts", "contrast-report.mjs"),
];

describe("ShiftCut skill helpers", () => {
  for (const helper of HELPERS)
    it(`${helper.split("/").at(-1)} bundles modular input and uses rational fps`, () => {
      const root = mkdtempSync(join(tmpdir(), "shiftcut-skill-helper-test-"));
      const packageDir = join(root, "node_modules", "@shiftcut", "producer");
      const corePackageDir = join(root, "node_modules", "@shiftcut", "core");
      ...
      writeFileSync(
        ...
        JSON.stringify({ name: "@shiftcut/producer", type: "module", exports: "./index.mjs" }),
      );
      writeFileSync(
      ...
      try {
        const result = spawnSync(
          ...
          [helper, compositionDir, "--fps", "30000/1001", "--out", join(root, "output")],
          {
            ...
            env: {
              ...process.env,
              SHIFTCUT_SKILL_NODE_MODULES: join(root, "node_modules"),
            },
          },
        );
...
describe("package-loader parity", () => {
  it("package-loader.mjs is byte-identical to shiftcut-creative's copy (the stated contract)", () => {
    const here = readFileSync(
      join(REPO_ROOT, "skills", "shiftcut-animation", "scripts", "package-loader.mjs"),
      "utf8",
    );
    const sibling = readFileSync(
      join(REPO_ROOT, "skills", "shiftcut-creative", "scripts", "package-loader.mjs"),
      "utf8",
    );
...
function runHelper(helper, root, compositionDir) {
  const result = spawnSync(process.execPath, [helper, compositionDir, "--out", join(root, "out")], {
    encoding: "utf8",
    env: { ...process.env, SHIFTCUT_SKILL_NODE_MODULES: join(root, "node_modules") },
  });
...
describe("transient-init retry", () => {
  for (const helper of HELPERS) {
    it(`${helper.split("/").at(-1)} retries a transient zero-duration init once with a fresh session`, () => {
      const root = mkdtempSync(join(tmpdir(), "shiftcut-skill-retry-test-"));
      try {
