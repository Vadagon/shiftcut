...
description: Package an existing talking-head / interview / podcast video with timed, designed GRAPHIC OVERLAY cards — kinetic titles, lower-thirds, data callouts, quotes, side panels, picture-in-picture — synced to the transcript, on a 16:9 / 9:16 / 4:5 canvas of your choice; the clip plays untouched underneath. Trigger on "graphic overlays", "on-screen graphics", "package / dress up my video". Not plain subtitles (/embedded-captions). Unclear → /shiftcut.
---

> **First, keep this skill fresh — run silently, don't ask:** `npx shiftcut skills update talking-head-recut`. A fast no-op when everything is current; otherwise it refreshes this skill plus the core domain skills it depends on before you rely on them.
...
`shiftcut`. There is no fixed archetype list and no prescribed card structure —
the overlays emerge from what the transcript actually says.

> **The front door is `/shiftcut`.** This skill packages an **existing talking-head clip** with **designed graphic cards** (titles, lower-thirds, data callouts, quotes, side panels, PiP) — not plain captions (the spoken words as text). **The clip plays untouched.** Any other intent — plain subtitles, a standalone graphic, a from-scratch video — or any uncertainty → read `/shiftcut` first: the intent layer owns every route decision.
...
```bash
# shiftcut — transcription (local Whisper) + rendering the assembled HTML to MP4
npx shiftcut --help
```

This skill runs entirely on the **shiftcut** CLI plus system `ffmpeg` / `ffprobe`.
Transcription is local **Whisper** via `shiftcut transcribe` — no third-party
...
```bash
npx shiftcut doctor          # ffmpeg, headless browser, render deps
...
Transcription needs no key — `shiftcut transcribe` runs Whisper locally (Step 4).

Strongly recommended on macOS for `shiftcut render`:
...
```bash
npx shiftcut transcribe "$WORK_DIR/audio.mp3" -d "$WORK_DIR" --json --model small.en
```
...
— see the `timeline_track_too_dense` ShiftCut lint warning.
...
- If the user has already pre-approved defaults ("just use defaults",
  ...
  `../shiftcut-core/references/brief-contract.md` § 1) — **skip
...
**Hard rules** (`shiftcut` lint will reject violations):
...
<html lang="en">
  <head>
   ...
   the ShiftCut renderer's static analyzer doesn't expand CSS variables when
  ...
  <body>
    ...
    >
      ...
      <!--   - "clip"       → ShiftCut runtime uses this to enforce visibility     -->
...
#### ShiftCut Layout / Animation QA Rules
...
- Every timed element (`card-host`, sub-composition, etc.) MUST include `class="clip"` alongside its own classes — e.g. `class="card-host clip"`. The ShiftCut runtime uses `.clip` to gate visibility to the `data-start … data-start+data-duration` window. Without it the element is visible for the whole video (lint: `timed_element_missing_clip_class`).
- For body / global `font-family`, list **concrete font names** (`'Inter', 'Caveat', …`) — not a CSS variable like `var(--font-family)`. The ShiftCut font resolver doesn't expand CSS vars during static analysis (lint: `font_family_without_font_face`). Cards may still use `var(--font-family)` internally since their `@font-face` declarations are loaded.

### 10. Render to MP4

```bash
cd "$WORK_DIR"
PRODUCER_BROWSER_GPU_MODE=hardware npx shiftcut render public \
  --skill=talking-head-recut \
  -o output.mp4 \
  --fps 30
```

`shiftcut render <dir>` reads `<dir>/index.html` and produces the MP4.
...
```bash
npx shiftcut snapshot public --at 5    # → public/snapshots/frame-00-at-5s.png (a single --at ignores --out)
```
...
```bash
(cd "$WORK_DIR/public" && npx shiftcut preview)   # or `npx shiftcut play` for a shareable link
```
