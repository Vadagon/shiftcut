#!/usr/bin/env bash
# End-to-end: shiftcut render + ffmpeg overlay matte → final.mp4
#
# Usage:
#   bash render-and-composite.sh <project-dir> [shiftcut-repo-path]
#
# Env:
#   SHIFTCUT_ROOT  override the shiftcut checkout location

set -euo pipefail

PROJECT="${1:?usage: render-and-composite.sh <project-dir> [shiftcut-repo]}"
PROJECT="$(cd "$PROJECT" && pwd)"

# Resolve the shiftcut checkout. Candidate order:
#   1. arg 2   2. $SHIFTCUT_ROOT   3. repo root if this skill ships INSIDE the
#   shiftcut repo (skills/embedded-captions/scripts → ../../..)   4. ~/Downloads/shiftcut
SKILL_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
HF=""
for cand in "${2:-}" "${SHIFTCUT_ROOT:-}" "$(cd "$SKILL_SCRIPT_DIR/../../.." 2>/dev/null && pwd)" "$HOME/Downloads/shiftcut"; do
  if [[ -n "$cand" && -f "$cand/packages/cli/dist/cli.js" ]]; then HF="$cand"; break; fi
done
if [[ -z "$HF" ]]; then
  echo "[render] shiftcut CLI not found. Set SHIFTCUT_ROOT to your shiftcut" >&2
  echo "         checkout (needs packages/cli/dist/cli.js — 'bun install && bun run build')." >&2
  exit 1
fi
export SHIFTCUT_ROOT="$HF"   # so the occlusion gate's measure-layout.cjs finds puppeteer too
...
# Embed template fonts BEFORE the gates + render. shiftcut only auto-supplies
...
echo "[render] shiftcut render @ ${FPS}fps"

# Shiftcut occasionally hangs on Chromium shutdown *after* the output file
...
# hf_render_dir: render one shiftcut composition.
...
# the ORIGINAL video filename `shiftcut init` scaffolds (e.g. clip.mp4), not just a
...
# Hybrid renders need 2 independent shiftcut passes. They share no state, so
...
if [[ -f "$PROJECT/index_fg.html" ]]; then
  ...
  # Standard mode: TWO independent shiftcut passes (base = index.html with the
