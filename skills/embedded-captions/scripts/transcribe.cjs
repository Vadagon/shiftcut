#!/usr/bin/env node
/*
 * transcribe.cjs — word-level transcription via shiftcut' native Whisper
...
function hfRoot() {
  ...
  console.error("[transcribe] shiftcut CLI not found — set SHIFTCUT_ROOT");
  process.exit(3);
}
...
function main() {
  ...
  // `shiftcut init` drops a whisper.cpp segment/token-format transcript.json
  ...
  // fallback shiftcut whisper.cpp. Force with TRANSCRIBE_ENGINE=whisper|whisperx.
  let words = null,
    engine = null;
  ...
  if (!words) {
    // run shiftcut Whisper → writes a flat word array to <dir>/transcript.json
    ...
    try {
      ...
      console.error("[transcribe] shiftcut whisper failed:", e.message);
      process.exit(1);
    }
