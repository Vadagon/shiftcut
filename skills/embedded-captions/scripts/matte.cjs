#!/usr/bin/env node
/*
 * matte.cjs — subject matting via shiftcut' built-in `remove-background`
 ...
 * weights — the model auto-downloads once (~168 MB) to ~/.cache/shiftcut/.
 *
 ...
 *
 * Pipeline:
 *   source.mp4 → shiftcut remove-background → ProRes 4444 .mov (lossless
 ...
 * Env:    SHIFTCUT_ROOT — shiftcut checkout (default ~/Downloads/shiftcut)
 */
...
function hfCli() {
  const roots = [
    process.env.SHIFTCUT_ROOT,
    path.resolve(__dirname, "..", "..", ".."), // skills/embedded-captions/scripts → repo root if in-repo
    path.join(os.homedir(), "Downloads", "shiftcut"),
  ].filter(Boolean);
  for (const root of roots) {
    const cli = path.join(root, "packages", "cli", "dist", "cli.js");
    if (fs.existsSync(cli)) return cli;
  }
  console.error("[matte] cannot find shiftcut cli — set SHIFTCUT_ROOT to a built checkout");
  process.exit(3);
}
...
async function main() {
  ...
  // 1) subject matte via shiftcut (ProRes 4444 keeps the alpha lossless).
  ...
  console.log(
    `[matte] shiftcut remove-background (u2net_human_seg${cached ? "" : "; first run downloads ~168 MB"})… model load takes ~1-2 min with no output — not hung`,
  );
