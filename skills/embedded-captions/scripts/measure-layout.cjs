...
// Locate shiftcut' bundled puppeteer. render-and-composite.sh exports
// SHIFTCUT_ROOT; standalone we also try the in-repo path + ~/Downloads, and
...
if (!puppeteer) {
  console.error(
    "[measure] could not locate puppeteer — set SHIFTCUT_ROOT to a built shiftcut checkout",
  );
  process.exit(3);
}

// Resolve shiftcut' bundled GSAP. The templates load GSAP from a CDN
