...
metadata:
  tags: caption, talking-head, matte, occlusion, verbatim, climax, gsap, shiftcut
---

# Caption Template — Anatomy (the shared engine)

A **caption template** = one complete, reproducible ShiftCut scene:
...
ShiftCut-native, so anyone can reproduce it:
...
For the composition contract see `shiftcut-core`; eases + the animated-property allowlist see `shiftcut-gsap`; caption grouping/positioning/exit guarantees see `shiftcut-captions`.
...
npx shiftcut transcribe subject.mp4 --model small            # → transcript.json
...
The gallery used a `setInterval` loop; ShiftCut needs the same beats as **absolute-time tweens on one paused timeline**. The cycle is **FLOW line → (FLOW line) → CLIMAX in → hold → out**. Restraint is the rule: flow stays clean; the one big mood move happens only at the climax.
...
`FLOW_IN` / `FLOW_OUT` / `CLIMAX_IN` / `CLIMAX_OUT` are the named recipes in **`_motion.md`** — each returns a GSAP tween/timeline so the per-template file just picks four names. A simpler equivalent for the flow active-word _glow_ (rather than discrete reveal) is the single-driver envelope in `shiftcut-animation/rules/asr-keyword-glow.md`.
...
- `shiftcut-captions` — transcript consumption, grouping, positioning, exit guarantees, `fitTextFontSize`.
- `shiftcut-animation/rules/asr-keyword-glow.md` — the verbatim active-word envelope.
- `shiftcut-gsap` — single paused timeline, transform aliases, ease palette.
