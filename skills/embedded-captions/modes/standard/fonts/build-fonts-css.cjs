 ...
 * deterministic on ANY machine: shiftcut' renderer only auto-supplies its
 * ~18 CANONICAL_FONTS; every other family falls back to a generic font unless
 * the HTML ships a local @font-face. (shiftcut' own font linter says exactly
...
const header =
  ...
  `   These are the template fonts NOT in shiftcut' auto-resolved set; inlining\n` +
