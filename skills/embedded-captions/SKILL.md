...
> **First, keep this skill fresh — run silently, don't ask:** `npx shiftcut skills update embedded-captions`. A fast no-op when everything is current; otherwise it refreshes this skill plus the core domain skills it depends on before you rely on them.
...
2. `shiftcut init` (skip it if the project dir already exists with the video inside — `matte.cjs`/`transcribe.cjs` adopt any video in the dir as source.mp4) → **`bash scripts/prepare.sh <project>`** (matte ∥ transcribe ∥ audio-envelope in parallel, then safe-zones v2 with scene palette/optics/lighting — one command, nothing forgotten)
...
The identity pick is a **preference gate** (`../shiftcut-core/references/brief-contract.md` § 1): in autonomous mode ("surprise me" / "decide for me"), pick from your shortlist yourself and state the one-line why instead of asking.
...
```
1. shiftcut init <project> --non-interactive --video <video.mp4>
...
- **shiftcut**, built (`packages/cli/dist/cli.js`). Scripts auto-resolve the checkout: `SHIFTCUT_ROOT` env → repo root if this skill ships _inside_ shiftcut → `~/Downloads/shiftcut`. Build with `bun install && bun run build`.
- **Node-first; two Python touchpoints via `uvx` (no manual installs):** transcription runs WhisperX through `uvx` (word-level timings; falls back per SKILL §transcription), and Theme's `drawon` setpiece shells `python3 scripts/gen-stroke-path.py` at compile time. Everything else runs on the toolchain shiftcut already ships: matting via the shiftcut CLI's **`remove-background`** (u2net_human_seg; weights auto-download once, ~168 MB, to `~/.cache/shiftcut/`), image/alpha math via **`sharp`**, layout/occlusion/overflow via **`puppeteer`**, plus **`ffmpeg`**. The scripts auto-resolve these from the shiftcut checkout — nothing extra to install.
...
- **Source video** — `matte.cjs` / `transcribe.cjs` auto-resolve `source.mp4` (or glob the clip / read `shiftcut.json`), so `shiftcut init --video X.mp4` needs no manual rename.
...
- Matting weights are NOT bundled: `matte.cjs` shells the shiftcut CLI's `remove-background`, which downloads u2net_human_seg (~168 MB, Apache-2.0) once to `~/.cache/shiftcut/background-removal/models/`. First prepare on a fresh machine needs network for that one download.
