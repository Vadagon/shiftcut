...
//
//   node transitions.mjs inject --storyboard ./STORYBOARD.md --shiftcut .
...
function extendFrameTail(shiftcutDir, frame, baseDuration, targetDuration, die) {
  if (!frame?.src || targetDuration <= baseDuration) return;
  const framePath = join(shiftcutDir, frame.src);
  let html;
  try {
...
function runInject(argv) {
  ...
  const indexPath = join(shiftcutDir, "index.html");
  const die = (msg) => {
  ...
  for (let i = 1; i < order.length; i++) {
    ...
    extendFrameTail(shiftcutDir, order[i - 1].frame, baseDuration, outgoing.duration, die);
...
function runVerify(argv) {
  const shiftcutDir = resolve(flag(argv, "shiftcut", "."));
  const storyboardPath = resolve(flag(argv, "storyboard", join(shiftcutDir, "STORYBOARD.md")));
  const indexPath = resolve(flag(argv, "index", join(shiftcutDir, "index.html")));
  const bail = (msg) => {
