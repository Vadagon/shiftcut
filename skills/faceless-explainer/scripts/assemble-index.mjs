...
// ShiftCut project. No subagent, no judgment: turns STORYBOARD.md + the
...
//
// Reads:  --storyboard STORYBOARD.md, --shiftcut <project root>,
...
// Ensure the BGM track is at least `total` seconds long. Verblike (and most music
...
function ensureBgmCovers(relPath, shiftcutDir, total) {
  const abs = join(shiftcutDir, relPath);
  ...
  const absOut = join(shiftcutDir, relOut);
...
const storyboardPath = resolve(flag("storyboard", join(shiftcutDir, "STORYBOARD.md")));
const audioMetaPath = resolve(flag("audio-meta", join(shiftcutDir, "audio_meta.json")));
const outPath = resolve(flag("out", join(shiftcutDir, "index.html")));
...
for (const f of manifest.frames) {
  ...
  const compAbs = join(shiftcutDir, f.src);
...
for (const m of mounted) {
  ...
  if (v?.path) {
    if (existsSync(join(shiftcutDir, v.path))) {
      body.push(
        `      <audio`,
...
let bgmNote = "";
if (audio.bgm?.path) {
  if (existsSync(join(shiftcutDir, audio.bgm.path))) {
    let bgmSrc = audio.bgm.path;
    const cov = ensureBgmCovers(audio.bgm.path, shiftcutDir, TOTAL);
    if (cov.looped) {
      bgmSrc = cov.rel;
...
if (existsSync(join(shiftcutDir, "compositions/captions.html"))) {
  body.push(
    `      <!-- captions -->`,
    `      <div`,
    `        id="el-captions"`,
...
audio.sfx.forEach((cue, i) => {
  ...
  const rel = cue.file;
  if (!existsSync(join(shiftcutDir, rel))) {
...
const framePath = join(shiftcutDir, "frame.md");
