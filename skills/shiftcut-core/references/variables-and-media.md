...
```js
const { title, accent } = window.__shiftcut.getVariables();
...
- Use `npx shiftcut render --variables '{"title":"Q4 Report"}'` or `--variables-file` for render-time overrides.
...
- **Do not** call `video.play()`, `audio.play()`, pause, or seek in composition code. ShiftCut owns playback.
