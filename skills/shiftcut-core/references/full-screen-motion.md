...
- **Do not animate visibility on `.clip` elements.** ShiftCut already shows/hides clips based on `data-start` and `data-duration`. Animating `display` / `visibility` on the clip itself races with the framework's own show/hide. Animate a _child wrapper_ inside the clip instead.
- **Verify intentional overflow with snapshots.** Before adding `data-layout-allow-overflow` to silence an inspect warning, run `npx shiftcut snapshot` and confirm the overflow is what you want.
