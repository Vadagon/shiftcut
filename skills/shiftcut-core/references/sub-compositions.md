...
A sub-composition is a separate HTML file embedded in a host composition. ShiftCut loads it, seeks it independently, and composites the result into the host at `data-start`.
...
**Why this happens:** standard HTML conventions tell you to put `<style>` in `<head>`. In a standalone HTML file that's correct. In a ShiftCut sub-composition it is **not** — the runtime only clones `<template>` contents, so `<head><style>` is dropped on the floor.
...
For the runtime end-to-end check (a fast `snapshot` pass + per-scene frame eyeball), see the **Visual smoke test** step in `shiftcut-cli`'s Minimum Completion Gate — that is the only gate that catches these three pitfalls.

## What ShiftCut Does With the Sub-Composition
...
**Do not** manually `master.add(child)` a sub-composition timeline into the host timeline. ShiftCut already drives them independently — nesting them in GSAP causes double-seeks.
