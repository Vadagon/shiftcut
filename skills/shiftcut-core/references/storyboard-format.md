...
A storyboard is the **plan layer** for a video — an ordered set of **frames** (key moments) in one markdown file. ShiftCut Studio renders it as a contact sheet (the Storyboard view, available by default in every Studio session). Parser: `@shiftcut/core/storyboard` → `StoryboardManifest`; read API: `GET /api/projects/<id>/storyboard`.
...
## Frame comments — `.shiftcut/frame-comments.json`
