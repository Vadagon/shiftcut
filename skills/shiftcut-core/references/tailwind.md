...
ShiftCut `init --tailwind` uses the Tailwind browser runtime pinned by the scaffold. Treat it as Tailwind v4, not Studio's Tailwind v3 setup.

## When To Use

- The project was scaffolded with `npx shiftcut init --tailwind`.
...
- Keep the readiness shim deterministic; ShiftCut waits for `window.__tailwindReady` before frame 0 capture.
...
Use Tailwind for static layout and style. Keep render-critical timing in GSAP or another seekable ShiftCut adapter.
...
npx shiftcut render . --workers 1 --quality draft --output tailwind-proof.mp4
```
...
1. Project scaffolded with `npx shiftcut init --tailwind`?
...
7. Re-run `npx shiftcut check`, then the render proof above.
